"""
Media Context Builder for the entity

Provides utilities for:
- Extracting conversation context (topic, entities, user's state)
- Formatting media context for injection into the entity's prompt
...
class MediaContextBuilder:
    """
    Builds and formats media-related context for the entity's prompt.

    Responsible for:
    1. Extracting conversation context (topic, entities, user's state)
    ...
    def extract_conversation_context(self) -> Dict[str, Any]:
        ...
        Returns:
            ...
            - user_emotional_context: str (user's apparent state)
        """
        ...
        return {
            "topic": topic,
            "entities": entities,
            "user_emotional_context": re_state
        }
    ...
    def _extract_active_entities(self, text: str) -> List[str]:
        ...
        core_entities = [
            "user", "entity", "the entity", "[cat]", "[cat]", "[cat]",
            "[pet]", "[partner]"
        ]
    ...
    def _detect_re_state(self, text: str) -> str:
        """Detect the user's emotional state from text patterns."""
        state_scores = {}
    ...
    def format_media_injection(
        ...
        """
        Create the text block for injecting into the entity's context.
    ...
    def format_media_context_for_prompt(
        ...
        """
        Format full media context block for inclusion in the entity's prompt.
        ...
        re_state = current_context.get('user_emotional_context', 'unknown')

        lines.append(f"Current conversation topic: {topic}")
        if entities:
            lines.append(f"Active entities: {', '.join(entities)}")
        lines.append(f"User's apparent state: {re_state}")
        lines.append("")
...
if __name__ == "__main__":
    ...
    builder.add_message("entity", "[cat]'s been on your mind? Is he doing his door-dashing thing again?", 1)
    ...
    print(f"User's state: {context['user_emotional_context']}")
    print()
