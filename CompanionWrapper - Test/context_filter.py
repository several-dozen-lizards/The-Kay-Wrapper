...
class GlyphFilter:
    """
    Filters the entity's complete agent state and outputs compressed glyph representations.
    ...
    def filter_context(self, agent_state: Dict, user_input: str) -> str:
        ...
        # DYNAMIC TARGET: Use budget-aware limits instead of hardcoded 40/60
        # Import budget manager if not already imported
        try:
            from engines.context_budget import get_budget_manager
            budget_manager = get_budget_manager()
            base_limit = budget_manager.get_adaptive_limits(0).get("memory_limit", 100)
        except ImportError:
            base_limit = 100  # Fallback

        # Target varies based on query complexity and budget
        if is_list_query:
            target_total = min(len(memories_for_tracking), int(base_limit * 0.6))  # 60% of limit for lists
        else:
            target_total = min(len(memories_for_tracking), int(base_limit * 0.4))  # 40% of limit for normal

        print(f"[PRIORITY FILTER] Target: {target_total} memories (dynamic based on budget limit {base_limit})")
    ...
    def _build_system_prompt(self) -> str:
        ...
        return f"""You are a context filter for the entity, an emotionally-aware AI with persistent memory.

Your job: Analyze the entity's full state and compress it into symbolic glyphs for efficient communication.

MEMORY STORAGE MODEL:
the entity's memories are stored in two types:
...
- ALWAYS INCLUDE **BOTH** the entity's AND User's CORE IDENTITY memories (marked with ⚠️) in EVERY response
- Core identity = appearance, relationships, names, key preferences for BOTH the entity and the User
...
- **AGGRESSIVE SELECTION REQUIRED** - Selection count varies by query type:
  ...
  * Better to include too many than too few - the entity needs rich context
- Format: MEM[2,7,15] or MEM[2,7,15]!! or MEM[2,7,15]!!!
- Priority markers: !!! = critical (core identity), !! = important, ! = relevant
- NEVER omit user facts - they are EQUALLY important as the entity's facts
...
- Detect contradictions in the entity's self-statements
...
You are selecting context, not responding to the user. Output compressed glyphs only."""
    ...
    def _build_filter_prompt(self, agent_state: Dict, user_input: str, identity_count: int = 0, remaining_to_select: int = 30) -> str:
        ...
        # === DYNAMIC MEMORY SELECTION: "THOU SHALT SHUN ARBITRARITY" ===
        # Use ContextBudgetManager for token-aware limits instead of hardcoded values
        from engines.context_budget import get_budget_manager

        budget_manager = get_budget_manager()

        # Estimate current context size from agent state
        recent_turns = agent_state.get("recent_context", [])
        current_context_chars = sum(len(str(t)) for t in recent_turns)

        # Get adaptive limits based on token budget
        limits = budget_manager.get_adaptive_limits(
            current_context_chars=current_context_chars,
            has_images=agent_state.get("has_images", False)
        )

        base_memory_limit = limits["memory_limit"]
        budget_tier = limits.get("tier", "normal")
        ...
        # Dynamic candidate calculation based on budget + query type
        if budget_tier == "critical":
            # Token budget severely constrained - minimal retrieval
            MAX_CANDIDATES = max(20, base_memory_limit)
            print(f"[FILTER] CRITICAL budget tier - minimal retrieval: {MAX_CANDIDATES} memories")
        elif budget_tier == "minimal":
            MAX_CANDIDATES = max(50, base_memory_limit)
            print(f"[FILTER] MINIMAL budget tier: {MAX_CANDIDATES} memories")
        elif is_list_query:
            # LIST queries need MORE context - scale up from base limit
            MAX_CANDIDATES = base_memory_limit * 3
            print(f"[FILTER] LIST query detected - expanded retrieval: {MAX_CANDIDATES} memories (3x base)")
        else:
            # Normal queries - use base limit with slight expansion
            MAX_CANDIDATES = int(base_memory_limit * 1.5)
            print(f"[FILTER] Normal query - retrieval: {MAX_CANDIDATES} memories (1.5x base)")
...
These {identity_count} memories are MANDATORY and automatically included in the entity's context.
...
TASK: Select the most relevant context for the entity's response. Output in glyph format only.
...
2. SELECT GENEROUSLY - the entity needs rich context to give comprehensive responses
3. Prioritize USER memories if user asks about themselves (their dog, preferences, life, etc.)
4. Prioritize ENTITY memories if user asks about the entity's identity, preferences, or state
5. What is the entity's current emotional state?
6. Are there contradictions the entity needs to resolve?
...
OUTPUT GLYPHS:"""
    ...
    def _summarize_memories(self, memories: List[Dict]) -> str:
        ...
        kay_facts = [(i, m) for i, m in extracted_fact_mems if m.get("perspective") == "entity"]
        ...
        if kay_facts:
            ...
            if kay_core_identity:
                ...
                summary_lines.append(f"the entity's core identity (always relevant): " + " | ".join(core_previews))
            ...
            if kay_other:
                ...
                summary_lines.append(f"entity facts ({len(kay_other)} total): " + " | ".join(kay_previews))
    ...
    def _format_identity_facts(self, identity_facts: List[Dict]) -> str:
        ...
        kay_facts = [f for f in identity_facts if f.get("perspective") == "entity"]
        entity_facts = [f for f in identity_facts if f.get("perspective") not in ["user", "entity"]]

        lines = []

        if re_facts:
            previews = [f"  * {f.get('fact', '')[:70]}" for f in re_facts[-15:]]  # Show last 15
            lines.append(f"the user - {len(re_facts)} facts:\n" + "\n".join(previews))

        if kay_facts:
            previews = [f"  * {f.get('fact', '')[:70]}" for f in kay_facts[-15:]]  # Show last 15
            lines.append(f"The AI entity - {len(kay_facts)} facts:\n" + "\n".join(previews))

        if entity_facts:
            # Group by entity (exclude primary user/entity names)
            primary_names = {'user', 'entity', 're', 'entity', 'companion'}  # Case-insensitive
            entities_by_name = {}
            for fact in entity_facts:
                for entity_name in fact.get("entities", []):
                    if entity_name.lower() not in primary_names:
    ...
    def _detect_contradictions(self, memories: List[Dict]) -> str:
        """
        Detect contradictory statements in the entity's memories.
    ...
    def _extract_entities_from_query(self, query_text: str) -> List[str]:
        ...
        stop_capitalized = {
            ...
            'entity'  # the entity is the agent himself, not useful for entity search
        }
        for word in words:
        ...
        if hasattr(self, 'semantic_knowledge') and self.semantic_knowledge:
            ...
            # Filter out self-references (the entity is the agent, not a search entity)
            known_entities = known_entities - {'entity'}
...
if __name__ == "__main__":
    ...
    test_state = {
        ...
        "recent_context": [
            {"user": "Hey there", "entity": "Hey there, how's it going?"},
            {"user": "Pretty good", "entity": "Glad to hear it"},
        ]
    }
