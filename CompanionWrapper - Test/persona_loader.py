...
# If COMPANION_PERSONA_DIR environment variable is set, use that directory
# instead of the default ./persona/ directory. This allows companion_manager.py
# to run multiple isolated instances with different personas.

# Check for managed instance (set by companion_manager.py)
_managed_persona_dir = os.environ.get("COMPANION_PERSONA_DIR")

if _managed_persona_dir and os.path.exists(_managed_persona_dir):
    _persona_dir = _managed_persona_dir
else:
...
try:
    ...
    if _managed_persona_dir:
        print(f"[PERSONA] Source: {_persona_dir} (managed)")
