...
# =============================================================================
# COMPANION MANAGER ENVIRONMENT VARIABLES
# =============================================================================
# These environment variables allow multiple companion instances to run
# simultaneously with isolated memory and unique ports.
#
# Set by companion_manager.py when starting an instance:
#   COMPANION_NAME       - Persona name (e.g., "aurora")
#   COMPANION_PORT       - WebSocket port for private room (e.g., 8780)
#   COMPANION_PERSONA_DIR - Path to persona config files
#   COMPANION_MEMORY_DIR  - Path to isolated memory directory
#   COMPANION_SESSION_DIR - Path to isolated session logs
#   COMPANION_INSTANCE_DIR - Path to instance data root
# =============================================================================

COMPANION_NAME = os.environ.get("COMPANION_NAME")
COMPANION_PORT = int(os.environ.get("COMPANION_PORT", "8780"))
COMPANION_PERSONA_DIR = os.environ.get("COMPANION_PERSONA_DIR")
COMPANION_MEMORY_DIR = os.environ.get("COMPANION_MEMORY_DIR", "memory")
COMPANION_SESSION_DIR = os.environ.get("COMPANION_SESSION_DIR", "sessions")
COMPANION_INSTANCE_DIR = os.environ.get("COMPANION_INSTANCE_DIR")

# If COMPANION_PERSONA_DIR is set, we're running in managed mode
# Update the persona_loader to use the correct directory
if COMPANION_PERSONA_DIR:
    os.environ["COMPANION_PERSONA_DIR"] = COMPANION_PERSONA_DIR
    print(f"[MANAGER] Running as managed instance: {COMPANION_NAME}")
    print(f"[MANAGER] Persona dir: {COMPANION_PERSONA_DIR}")
    print(f"[MANAGER] Memory dir: {COMPANION_MEMORY_DIR}")
    print(f"[MANAGER] Port: {COMPANION_PORT}")

# Ensure memory directory exists
os.makedirs(COMPANION_MEMORY_DIR, exist_ok=True)
os.makedirs(COMPANION_SESSION_DIR, exist_ok=True)

def memory_path(subpath: str) -> str:
    """Get the full path for a memory file, respecting COMPANION_MEMORY_DIR."""
    return os.path.join(COMPANION_MEMORY_DIR, subpath)

# Project branding
from branding import PROJECT_NAME, PROJECT_TAGLINE, LOG_TAG
...
print(f"{LOG_TAG} Loading {persona.name} ({persona.entity_id})")
...
async def main(ui_message_queue=None, ui_response_queue=None, private_room=None):
    ...
    try:
        vector_store = VectorStore(persist_directory=memory_path("vector_db"))
    ...
    try:
        snapshot_path = memory_path("state_snapshot.json")
    ...
    forest = MemoryForest.load_from_file(memory_path("forest.json"))
    state.forest = forest
    ...
    if RESONANCE_AVAILABLE:
        try:
            resonance = ResonantIntegration(
                state_dir=memory_path("resonant"),
    ...
    if ROOM_AVAILABLE and room:
        try:
            autonomous_spatial = AutonomousSpatialEngine(
                ...
                room_engine=room,
                persist_path=memory_path("autonomous_spatial_state.json")
            )
    ...
    try:
        media_orchestrator = MediaOrchestrator(
            ...
            media_storage_path=memory_path("media")
        )
    ...
    while True:
        ...
        if media_orchestrator:
            ...
            media_orchestrator.update_conversation_context(
                ...
                user_state=conv_context.get("user_emotional_context")
            )
...
{chunk['text']}"""
        ...
        # ================================================================
        # the entity's response may reference topics not in the user's input.
        ...
        #
        # Example: User says "How you feeling?" -> retrieval finds nothing about
        ...
        try:
            ...
            if len(reply) > 100:
                ...
                # New topics = words the entity used that the user didn't
        ...
        if doc_reader.current_doc:
            ...
            if "continue reading" in response_lower or "next section" in response_lower or "let's move on" in response_lower:
                if doc_reader.advance():
                    print(f"[NAV] Entity requested advance -> section {doc_reader.current_position + 1}/{doc_reader.total_chunks}")
                    navigation_triggered = True
                else:
                    print(f"[NAV] Entity requested advance but already at end of document")
                ...
                if doc_reader.previous():
                    print(f"[NAV] Entity requested previous -> section {doc_reader.current_position + 1}/{doc_reader.total_chunks}")
                    navigation_triggered = True
                else:
                    print(f"[NAV] Entity requested previous but already at beginning")
                ...
                print(f"[NAV] Entity requested restart -> section 1/{doc_reader.total_chunks}")
                navigation_triggered = True
                ...
                if match:
                    ...
                    if doc_reader.jump_to(target):
                        print(f"[NAV] Entity requested jump -> section {target + 1}/{doc_reader.total_chunks}")
                        navigation_triggered = True
        ...
        if new_document_loaded and doc_reader.current_doc and doc_reader.total_chunks > 1:
            ...
            try:
                result = auto_reader.read_document_sync(
                    ...
                    agent_state=state,
                    start_segment=2  # Start from segment 2 (entity already read segment 1)
                )
        ...
        try:
            ...
            with open(memory_path("state_snapshot.json"), "w", encoding="utf-8") as f:
        ...
        # Save forest to file
        try:
            forest.save_to_file(memory_path("forest.json"))
        except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Companion Wrapper")
    parser.add_argument("--ui", action="store_true", help="Enable UI mode with WebSocket server")
    parser.add_argument("--room-port", type=int, default=COMPANION_PORT,
                        help=f"WebSocket port for private room (default: {COMPANION_PORT})")
    args = parser.parse_args()

    if args.ui:
        # UI mode: start with private room server
        asyncio.run(main_with_ui(args.room_port))
    else:
        # Terminal mode: standard conversation loop
        asyncio.run(main())
