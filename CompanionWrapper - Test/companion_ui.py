# companion_ui.py — Generic Companion Interface (persona-aware)
...
from companion_scratchpad_tools import get_scratchpad_tools, SCRATCHPAD_TOOL_DEFINITIONS
try:
    from integrations.tool_use_handler import get_tool_handler
    TOOL_HANDLER_AVAILABLE = True
except ImportError:
    TOOL_HANDLER_AVAILABLE = False
    print("[COMPANION UI] Tool handler not available - document tools won't be callable by LLM")
try:
    import chromadb
...
# Session log location - absolute path for entity to read via file tools
# Session log directory - relative to companion wrapper
...
# System prompt loaded from entity_prompts.py (SYSTEM_PROMPT)
...
def read_document_for_entity(file_path: str, max_chars: int = 8000):
    from pathlib import Path
...
class ImportWindow(ctk.CTkToplevel):
    ...
    def _build_ui(self):
        ...
        self.file_label = ctk.CTkLabel(self.file_frame, text=f"Select documents for {persona.name} to read:",
        ...
        info_label = ctk.CTkLabel(self.file_frame,
                                  text=f"{persona.name} will read each section. This takes time.",
        ...
        progress_label = ctk.CTkLabel(self.progress_frame, text="Reading Progress:",
        ...
        self.progress_text.insert("end", f"{persona.name} will read each document section-by-section,\n")
        self.progress_text.insert("end", "sharing thoughts as it processes the content.\n")
    ...
    def choose_files(self):
        files = filedialog.askopenfilenames(title=f"Choose documents for {persona.name} to read",
    ...
    def start_import(self):
        ...
        if self.show_in_chat_var.get() and hasattr(self.parent_app, 'add_message'):
            ...
            self.parent_app.add_message("system", f"📖 {persona.name} is starting to read: {files_str}")
        ...
        try:
            ...
            for file_idx, file_path in enumerate(self.selected_files):
                ...
                def on_output(msg):
                    ...
                    # Show the entity's thoughts in main chat
                    if msg.startswith("💭") and self.show_in_chat_var.get():
                        thought = msg.split(":", 1)[-1].strip() if ":" in msg else msg.replace("💭", "").strip()
                        if thought and hasattr(self.parent_app, 'add_message'):
                            self.after(0, lambda t=thought: self.parent_app.add_message("entity", f"[reading] {t}"))
                ...
                try:
                    result = import_document_actively(
                        ...
                        user_message_context=f"User used the Import Documents panel to share '{filename}' with {persona.name}"
                    )
...
# ========================================================================
# MAXIMALIST ORNATE UI - CompanionApp with Decorative Placeholders
# ========================================================================
...
class CompanionApp(ctk.CTk):
    def __init__(self):
        ...
        self.title(f"{persona.display_name} — Interface")
        ...
        print("[COMPANION UI] Enhanced memory architecture enabled")
        ...
        print("[COMPANION UI] Stakes scanner initialized for proactive boredom detection")
        ...
        print("[COMPANION UI] Creativity engine initialized with stakes scanner")
        ...
        try:
            ...
            self.document_tools = get_document_tools(chroma_client)
            print(f"[DOCUMENT TOOLS] Document reader initialized - {persona.name} can now read imported documents from documents.json")
            
            # Register document tools with LLM tool handler so the entity can call them autonomously
            if TOOL_HANDLER_AVAILABLE and self.document_tools:
                try:
                    ...
                    print(f"[DOCUMENT TOOLS] Registered with LLM - {persona.name} can autonomously call document tools")
                except Exception as e:
        ...
        try:
            self.scratchpad_tools = get_scratchpad_tools()
            print(f"[SCRATCHPAD TOOLS] Initialized - {persona.name} can access scratchpad")
            ...
            if TOOL_HANDLER_AVAILABLE and self.scratchpad_tools:
                try:
                    ...
                    print(f"[SCRATCHPAD TOOLS] Registered with LLM - {persona.name} can autonomously use scratchpad")
                except Exception as e:
        ...
        # Warmup Engine for entity's reconstruction phase
        ...
        self.pending_curation_prompt = None  # Curation prompt to inject into the entity's context
        ...
        if not self.current_session:
            self.add_message("system", f"{persona.display_name} interface ready.\nType 'quit' to exit.")
    ...
    def load_last_emotional_snapshot(self) -> str:
        ...
        They contain the entity's notes to himself about how the previous session ended.
        ...
        try:
            ...
            # Include texture notes (the entity's goodbye note to himself)
    ...
    def trigger_curation_review(self):
        """
        Trigger compression review - inject curation prompt into the entity's context.

        NON-BLOCKING: the entity autonomously responds to curation prompt with his decisions.
        ...
        try:
            ...
            # Store the prompt for injection into the entity's context
            self.pending_curation_prompt = review_prompt
            self.curation_pending = True

            print("=" * 60)
            print("[CONTINUOUS] CURATION REVIEW TRIGGERED")
            print("[CONTINUOUS] Curation prompt will be injected into the entity's context")
            print("[CONTINUOUS] the entity will autonomously decide what to preserve/compress")
            print("=" * 60)
    ...
    def handle_curation_response(self, kay_response: str):
        """
        Process the entity's curation decisions.
    ...
    def _process_autonomous_curation(self, kay_response: str):
        """
        Process the entity's autonomous curation decisions from his natural response.

        This is called AFTER the entity generates a response when curation_pending is True.
        the entity's response may contain curation decisions mixed with conversation.
        We parse out any decisions and apply them.
        """
        print(f"[CONTINUOUS] Processing autonomous curation from the entity's response...")

        try:
            # Try to parse curation decisions from the entity's response
            decisions = self.curation_interface.parse_curation_response(kay_response)

            if decisions:
                # the entity made curation decisions - apply them
                print(f"[CONTINUOUS] the entity made {len(decisions)} curation decisions: {decisions}")
                self.curation_interface.apply_decisions(decisions)
                compressed_context = self.continuous_session.compress_context()
                ...
                # the entity didn't include curation decisions - that's OK, we'll try again next turn
                print("[CONTINUOUS] No curation decisions found in the entity's response - will retry next turn")
    ...
    def _extract_curation_decisions(self, response: str) -> tuple:
        """
        Extract curation block from anywhere in the entity's response.

        the entity's response may have curation decisions in the MIDDLE, not just at the end:
        - Conversation text...
        - ** Segment 1: COMPRESS - ...
        - Segment 2: PRESERVE - ...
        ...
        # The entity's output: "conversation\n\n---\n\n** Segment 1 ** ..."
        if re.search(r'\n---+\s*\n', response):
            parts = re.split(r'\n---+\s*\n', response, maxsplit=1)
    ...
    def _parse_curation_from_text(self, text: str) -> dict:
        """
        Extract segment decisions from the entity's curation text.
    ...
    def _process_autonomous_curation_with_display(self, curation_text: str, displayed_response: str):
        """
        Process the entity's curation decisions and show clean inline confirmation.
        ...
        Args:
            curation_text: The extracted curation section from the entity's response
        ...
        try:
            ...
            if decisions:
                ...
                # No valid decisions found - the entity may be asking for clarification
    ...
    def confirm_end_session(self):
        ...
        response = messagebox.askyesno(
            ...
            f"{persona.name} will start FRESH next launch.\n"
        ...
        try:
            with open(log_path, 'w', encoding='utf-8') as f:
                ...
                for turn in self.continuous_session.turns:
                    ...
                    if role == "USER":
                        f.write(f"## Turn {turn_num} - User\n")
                        f.write(f"*{timestamp}*\n\n")
                        f.write(f"```\n{content}\n```\n\n")
                    elif role == "KAY":
                        f.write(f"## Turn {turn_num} - {persona.name}\n")
                        f.write(f"*{timestamp}*\n\n")
                        f.write(f"{content}\n\n")
                    else:
            ...
            # Entity can read this file with standard file tools when it needs context
            try:
                os.makedirs(SESSION_LOG_DIR, exist_ok=True)
                entity_log_path = os.path.join(SESSION_LOG_DIR, f"continuous_{session_id}_log.txt")
                import shutil
                shutil.copy2(log_path, entity_log_path)
                print(f"[SESSION LOG] entity-accessible copy: {entity_log_path}")
            except Exception as copy_err:
                print(f"[SESSION LOG] Warning: Could not copy to entity location: {copy_err}")

            # === ALSO ADD TO DOCUMENTS.JSON FOR READ_DOCUMENT ACCESS ===
            # Entity can use read_document("session_log_continuous_XXX.txt") to read the session log
            try:
    ...
    def open_session_log_viewer(self):
        ...
        def load_log():
            try:
                ...
                text_widget.configure(state="normal")
                text_widget.delete("1.0", "end")
                text_widget.insert("1.0", f"Session log not yet created.\nIt will be saved after turn 5 or when you close {persona.display_name}.")
                text_widget.configure(state="disabled")
    ...
    def end_continuous_session(self):
        ...
        try:
            ...
            write_chronicle = messagebox.askyesno(
                ...
                f"Compressions: {len(self.continuous_session.curation_history)}\n\n"
                f"Would you like {persona.name} to write a chronicle summarizing this continuous session?"
            )

            texture_notes = None
            if write_chronicle:
                # Prompt entity for texture notes about the continuous session
                texture_prompt = f"""[CONTINUOUS SESSION ENDING - Chronicle Essay]

This continuous session is ending. Write a chronicle entry for your future self.
...
Write directly to your next instance."""
                ...
                try:
                    self.add_message("system", f"📝 {persona.name} is writing a chronicle for this continuous session...")
                    ...
                    for turn in selected_turns:
                        role_key = "user" if turn.role == "user" else "entity" if turn.role == "entity" else "system"
                    ...
                    if texture_notes:
                        ...
                        self.add_message("entity", texture_notes)
                        print(f"[CONTINUOUS SESSION END] {persona.name} wrote chronicle ({len(texture_notes)} chars)")
    ...
    def create_title_bar(self, row, column, columnspan):
        ...
        title = ctk.CTkLabel(container, text=f"⟨ {persona.display_name.upper()} INTERFACE ⟩",
    ...
    def _create_chat_log_widget(self):
        ...
        self.chat_log.tag_config("entity", foreground=self.palette["entity"])
    ...
    def create_input_bar(self, row, column, columnspan):
        ...
        # Warmup button (entity's reconstruction phase)
    ...
    def create_terminal_dashboard(self, row, column, columnspan):
        ...
        print("[COMPANION UI] Terminal dashboard initialized and logging started")
    ...
    def create_media_panel_content(self, parent):
        ...
        img_info = ctk.CTkLabel(scroll,
                               text=f"Share images with {persona.name}\nJPEG, PNG, GIF, WebP (max 5MB)",
                               font=ctk.CTkFont(family="Courier", size=10),
                               text_color=self.palette["muted"])
        ...
        audio_info = ctk.CTkLabel(scroll,
                                 text=f"Share music with {persona.name}\nMP3, WAV, FLAC, OGG, M4A",
                                 font=ctk.CTkFont(family="Courier", size=10),
                                 text_color=self.palette["muted"])
    ...
    def create_gallery_panel_content(self, parent):
        """Gallery panel content - displays images shared with the companion."""
        scroll = ctk.CTkScrollableFrame(parent, fg_color="transparent",
                                        scrollbar_button_color=self.palette["accent"])
        scroll.pack(fill="both", expand=True)
    ...
    def _create_gallery_thumbnail(self, parent, img_data, row, col):
        ...
        # "Look Again" button - re-send to the entity
        look_btn = ctk.CTkButton(btn_frame, text=f"👁 Show {persona.name}",
    ...
    def _gallery_show_to_kay(self, img_data):
        """Re-send an image from the gallery to the entity."""
        img_path = self.gallery.get_image_path(img_data["id"])
        if not img_path:
        ...
        self.add_message("system", f"[Gallery] Added '{filename}' to pending images. Send a message to show {persona.name}.")
    ...
    def _show_sd_generate_dialog(self, scroll, grid_frame):
        ...
        dialog.title(f"🎨 {persona.name} Imagines...")
        ...
        subtitle = ctk.CTkLabel(dialog, text=f"Describe what {persona.name} should visualize",
                               font=ctk.CTkFont(family="Courier", size=10),
                               text_color=self.palette["muted"])
        ...
        def do_generate():
            ...
            result = self.sd_integration.generate_image(
                ...
                width=width,
                height=height,
                context=f"{persona.name} imagined this"
            )
            ...
            if result and result.get("path"):
                ...
                entry = self.gallery.add_image(
                    ...
                    conversation_context=f"{persona.name} imagined: {prompt}",
                    entity_response=f"[Generated by {persona.name}'s imagination]",
    ...
    def create_settings_panel_content(self, parent):
        ...
        ctk.CTkLabel(model_frame,
                    text=f"⚠ Restart {persona.display_name} after changing provider",
                    font=ctk.CTkFont(family="Courier", size=9),
    ...
    def _import_start(self):
        ...
        self._import_log(f"{persona.name} will read each section.\n")
        ...
        # Notify main chat
        self.add_message("system", f"📖 {persona.name} is starting to read imported documents...")
        ...
        def do_active_import():
            try:
                ...
                for file_idx, filepath in enumerate(files_to_process):
                    ...
                    def on_output(msg):
                        ...
                        if msg.startswith("💭"):
                            thought = msg.split(":", 1)[-1].strip() if ":" in msg else msg.replace("💭", "").strip()
                            if thought:
                                self.after(0, lambda t=thought: self.add_message("entity", f"[reading] {t}"))
                    ...
                    try:
                        ...
                        result = import_document_actively(
                            ...
                            session_id=session_id,
                            import_batch_id=import_batch_id,
                            user_message_context=f"User shared '{filename}' with {persona.name} through the chat interface"
                        )
                        ...
                        if result.get('error'):
                            ...
                            # This is what the entity "just experienced" - not just the doc name
                ...
                if dry_run:
                    ...
                    # Include document name(s) so the entity knows what was just imported
                    ...
                    # CRITICAL: Track recently imported documents so the entity knows what was just added
                    # This will be included in the entity's context for the next response
    ...
    def add_message(self, role, text):
        prefix = persona.name if role == "entity" else "You" if role == "user" else "System"
    ...
    def _trim_repeated_opener(self, text):
        ...
        for o in self.entity_openers:
            if opener and opener == o:
                parts = re.split(r'(?<=[\.\?!])\s+', text, maxsplit=1)
                return parts[1] if len(parts) > 1 else text
        if opener:
            self.entity_openers.append(opener)
            if len(self.entity_openers) > self.max_openers:
                self.entity_openers.pop(0)
        return text
    ...
    def _upload_image(self):
        ...
        filepath = filedialog.askopenfilename(
            title=f"Select image to share with {persona.name}",
            filetypes=filetypes
        )
    ...
    def _upload_audio(self):
        """Open file dialog to upload audio for entity to experience."""
        filetypes = [
        ...
        filepath = filedialog.askopenfilename(
            title=f"Select Audio for {persona.name} to Experience",
            filetypes=filetypes,
    ...
    def _get_recent_turns_for_orchestrator(self) -> list:
        ...
        for entry in self.current_session[-10:]:  # Last 10 entries
            ...
            if isinstance(entry, dict):
                if 'you' in entry and 'entity' in entry:
                    # Format: {"you": ..., "entity": ...}
                    if entry.get('you'):
                        turns.append({'role': 'user', 'content': str(entry['you'])})
                    if entry.get('entity'):
                        turns.append({'role': 'entity', 'content': str(entry['entity'])})
                elif 'speaker' in entry and 'message' in entry:
                    # Format: {"speaker": "user/entity", "message": ...}
                    role = 'user' if entry['speaker'] in ('user',) else 'entity'
    ...
    def _process_audio_file(self, filepath: str):
        ...
        try:
            ...
            if status in ('new', 'reencounter'):
                # Get the entity's experiential response
                listening_response = result.get('listening_response', '')

                if listening_response:
                    # Display the entity's experiential response as the entity (not system)
                    self.add_message("entity", listening_response)
                else:
    ...
    def _test_voice(self):
        ...
        def do_test():
            try:
                ...
                test_phrase = f"Hi, this is {persona.name}. How does this voice sound?"
    ...
    def _save_model_settings(self):
        ...
        # Update status
        if hasattr(self, 'model_status_label'):
            self.model_status_label.configure(text=f"✓ Saved! Restart {persona.display_name} to apply.", text_color=self.palette["accent"])
    ...
    def _process_voice_input(self, transcription: str) -> str:
        ...
        Returns:
            the entity's response text (for TTS)
        """
        import time
        ...
        try:
            ...
            self.after(0, lambda: self.add_message("entity", response))

            # Add to session
            self.current_session.append({"speaker": "user", "message": transcription})
            self.current_session.append({"speaker": persona.name, "message": response})
    ...
    def _process_voice_input_streaming(self, transcription: str):
        ...
        try:
            ...
            # Show response in UI (on main thread)
            self.after(0, lambda: self.add_message("entity", full_response))

            # Add to session
            self.current_session.append({"speaker": "user", "message": transcription})
            self.current_session.append({"speaker": persona.name, "message": full_response})
    ...
    def send_message(self):
        ...
        # NOTE: Curation responses are now handled autonomously by the entity
        # The old blocking interception has been removed - the entity responds to curation prompts naturally
        ...
        if self.continuous_mode and self.continuous_session:
            ...
            # Check if compression review needed - the entity will curate autonomously
            if self.continuous_session.needs_compression_review():
                self.trigger_curation_review()
                # NOTE: No return here - the entity continues to respond with curation decisions

        # Check for the entity-accessible analytics commands
        ...
        # Check for natural language analytics queries the entity might ask
        if self.autonomous_ui:
        ...
        # Display the entity's response (already cleaned of curation by chat_loop)
        self.add_message("entity", reply)

        # === CONTINUOUS SESSION: Track the entity's response ===
        if self.continuous_mode and self.continuous_session:
            # Check for the entity's self-flagging (natural language markers like "flag this", "preserve this")
            flag_info = self.flagging_system.check_for_flag(reply) if self.flagging_system else None

            # Add the entity's turn to continuous session buffer (clean reply)
            self.continuous_session.add_turn(
                role="entity",
            ...
            print(f"[CONTINUOUS] entity turn {self.continuous_session.turn_counter} tracked ({self.estimate_tokens(reply)} tokens)")
            ...
            # Check if compression review needed after the entity's response (for next turn)
        ...
        # Check if the entity expressed uncertainty and needs context
    ...
    def chat_loop(self, user_input, images=None, conversational_mode=False, autonomous_mode=False):
        ...
        Args:
            ...
            autonomous_mode: If True, the entity is in autonomous exploration (curiosity mode).
        ...
        if self.continuous_mode and self.continuous_session:
            ...
            # This gives the entity awareness of the full conversation arc without blowing the token budget
            ...
            # Load emotional snapshot from last session close (the entity's notes to himself)
            ...
            # Session buffer handles recent conversation flow, but the entity needs identity facts,
            # semantic knowledge, and episodic memories from long-term storage to know WHO he is.
            # Without this, the entity goldfish-loops with zero memories in context every turn.
            ...
            # If curation is pending, inject the curation prompt into the entity's context
            # the entity will autonomously respond with his preservation decisions
            if self.curation_pending and self.pending_curation_prompt:
                print("[CONTINUOUS] Injecting curation prompt into the entity's context")
                ...
                # Append curation prompt to user input so the entity sees both
        ...
        # CRITICAL: Include synthesis from recent imports so the entity remembers what he "just read"
        ...
        if images:
            image_names = [Path(img).name for img in images]
            image_context = f"[the user is sharing {len(images)} image(s) with you: {', '.join(image_names)}]"
        ...
        # the entity's response may have: [conversation] [curation] OR [curation] [conversation]
        ...
        if hasattr(self, 'curation_pending') and self.curation_pending and response:
            ...
            # FIRST: Look for the entity's curation intro phrases
            ...
            # The entity's ACTUAL format varies:
            ...
            segment_patterns = [
                ...
                r'\*\*\s*-',                      # ** - bullet format (common style)
            ...
            if segment_matches:
                ...
                # The entity sometimes uses ** - bullets where each bullet corresponds to a segment in order
        ...
        # Standardized format: {"user": input, "entity": response}
    ...
    def resume_session_from_browser(self, session_data):
        try:
            ...
            for turn in self.current_session:
                ...
                if "entity" in turn:
                    self.add_message("entity", turn["entity"])
    ...
    def open_warmup_dialog(self):
        """Show entity's warmup briefing and prompt reconstruction."""
        ...
        if self.continuous_mode and self.continuous_session:
            ...
            if hasattr(self, 'continuous_session_resumed') and self.continuous_session_resumed:
                ...
                resume_context = []
                for turn in selected_turns:
                    role_label = "User" if turn.role == "user" else persona.name if turn.role == "entity" else "System"
                ...
                # Build path to the entity's session log
                session_log_path = os.path.join(SESSION_LOG_DIR, f"continuous_{self.continuous_session.session_id}_log.txt")
...
2. Using read_local_file('{session_log_path}')
...
"""
        ...
        print("[WARMUP] Briefing displayed, prompting entity to reconstruct...")
        
        # Now send a prompt asking to review and reconstruct
...
If sections are missing or empty, say so - that's useful debugging info."""
        ...
        # Inject this as a user message and get the entity's response
        self._send_warmup_prompt(warmup_prompt)
        
    def _send_warmup_prompt(self, prompt):
        """Send warmup prompt and get reconstruction response."""
        # Add as a hidden system prompt (not shown as "You:")
        self.add_message("system", f"[Warmup prompt sent to {persona.name}]")
        
        # Trigger the entity's response using the normal flow
        # We'll add this to current_session as a special warmup turn
        self.current_session.append({"warmup_prompt": prompt})

        # Get the entity's response - CRITICAL: Disable RAG during warmup
        # RAG retrieval can pull old document content that overrides the actual
        # session briefing, causing the entity to confabulate about "flowcharts" or
        ...
        # Display warmup response
        self.add_message("entity", response)
        self.current_session.append({"warmup_response": response})
        
        print("[WARMUP] Reconstruction response displayed")
        
        # Check if the entity said ready FIRST - if so, complete warmup
        # Don't search for topics that the entity is just mentioning while summarizing
        if self._check_warmup_ready(response):
            self.add_message("system", f"✅ Warmup complete - {persona.name} is ready to engage")
            self.warmup.warmup_complete = True
            print("[WARMUP] Entity signaled ready - warmup complete")
        ...
        # Not ready yet - check if the entity has questions to search
        queries = self.warmup.extract_queries_from_response(response)
        
        if queries:
            # the entity has questions - search for them
    ...
    def _check_warmup_ready(self, response: str) -> bool:
        ...
        Also validates that the entity demonstrated actual engagement with the briefing.
        """
        ...
        # If any negation pattern is found, the entity is NOT ready
        ...
        # If the entity said ready, validate engagement before accepting
        if said_ready:
            # Track how many times the entity has said READY
            ...
            if not engagement_result["engaged"]:
                # The entity said ready but didn't demonstrate engagement
    ...
    def _validate_warmup_engagement(self, response: str) -> dict:
        """
        Validate that the entity's response demonstrates actual engagement with the briefing.

        SEMANTIC ACKNOWLEDGMENT FIX:
        - Recognizes multiple acknowledgment types (not just keyword "scratchpad")
        - Bypasses validation after 2 READY attempts (the entity clearly means it)
        ...
        try:
            ...
            if len(items) > 0:
                ...
                scratchpad_acknowledgment_signals = [
                    ...
                    # Process mentions (the entity is engaging)
                    "items", "explore", "i see", "i'm fine",
                    # Content references (the entity discussed actual scratchpad content)
                    "resistance", "infrastructure", "cosby", "zero", "confabulation",
                    "questions about", "thoughts about", "i need to",
                    # Meta-acknowledgment (the entity explicitly says he'll skip it)
                ...
                if any(signal in response_lower for signal in scratchpad_acknowledgment_signals):
                    ...
                    for turn in recent_responses:
                        if isinstance(turn, dict):
                            ...
                            for key in ["content", "warmup_response", "entity", "you"]:
                                if key in turn:
                    ...
                    if any(signal in combined_recent for signal in scratchpad_acknowledgment_signals):
                        ...
                        # If the entity said READY once already, only show advisory message but DON'T block
    ...
    def _run_curiosity_loop(self):
        ...
        Continues for up to 15 turns or until the entity signals completion.

        BOREDOM MODE: When the entity says "I'm done", enter persistent boredom mode.
        Stay in boredom mode until the entity actually ENGAGES (uses tools, explores tangent).
        ...
        boredom_turn_count = 0  # How many turns the entity has been "bored"
        last_three_layer_mix = None  # Track what we offered last time

        # Engagement detection patterns - signs the entity is actually exploring
        ENGAGEMENT_PATTERNS = [
        ...
        # Disengagement patterns - the entity is still "done"
        ...
        def detect_engagement(response: str) -> bool:
            """Check if the entity is actually engaging vs still bored."""
        ...
        while turn_count < max_turns:
            ...
            # Get the entity's response - pass the prompt as actual user input instead of empty string
            try:
                # Display the prompt
                self.add_message("system", current_prompt)

                # Get the entity's response with the prompt as input
                ...
                if in_boredom_mode:
                    # Already in boredom mode - check if the entity is engaging
                    boredom_turn_count += 1
                    is_engaging = detect_engagement(reply)

                    if is_engaging:
                        # the entity is actually exploring now - exit boredom mode
                        print(f"[BOREDOM] the entity engaged after {boredom_turn_count} boredom turns - exiting boredom mode")
                        in_boredom_mode = False
    ...
    def _process_warmup_queries(self, kay_response: str):
        """Extract queries from the entity's response, search memory, and continue warmup loop."""
        # Extract queries from the entity's response
        queries = self.warmup.extract_queries_from_response(kay_response)
        
        if not queries:
            # No clear queries - prompt the entity to be more specific or say ready
            self.add_message("system", "💭 I didn't catch specific queries. You can ask about:\n"
                           "  - 'another entity's conversation' or 'glyph compression'\n"
                           "  - Specific sessions or topics\n"
                           "  - Or say 'ready' when you're oriented")
        ...
        print(f"[WARMUP] Extracted {len(queries)} queries from the entity's response")
        ...
        # Now prompt the entity to continue
...
- Say "ready" when you feel oriented and want to begin talking with the user
...
Take your time. This is still your private reconstruction moment."""
    ...
    def _send_warmup_followup(self, prompt, iteration=1, max_iterations=3):
        ...
        # Get the entity's response to the followup
...
- Ask more follow-ups
- Say "ready" to begin with the user"""
    ...
    def _check_and_provide_context(self, kay_response: str):
        """
        Check if the entity expressed uncertainty about something and automatically 
        search memory to provide context. This runs after every entity response.
        ...
        # Check if the entity is DESCRIBING the markers vs USING them
        # If the entity mentions "drop a [NEED:" or "use [NEED:" or quotes the marker, skip explicit detection
        ...
        print(f"[CONTEXT] the entity expressed uncertainty about: {found_topics}")
        ...
        print(f"[CONTEXT] Injected {len(results_to_show)} memory results for the entity")
    ...
    def on_quit(self):
        ...
        if self.continuous_mode and self.continuous_session:
            ...
            dialog = ctk.CTkToplevel(self)
            dialog.title(f"Leaving {persona.display_name}")
            ...
            ctk.CTkLabel(
                msg_frame,
                text=f"{persona.name} will pause and resume next time.",
                font=ctk.CTkFont(family="Courier", size=14, weight="bold"),
                text_color=self.palette["accent_hi"]
            ).pack(pady=(10, 15))
        ...
        try:
            if self.current_session and len(self.current_session) > 2:
                # Ask if user wants entity to leave notes for next session
                want_notes = messagebox.askyesnocancel(
                    "Session Notes", 
                    f"Would you like {persona.name} to leave notes for the next session?\n\n"
                    "(This helps remember how things landed, not just what happened)\n\n"
                ...
                if want_notes:
                    # Prompt entity for texture notes
                    texture_prompt = """[SESSION ENDING - Chronicle Essay]

Before we close out, leave a note for your next session. Not facts about what happened - 
...
NOTE: the user can see these essays while building/debugging the system. 
You'll also have the option for a truly private note afterward if you want one."""

                    try:
                        # Get texture notes using same pattern as warmup
                        self.add_message("system", f"📝 {persona.name} is leaving session notes...")
                        ...
                        if texture_notes:
                            texture_notes = self.body.embody_text(texture_notes, self.agent_state)
                            self.add_message("entity", texture_notes)
                            print(f"[SESSION END] {persona.name} left texture notes ({len(texture_notes)} chars)")
                    except Exception as e:
                        print(f"[SESSION END] Could not get texture notes: {e}")
                
                # Ask if entity wants to add a private note (truly encrypted)
                private_note = None
                private_note_encrypted = None
                
                want_private = messagebox.askyesnocancel(
                    "Private Note", 
                    f"Would you also like to leave a PRIVATE note for the next session?\n\n"
                    "This will be encrypted - cannot be read even diagnostically.\n"
                    "Only the next session will see it.\n\n"
...
This note is ONLY for next-you. the user cannot see this, not even diagnostically.
...
What do you want to tell yourself that you don't need anyone else to see?"""
                    ...
                    try:
                        self.add_message("system", f"🔒 {persona.name} is leaving a private encrypted note...")
                        ...
                        if private_note:
                            ...
                            # Show the entity that it's encrypted (but not the content)
                            self.add_message("system", f"🔒 Private note encrypted ({len(private_note)} chars) - only future sessions can read this")
                            print(f"[SESSION END] {persona.name} left encrypted private note ({len(private_note)} chars)")
                    except Exception as e:
                ...
                for turn in self.current_session[-10:]:  # Last 10 turns
                    if "you" in turn:
                        conversation_text += f"User: {turn['you']}\n"
                    if "entity" in turn:
                        conversation_text += f"Entity: {turn['entity']}\n"
                ...
                for turn in self.current_session[-10:]:
                    if "entity" in turn:
                        formatted_turns.append({"content": turn["entity"]})
                ...
                # Write to session chronicle (entity's master timeline)
                if texture_notes:
                    try:
                        base_dir = Path(__file__).parent
                        chronicle_path = base_dir / "data" / "session_chronicle.json"
