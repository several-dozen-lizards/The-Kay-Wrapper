"""
Purge Reserve UI Components for the entity
...
class PurgeReserveListView(ctk.CTkFrame):
    ...
    def _create_memory_item(self, purged):
        ...
        # Entity note if present
        if purged.entity_note:
            ctk.CTkLabel(
                item_frame,
                text=f"Entity note: \"{purged.entity_note}\"",
                font=ctk.CTkFont(family="Courier", size=8),
...
class MemoryDetailView(ctk.CTkToplevel):
    ...
    def _build_ui(self, purged):
        ...
        if purged.entity_note:
            meta_text += f"\nEntity note: {purged.entity_note}"
