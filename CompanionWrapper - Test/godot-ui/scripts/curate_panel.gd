...
func _build_ui() -> void:
	...
	_entity_select.add_item("Companion")
...
func _get_entity_name() -> String:
	## Returns the selected entity name from dropdown
	## For single-entity companion wrapper, always returns "Companion"
	if _entity_select.item_count > 0:
		return _entity_select.get_item_text(_entity_select.selected)
	return "Companion"
