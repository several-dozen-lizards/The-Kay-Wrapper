...
## Speaker colors - BBCode color tags (generic companion colors)
var COLORS = {
	"Re": "#4EC9B0",
	"Companion": "#9B8ED9",  # Generic companion purple
	"System": "#808080",
	"default": "#D4D4D4",
}

## Panel theme presets (single generic companion theme)
const THEMES = {
	"companion": {
		"bg": Color(0.06, 0.04, 0.10, 1.0),
		"header_bg": Color(0.08, 0.05, 0.14, 1.0),
		"header_text": Color(0.65, 0.58, 0.80),
		"border": Color(0.25, 0.18, 0.40, 1.0),
		"input_bg": Color(0.06, 0.04, 0.10, 1.0),
		"accent": Color(0.45, 0.35, 0.65),
	},
}
...
var _panel_id: String = "companion"
...
## Per-panel background images (single companion)
const BG_IMAGES = {
	"companion": "res://assets/bg_companion.png",
}

## Per-panel accent images (additive blend — black = invisible)
const ACCENT_IMAGES = {
	"companion": "res://assets/accent_companion.png",
}
...
func configure(panel_id: String, embedded: bool = false) -> void:
	_panel_id = panel_id
	_embedded = embedded
	_theme_data = THEMES.get(panel_id, THEMES["companion"])
	_setup_background()
	_apply_theme()
	_configure_controls()


func set_companion_name(name: String) -> void:
	"""Set the companion's name in the speaker colors."""
	if name != "Companion" and not COLORS.has(name):
		COLORS[name] = "#9B8ED9"
...
func _on_image_file_selected(path: String) -> void:
	...
	add_system_message("Image selected: %s (%d KB) - type message and press Send, or just press Send" % [
