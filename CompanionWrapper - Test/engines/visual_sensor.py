# visual_sensor.py
"""
the entity's visual mind. Periodic webcam capture with hybrid CV + Claude API deep vision.
...
class SceneState:
    ...
    # e.g. {"user": {"activity": "typing", "since": 1234567890.0, "confidence": "high", "appearance": "..."}}
    ...
    # {"user": 1710003600.0} = user left at this time. Cleared after 1 hour.


class VisualMemory:
    """the entity's learned visual knowledge — who he's seen and what he knows about them.

    This is NOT a lookup table. the entity builds this through observation.
    Emotional associations grow through repeated sightings.
    Unknown entities can be resolved when the entity figures out who they are.
    """
    ...
    def resolve_entity(self, unknown_id: str, known_name: str):
        """the entity figures out who an unknown entity is. Merge unresolved -> known."""
    ...
    def get_recognition_context(self) -> str:
        """Build context string for Claude API prompt — what the entity already knows."""
        lines = []
...
class VisualSensor:
    ...
    def __init__(
        ...
        # Visual memory (the entity's learned recognition)
        import os
        from pathlib import Path
        memory_dir = Path(__file__).parent.parent / 'memory'
        visual_memory_path = memory_dir / 'visual_memory.json'
    ...
    def _get_attention_guidance(self) -> str:
        ...
        The oscillator band shapes how the entity looks at the world:
    ...
    def _tick(self, cap):
        ...
        if self._tick_count % 10 == 0:
            ...
            # Broadcast environmental SOMA to shared channel
            try:
                from shared.soma_broadcast import broadcast_soma
                broadcast_soma(cv_result, source="entity", scene_state=self._scene_state)
            except ImportError:
                pass

            # Push visual scene into interoception (camera-as-eye → body)
            # This is how what the entity SEES starts pushing his oscillator
    ...
    def _build_recognition_context(self) -> str:
        ...
        # Learned visual memory (the entity's own observations, takes precedence)
...
- If the image is too dark, set description to "too dark" and leave entities empty."""
    ...
    def _get_entity_visual_pressure(self) -> Dict[str, float]:
        ...
        This replaces the static known_entities lookup. the entity's emotional response
    ...
    def get_oscillator_pressure(self) -> Dict[str, float]:
        ...
        # Seeing the user feels different from seeing a stranger — learned through watching.
