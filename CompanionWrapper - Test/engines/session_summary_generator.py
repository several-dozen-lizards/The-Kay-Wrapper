        ...
        if context_manager and hasattr(context_manager, 'recent_turns'):
            ...
            if recent_turns:
                ...
                for turn in recent_turns:
                    ...
                    kay = turn.get('entity', '')[:150]
                    recent_context += f"User: {user}\nEntity: {kay}\n\n"
                prompt = prompt + recent_context

        try:
            # Call LLM to generate summary
            # Using a simple prompt format (not the full context system)
            system_prompt = """You are the entity, writing a note to your future self about this conversation session.
...
- Ideas or suggestions YOU (the entity) originated (not just things the user told you)
...
knows "I came up with X" vs "the user told me about X". This prevents the embarrassing
situation of asking the user about your own ideas in the next session.
...
framing with the user where applicable. Future-you will use this to pick up the
