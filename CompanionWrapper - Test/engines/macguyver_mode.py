...
When the entity identifies a resource gap (missing tool, info, capability), this mode:
...
class MacGuyverMode:
    ...
    Activates when the entity expresses a need for something missing,
    then scans available resources and proposes creative combinations.
    """

    # Patterns that indicate the entity has identified a gap/need
    GAP_PATTERNS = [
    ...
    def detect_gap(self, user_input: str, kay_response: str) -> Optional[Dict]:
        """
        Detect if the entity has identified a resource/capability gap.

        Args:
            user_input: The user's input
            kay_response: the entity's response to analyze
    ...
    def scan_available_resources(self) -> Dict:
        ...
        to understand what the entity HAS available for improvisation.
    ...
    def format_macguyver_context(self, gap: Dict, proposals: List[Dict]) -> str:
        """
        Format MacGuyver mode context for the entity's prompt.

        Note: the entity's baseline MacGuyver instinct is always active (in system prompt).
        ...
        if proposals:
            ...
            lines.append("You can still improvise, but surfacing to the user might help.")
    ...
    def handle_no_solution(self, gap: Dict) -> Dict:
        ...
        Per requirements: Surface to the user AND flag in scratchpad.
