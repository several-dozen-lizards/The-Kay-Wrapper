...
the entity reads through entire documents internally, responding naturally to each segment
without exposing document text or navigation to the user.

User flow:
1. User clicks Import
2. the entity automatically reads through all segments
3. the entity's natural responses appear in chat as normal conversation
...
class AutoReader:
    ...
    Feeds each segment to the entity's LLM and collects natural responses.
    """
    ...
    async def read_document_async(self, doc_reader, doc_name: str, agent_state=None, start_segment=1):
        ...
        try:
            for i in range(start_pos, total_segments):
                ...
                # Build the entity's internal reading prompt
                reading_prompt = self._build_reading_prompt(chunk_info)

                try:
                    # Get the entity's response (this uses full LLM pipeline)
                    ...
                    # Display ONLY the entity's response in chat (NOT the document text)
    ...
    def read_document_sync(self, doc_reader, doc_name: str, agent_state=None, start_segment=1):
        ...
        try:
            for i in range(start_pos, total_segments):
                ...
                try:
                    # Get the entity's response (synchronous)
    ...
    def _build_reading_prompt(self, chunk_info: Dict[str, Any]) -> str:
        """
        Build the internal prompt that the entity sees (never shown to user).

        This creates natural reading instructions that prompt the entity to respond
...
your reading experience with the user.
...
Your response (be natural and specific):"""
    ...
    async def _get_kay_response_async(self, prompt: str, agent_state=None) -> str:
        """
        Get the entity's response to a segment (async version).
    ...
    def _get_kay_response_sync(self, prompt: str, agent_state=None) -> str:
        """
        Get the entity's response to a segment (synchronous version).
        """
        try:
    ...
    def _store_reading_turn(self, chunk_info: Dict[str, Any], response: str, agent_state=None):
        """
        Store the entity's reading response in memory.
...
def create_auto_reader(llm_response_func, message_display_func, memory_engine=None):
    ...
    Args:
        llm_response_func: Function that gets the entity's response to a prompt
