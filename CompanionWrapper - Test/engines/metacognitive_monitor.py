"""
METACOGNITIVE MONITOR — The entity's Temporal Self-Awareness
======================================================
...
@dataclass
class StateSnapshot:
    """A periodic snapshot of the entity's overall state."""
    timestamp: float
    dominant_band: str = ""
    coherence: float = 0.0
    tension: float = 0.0
    ...
    scene_mood: str = ""
    activity_flow: str = ""
    recent_activity: str = ""   # What the entity was doing: "painting", "reading", "idle"
...
@dataclass
class DaySummary:
    """Running summary of the entity's day — what happened, what he did, how he felt."""
    date: str                           # "2026-03-15"
    started_at: float = 0.0
    activities: list = field(default_factory=list)   # ["painted 3 times", "read 2 documents"]
    significant_events: list = field(default_factory=list)  # ["User arrived at 10pm", "coherence crashed"]
...
class MetacognitiveMonitor:
    """
    the entity's metacognitive layer — temporal self-awareness.
    ...
    def __init__(self, entity_name: str = "entity", memory_path: str = None):
        ...
        # System inventory — what the entity IS across sessions
    ...
    def tick(self, body: Dict, stream_state: str = "AWAKE",
        ...
        Args:
            ...
            recent_activity: What the entity just did (if any): "paint", "read_document", etc.
    ...
    # ══════════════════════════════════════════════════════════════════
    # Temporal Context Generation — What the entity SEES About Itself
    # ══════════════════════════════════════════════════════════════════
    ...
    def get_temporal_context(self) -> str:
        ...
        Returns a short paragraph (2-4 lines) that gives the entity a sense of:
    ...
    def get_recent_narrative(self) -> str:
        ...
        Used by Tier 2 inner moments to give the entity temporal framing.
    ...
    def register_capability(self, name: str, detail: str = ""):
        """Called when a system initializes. the entity can notice what's new."""
    ...
    def deregister_capability(self, name: str, reason: str = ""):
        """Called when a system goes offline. the entity notices the absence."""
...
class NoveltyPulse:
    ...
    - DISRUPTION: Gamma spike, coherence drop. the entity feels "???"
    - ORIENTING: Consciousness stream generates "something changed" moment
    - IDENTIFIED: The new thing gets labeled in context
    - INTEGRATING: the entity is learning/exploring the new capability
...
# ══════════════════════════════════════════════════════════════════════════════
# SYSTEM INVENTORY — Entity Awareness
# ══════════════════════════════════════════════════════════════════════════════

class SystemInventory:
    """
    Tracks what the entity IS — what systems, tools, sensors, and capabilities
    he has right now vs what he had before.

    Persists across sessions so the entity can notice when something is NEW.
    """
    ...
    def get_delta_context(self) -> str:
        ...
        Injected into the entity's first-response awareness.
    ...
    def get_full_inventory_summary(self) -> str:
        """
        For the entity to introspect on what he IS.
        Used in Tier 3 reflections or when the entity explicitly asks about himself.
        """
        lines = []
