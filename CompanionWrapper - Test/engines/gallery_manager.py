...
"""
Gallery Manager for the entityZero
Tracks all images uploaded to the entity with metadata and emotional context.
"""
...
class GalleryManager:
    """
    Manages gallery of images shared with the entity.

    Features:
    - Tracks image metadata (filename, path, timestamp, size)
    - Links to visual memory (the entity's emotional response)
    - Stores copies in gallery folder for persistence
    - Supports re-sending images to conversation
    """

    _ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    GALLERY_DIR = os.path.join(_ROOT, "memory", "gallery")
    GALLERY_DATA_FILE = os.path.join(_ROOT, "memory", "gallery.json")
    ...
    def add_image(
        ...
        Args:
            ...
            kay_response: the entity's response to the image
            emotional_response: List of emotions the entity expressed
        ...
        entry = {
            ...
            "kay_response": kay_response[:500] if kay_response else "",
    ...
    def search_images(self, query: str) -> List[Dict[str, Any]]:
        ...
        for item in self.gallery_items:
            ...
            if query_lower in item.get("conversation_context", "").lower():
                results.append(item)
                continue
            # Search in the entity's response
            if query_lower in item.get("kay_response", "").lower():
                results.append(item)
                continue
    ...
    def update_image_entry(
        self,
        image_id: str,
        kay_response: str = None,
        emotional_response: List[str] = None
    ):
        """
        Update an existing gallery entry with the entity's response.

        Used when re-showing an image to the entity to record the new response.
        """
        for item in self.gallery_items:
            if item.get("id") == image_id:
                if kay_response:
                    # Append new response if different
                    existing = item.get("kay_response", "")
                    if kay_response not in existing:
                        item["kay_response"] = f"{existing}\n---\n{kay_response[:500]}" if existing else kay_response[:500]
