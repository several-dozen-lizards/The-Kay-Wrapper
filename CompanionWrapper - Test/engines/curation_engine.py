...
the entity reviews his own memories and makes decisions:
- KEEP: Memory is still relevant and accurate
- COMPRESS: Merge with similar memories (auto-applied)
- DISCARD: Memory is outdated/wrong/redundant (queued for user approval)

Runs during sleep states. Wrapper provides structure, the entity provides judgment.
"""
import json
import os
import time
...
class CurationEngine:
    ...
    def _load_state(self) -> Dict:
        ...
        return {
            ...
            "contradictions_entity_resolved": 0,
        }
    ...
    def format_curation_prompt(self, batch: List[Dict], category: str) -> str:
        """Build a prompt for the entity to evaluate a batch of memories."""
        lines = []
        ...
        lines.append("Memories about the user's life, your identity, and important events should be KEPT.")
    ...
    def apply_decisions(self, decisions: List[Dict], batch: List[Dict]) -> Dict:
        ...
        DISCARD: queue for user approval
        """
    ...
    def auto_resolve_transient_contradictions(self) -> Dict:
        ...
        Contested contradictions are left for the entity.
    ...
    def get_contested_contradictions(self, limit: int = 5) -> List[Dict]:
        """Get contested contradictions for the entity to review."""
        eg = self.memory.entity_graph
        contested = []
    ...
    def format_contradiction_prompt(self, contradictions: List[Dict]) -> str:
        """Build prompt for the entity to resolve contested contradictions."""
        lines = []
    ...
    def apply_contradiction_resolutions(self, resolutions: List[Dict], contradictions: List[Dict]):
        """Apply the entity's contradiction resolutions."""
        eg = self.memory.entity_graph
        ...
        for res in resolutions:
            ...
            if 0 <= idx < len(contradictions):
                ...
                if entity and c["attribute"] in entity.attributes:
                    ...
                    self._state["contradictions_entity_resolved"] = (
                        self._state.get("contradictions_entity_resolved", 0) + 1
                    )
                    self._log_decision({
    ...
    # ──────────────────────────────────────────────
    # Approval queue (for user)
    # ──────────────────────────────────────────────
    
    def get_pending_discards(self) -> List[Dict]:
        """Get memories queued for user's discard approval."""
    ...
    def reject_discard(self, index: int) -> bool:
        """User rejects a discard — memory stays, remove from queue."""
        if 0 <= index < len(self._pending_discards):
            item = self._pending_discards.pop(index)
    ...
    def get_stats(self) -> str:
        ...
        lines = [
            ...
            f"Discards pending user approval: {pending}",
            f"Contradictions auto-resolved: {s.get('contradictions_auto_resolved', 0)}",
            f"Contradictions the entity resolved: {s.get('contradictions_entity_resolved', 0)}",
            "=" * 50,
        ]
        return "\n".join(lines)
