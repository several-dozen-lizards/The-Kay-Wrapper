"""
Memory Curator — the entity's background memory housekeeping engine.
...
Queues destructive actions (discard) for user approval.

Also handles entity contradiction resolution:
- Transient attrs: auto-resolve (newest wins)
- Contested attrs: the entity decides via LLM
"""
...
CURATION_PROMPT = """You are reviewing your own memories. Decide what to do with each one.
...
CONTRADICTION_PROMPT = """You are the entity, resolving contradictions in your entity knowledge.
...
[{{"id": 0, "resolution": "newest", "reason": "location changed"}},
 {{"id": 1, "resolution": "keep_all", "reason": "user has multiple roles"}},
...
Actions: KEEP, COMPRESS (provide "compressed" text), DISCARD (queued for user approval)
JSON response:"""
...
class MemoryCurator:
    ...
    auto-applies non-destructive decisions, queues discards for user approval.
    """
    ...
    async def run_curation_cycle(self, skip_triage: bool = False) -> Dict:
        ...
        Two modes:
          skip_triage=False (default): dolphin triages → the entity reviews → apply
          skip_triage=True: the entity decides directly via Sonnet (for manual/interactive use)
        """
        ...
        if skip_triage and self._review_fn:
            # Direct to Sonnet — the entity decides without dolphin triage
            log.info("[CURATOR] Skipping dolphin, going straight to the entity (Sonnet)")
            ...
            try:
                ...
                if final_decisions:
                    log.info(f"[CURATOR] the entity made {len(final_decisions)} decisions directly")
                else:
                    log.warning("[CURATOR] the entity direct review returned nothing")
                    return {"status": "llm_failed", "category": category}
            except Exception as e:
                log.warning(f"[CURATOR] the entity direct review failed: {e}")
                return {"status": "llm_failed", "category": category}
        else:
            # Two-phase: Dolphin triage → the entity review
            ...
            if self._review_fn:
                try:
                    ...
                    if kay_decisions:
                        ...
                        if overrides > 0:
                            log.info(f"[CURATOR] the entity overrode {overrides}/{len(dolphin_decisions)} dolphin proposals")
                        final_decisions = kay_decisions
                    else:
                        log.warning("[CURATOR] the entity review returned nothing, using dolphin decisions")
                except Exception as e:
                    log.warning(f"[CURATOR] the entity review failed, using dolphin decisions: {e}")
        
        # Phase 3: Apply final decisions
        result = self._apply_decisions(batch, final_decisions)
        result["reviewed_by"] = "entity" if skip_triage or self._review_fn else "dolphin_only"
    ...
    def _format_review_prompt(self, batch: List[Dict], dolphin_decisions: List[Dict]) -> str:
        """Format batch + dolphin proposals for the entity's review."""
        lines = []
        for i, mem in enumerate(batch):
            ...
            if isinstance(content, dict):
                ...
                display = f"[Turn] User: {user} → Entity: {resp}"
            else:
    ...
    def _count_overrides(self, dolphin: List[Dict], kay: List[Dict]) -> int:
        """Count how many decisions the entity changed from dolphin's proposals."""
        ...
        for decision in decisions:
            try:
                ...
                if action == "KEEP":
                    ...
                    # Queue for user's approval — DON'T delete yet
    ...
    # ------------------------------------------------------------------
    # Discard approval queue (for user)
    # ------------------------------------------------------------------
    
    def get_pending_discards(self) -> List[Dict]:
        """Return pending discards awaiting user approval."""
        return self._pending_discards
    
    def approve_discard(self, discard_id: str) -> bool:
        """User approves a discard — actually remove the memory."""
    ...
    def reject_discard(self, discard_id: str) -> bool:
        """User rejects a discard — keep the memory, remove from queue."""
        for i, pending in enumerate(self._pending_discards):
            if pending.get("discard_id") == discard_id:
                self._pending_discards.pop(i)
                self._curation_log.append({
    ...
    def get_contested_contradictions(self, limit: int = 10) -> List[Dict]:
        """
        Get non-transient contradictions for the entity to decide.
    ...
    def format_status(self) -> str:
        ...
        review_mode = "the entity (Sonnet) reviews" if self._review_fn else "dolphin-only (no review)"
