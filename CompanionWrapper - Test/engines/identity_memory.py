...
- user_assigned: the user explicitly telling the entity something about himself
"""
...
class IdentitySourceType(Enum):
    ...
    USER_ASSIGNED = "user_assigned"        # the user explicitly telling the entity something about himself
...
class IdentityMemory:
    ...
    Stores facts about:
    - the user: appearance, family, pets, core traits
    ...
    def __init__(self, file_path: str = None):
        ...
        self.user_identity: List[Dict[str, Any]] = []  # Facts about the user
    ...
    def _load_from_disk(self):
        ...
        try:
            with open(self.file_path, "r", encoding="utf-8") as f:
                data = json.load(f)
                self.user_identity = data.get("re", [])
                self.kay_identity = data.get("entity", [])
                self.entities = data.get("entities", {})

                print(f"{etag('IDENTITY')} Loaded {len(self.user_identity)} user facts, "
    ...
    def is_identity_fact(self, fact: Dict[str, Any]) -> bool:
        ...
        IDENTITY = ONLY the entity's essential nature, physical form, architecture, and relationship to the user
        WORKING MEMORY = Everything else (the user's facts, entity facts, events, conversation)
        ...
        # Facts from imported documents are ABOUT things the entity READ, not about the entity himself
        ...
        # === RULE 3: ONLY entity perspective can be identity ===
        # the user's facts, entity facts → working memory (retrieved based on relevance)
        if perspective != "entity":
            return False

        # === RULE 4: ONLY the entity itself can have identity ===
        ...
        # Only match when it explicitly connects the entity to origins
        kay_architecture = [
            "kay is an llm",
        ...
        # === RULE 9: the entity's relationship to the user ===
        kay_re_relationship = [
    ...
    def classify_source_type(self, fact: Dict[str, Any]) -> IdentitySourceType:
        ...
        if source_speaker == "user":
            ...
            return IdentitySourceType.SELF_REPORT  # Actually user's self-report
    ...
    def add_fact(self, fact: Dict[str, Any]) -> bool:
        ...
        if perspective == "user":
            # Check for duplicates
            if not self._is_duplicate(self.user_identity, fact):
                self.user_identity.append(fact)
                print(f"{etag('IDENTITY VERIFIED')} Added user fact (source: {source_type.value}): {fact.get('fact', '')[:60]}...")
    ...
    def get_all_identity_facts(self) -> List[Dict[str, Any]]:
        ...
        the user's facts, entity facts (pets, pigeons, etc.) are now retrieved
        ...
        """
        # ONLY the entity's identity (physical form, nature, architecture, relationship to the user)
    ...
    def get_re_identity(self) -> List[Dict[str, Any]]:
        """Get facts about the user specifically."""
    ...
    def get_identity_summary_for_dashboard(self) -> Dict[str, Any]:
        ...
        # Categorize entity identity facts by source type
        for fact in self.kay_identity:
        ...
        return {
            ...
            "stats": {
                ...
                "total_re": len(self.user_identity)
            }
        }
    ...
    def get_summary(self) -> str:
        ...
        return f"{total} total facts ({len(self.user_identity)} User, {len(self.kay_identity)} the entity, {len(self.entities)} entities)"
    ...
    def check_ownership(self, entity_name: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict with:
            - "owner": canonical name of owner ("user", "entity", or None)
        ...
        # Check user's identity facts for ownership
        for fact in self.user_identity:
            fact_text = fact.get("fact", "").lower()
            entities = fact.get("entities", [])
            ...
            if entity_name.lower() in [e.lower() for e in entities]:
                ...
                if any(pattern in fact_text for pattern in [
                    ...
                    "my cat", "my dog", "my pet"  # in user perspective = user's
                ]):
                    result["owner"] = "user"
        ...
        if entity_name in self.entities:
            for fact in self.entities[entity_name]:
                ...
                # User perspective + possessive = the user owns it
                if perspective == "user" and any(pattern in fact_text for pattern in ["my", "re's"]):
                    result["owner"] = "user"
                    result["confidence"] = "ground_truth"
                    result["facts"].append(fact)
                # entity perspective + possessive = the entity owns it (but this should be rare)
    ...
    def get_facts_for_entity(self, entity_name: str) -> List[Dict[str, Any]]:
        ...
        # Check if entity is the user
        if entity_name.lower() == "re":
            return self.user_identity

        # Check if entity is the primary entity
        ...
        # Also check the user's facts for mentions of this entity
        for fact in self.user_identity:
