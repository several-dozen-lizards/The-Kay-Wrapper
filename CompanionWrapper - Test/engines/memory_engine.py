...
- Temporal awareness (the entity knows when facts changed)
...
class MemoryEngine:
    ...
    emotional tagging, perspective tagging ("user", "entity", or "shared"),
    ...
    def retrieve_biased_memories(self, bias_cocktail, user_input, num_memories: int = 7, relevance_floor: float = 0.3):
        ...
        # This ensures the entity never loses his core identity facts even with zero keyword overlap
...
3. **CRITICAL FOR LISTS**: If user provides a list (e.g., "My cats are A, B, C, D, E"), extract:
   - ONE fact for the complete list: "user has 5 cats: A, B, C, D, E"
   - SEPARATE facts for EACH item: "[pet] is the user's cat", "[pet] is the user's cat", etc.
   - DO NOT bundle everything into a single generic fact
4. Determine perspective for each fact:
   - "user" = facts about the user (the person typing)
   - "entity" = facts about the entity (the AI)
...
- The user is the USER (the person typing)
- the entity is the AI (being addressed)

FROM USER INPUT:
- "I/my/me" in user input = the user (user perspective)
- "you/your" in user input = the entity (entity perspective)
- Entities mentioned = the user's entities

FROM KAY'S RESPONSE:
- "your/you" in the entity's response = about the user (USER perspective, NOT the entity)
- "my/I/me" in the entity's response = about the entity ONLY if DIRECT SELF-ASSERTION
  - Direct self-assertion: "My eyes are gold", "I prefer coffee", "I am a dragon"
  - NOT self-assertion: "my memory says...", "my understanding...", "my cats - [known user entities]"
- When the entity mentions entities that are known to belong to User, DO NOT create entity ownership
- Conversational references to the user's life = the user's facts, not the entity's facts

CRITICAL: When the entity says things like "your cats", "your dog", "you have", extract these as REINFORCING the user's ownership, NOT creating entity ownership.

DOCUMENT/READING CONTEXT:
When conversation involves documents, files, or imported content:
- the entity READS documents, doesn't EXPERIENCE them
- "the entity examined Archive Zero" → WRONG (implies lived experience)
- "the entity read documents about Archive Zero" → CORRECT (reading activity)
- If the entity is reading about something, use: "the entity read about X", "the entity learned about X from document"
- Characters/events IN documents are NOT the entity's experiences - they are content the entity READ ABOUT
- Activity attributes for the entity + document content should use "reading" language, not "examining/exploring/investigating"

CAMERA/VISUAL OBSERVATION CONTEXT:
the entity has a camera and can see the user. When the entity describes what he SEES, these are transient observations, NOT permanent facts:
- "I can see you're on a call" → TRANSIENT (don't extract user.activity)
- "The light's hitting you from that angle" → TRANSIENT (don't extract user.lighting)
- "You're at the desk with your hand near your face" → TRANSIENT (don't extract user.posture, user.gesture)
- "The room is dark" → TRANSIENT (don't extract user.environment)
- "Looks like you're focused on something" → TRANSIENT (don't extract user.mood)
...
- What the user appears to be doing (on call, streaming, typing, thinking)
...
User: "My dog is [dog]"
Entity: "That's a great name!"
→ Extract:
[
  {{
    "fact": "[pet] is the user's pet",
    "perspective": "user",
    "topic": "pets",
    "entities": ["[dog]", "user"],
    "attributes": [{{"entity": "[dog]", "attribute": "species", "value": "dog"}}],
    "relationships": [{{"entity1": "user", "relation": "owns", "entity2": "[dog]"}}]
  }}
]
Note: the entity's response "That's a great name!" contains NO factual claims, so nothing extracted from it.

EXAMPLE 2 - the entity makes conversational reference to the user's pets:
User: "My cats are [cat], [cat], [cat]"
Entity: "Your cats - [cat], [cat], [cat] - sound wonderful!"
→ Extract:
[
  {{
    "fact": "user has 3 cats: [cat], [cat], [cat]",
    "perspective": "user",
    "topic": "pets",
    "entities": ["user", "[cat]", "[cat]", "[cat]"],
    "relationships": [{{"entity1": "user", "relation": "owns", "entity2": "cats"}}]
  }},
  {{
    "fact": "[pet] is the user's pet",
    "perspective": "user",
    ...
    "relationships": [{{"entity1": "user", "relation": "owns", "entity2": "[cat]"}}]
  }},
  {{
    "fact": "[pet] is the user's pet",
    "perspective": "user",
...
Note: the entity says "Your cats" - this is about the user's cats, NOT the entity's. Do NOT create "the entity owns X".

EXAMPLE 3 - the entity makes direct self-assertion:
User: "What color are your eyes?"
Entity: "My eyes are gold."
→ Extract:
[
  {{
    "fact": "the entity's eyes are gold",
    "perspective": "entity",
    "topic": "appearance",
    "entities": ["entity"],
    "attributes": [{{"entity": "entity", "attribute": "eye_color", "value": "gold"}}]
  }}
]
Note: This IS a direct self-assertion about the entity, so extract as kay perspective.

EXAMPLE 4 - the entity confused but describing the user's entities (DO NOT EXTRACT):
User: "My cats are [cat] and [cat]"
Entity: "Yeah, my cats - [cat] and [cat] - are great!"
→ Extract:
[
  {{
    "fact": "[pet] is the user's pet",
...
Note: Even though the entity says "my cats", the context shows these are the user's cats (user just stated it).
Do NOT create "the entity owns [cat]/[cat]". the entity is confused/echoing. Only extract from user input.

EXAMPLE 5 - User expresses desire/goal:
User: "I want to fix this wrapper persistence issue"
Entity: "What have you tried so far?"
→ Extract:
[
  {{
    "fact": "user desires to fix wrapper persistence",
    "perspective": "user",
    "topic": "goals",
    "entities": ["user", "wrapper"],
    "attributes": [{{"entity": "user", "attribute": "desire", "value": "fix wrapper persistence"}}, {{"entity": "user", "attribute": "goal_status", "value": "active"}}]
  }}
]

EXAMPLE 6 - User expresses frustration (progression update):
User: "Still not working. Third approach failed."
Entity: "That's frustrating."
→ Extract:
[
  {{
    "fact": "the user's wrapper fix attempts are stuck (3 failures)",
    "perspective": "user",
    "topic": "goals",
    "entities": ["user", "wrapper"],
    "attributes": [{{"entity": "user", "attribute": "goal_progression", "value": "stuck"}}, {{"entity": "user", "attribute": "attempt_count", "value": "3"}}]
  }}
]

EXAMPLE 7 - User expresses fear:
User: "I'm worried [cat] might get out through the broken window"
Entity: "That's a valid concern."
→ Extract:
[
  {{
    "fact": "user fears [pet] escaping through broken window",
    "perspective": "user",
    "topic": "goals",
    "entities": ["user", "[cat]", "window"],
    "attributes": [{{"entity": "user", "attribute": "fear", "value": "[cat] escaping"}}, {{"entity": "window", "attribute": "condition", "value": "broken"}}]
  }}
]

EXAMPLE 8 - User CORRECTS the entity about a fact:
User: "No, those ChatGPT conversations were from 2024-2025, not 2020"
Entity: "Oh, you're right - I had the dates wrong."
→ Extract:
[
  {{
...
EXAMPLE 9 - User corrects the entity's mistake:
User: "Actually, [dog] is 3 years old, not 5"
Entity: "Got it, my mistake."
→ Extract:
[
  {{
...
Extract facts now:"""
        ...
        try:
            ...
            if facts is None:
                ...
                fallback_perspective = "entity" if response and not user_input else "user"
            ...
            if not isinstance(facts, list):
                ...
                facts = [facts] if isinstance(facts, dict) else [{"fact": str(facts)[:200], "perspective": "entity", "topic": "exploration"}]

            # Validate structure and process entities
            validated_facts = []

            # CRITICAL FIX: Determine source speaker
            # If user_input is empty and response exists, facts are from the entity's response
            # If user_input exists and response is empty, facts are from user input
            source_speaker = "user" if user_input and not response else "entity"

            for fact in facts:
                if isinstance(fact, dict) and "fact" in fact:
                    perspective = fact.get("perspective", "user")

                    # CRITICAL BUG FIX: Detect the entity's claims about the user
                    # When the entity makes a statement about the user (perspective="user"),
                    # it should NOT be stored as ground truth - mark for confirmation
                    needs_confirmation = False
                    if source_speaker == "entity" and perspective == "user":
                        # the entity is making a claim about the user
                    ...
                    # IMPORTANT: Pass source_speaker to prevent the entity's claims from becoming truth
    ...
    def _process_entities(self, fact_data: Dict[str, Any], source_speaker: str = None):
        ...
        Prevents the entity from creating false relationships when confused about ownership.
        ...
        When the entity says "Your eyes are brown", perspective=user but source_speaker=kay.

        Args:
            fact_data: Fact dict with entities, attributes, and relationships
            source_speaker: Who said this fact ("user" or "entity")
        """
        ...
        for attr_data in fact_data.get("attributes", []):
            ...
            if entity_name and attribute_name and value:
                ...
                # the entity READS documents, doesn't EXPERIENCE them
                if entity_name == "entity" and attribute_name == "activity":
                    ...
                    if has_experience_word and has_document_indicator:
                        # Reframe: the entity READ these documents, didn't experience them
                        ...
                        print(f"[ACTIVITY CORRECTION] '{original_value}' → '{value}' (the entity reads, doesn't experience)")
        ...
        for rel_data in fact_data.get("relationships", []):
            ...
            if entity1 and relation_type and entity2:
                ...
                if relation_type == "owns":
                    # IMPORTANT: Only verify the entity's statements, not user's
                    # User statements are ALWAYS authoritative (ground truth)

                    if speaker == "entity":
                        # the entity is making an ownership claim - verify against identity layer
                        ...
                        if conflict_check["should_block"]:
                            # BLOCK: the entity is confused about ownership
                            ...
                            # No conflict - but still inferred since the entity said it
                            fact_data["confidence"] = "inferred"
                        ...
                        # Check if this CORRECTS a previous entity confusion
                        conflict_check = self.entity_graph.check_ownership_conflict(
                            entity=entity2,
                            claimed_owner=entity1,
                            identity_memory=self.identity
                        ...
                        if conflict_check["conflict"]:
                            # User is correcting the entity's previous confusion
                            print(f"[OWNERSHIP CORRECTION] User establishes ground truth: {entity1} owns {entity2} (corrects previous entity confusion)")
                        else:
    ...
    def _apply_user_correction(self, correction_data: Dict[str, Any]):
        ...
        When the user corrects the entity about a fact, this propagates that correction
        ...
        if result["corrections_applied"] > 0:
            ...
            BROAD_SEARCH_BLOCKED_VALUES = {"entity", "reed", "re", "john", "zero", "chrome", "saga"}
    ...
    def _detect_perspective(self, user_input: str) -> str:
        ...
        - "user" = facts about the user (the person typing)
        - "entity" = facts about the entity (the agent - the AI)
        - "shared" = facts about both or shared experiences

        CRITICAL RULES:
        1. The USER is ALWAYS the person typing
        2. "I/my/me" = the speaker (the user)
        3. "you/your" = the addressee (the entity, the AI)
        4. Mentioned names (other names mentioned) are THIRD PARTIES, not the user
        5. The user's name is "user" - any other name is someone else
        ...
        # First-person pronouns = user (User, the person typing)
        ...
        # Second-person pronouns = the entity (the AI being addressed)
    ...
    def _validate_fact_against_sources(self, fact: str, fact_perspective: str, retrieved_memories: List[Dict]) -> bool:
        """
        Validate that the entity's claimed facts about the user were actually stated by the user.

        Returns True if fact is VALID (should be stored), False if HALLUCINATION (should be blocked).

        CRITICAL: Prevents the entity from inventing/fabricating details that weren't mentioned.
        """
        # Only validate the entity's statements about the user
        if fact_perspective != "entity":
            return True  # User's own statements are always valid

        fact_lower = fact.lower()

        # Check if the entity is claiming a fact about the user (contains "you/your" or user entities)
        is_about_user = any(word in fact_lower for word in ["you", "your", "re's", "re "])

        if not is_about_user:
            return True  # the entity's statements about himself are not validated here

        # the entity is making a claim about the user - verify it was actually mentioned
            ...
            if fact_colors and "eye" in combined_user_text:
                ...
                for fact_color in fact_colors:
                    if fact_color not in mem_colors:
                        # the entity is adding a color detail user never mentioned
                        print(f"[HALLUCINATION DETAIL] the entity added color '{fact_color}' but user only mentioned {mem_colors}")
        ...
        # We can't validate everything, so we trust the entity unless we can prove fabrication
        return True
    ...
    def _extract_entity(self, fact_text: str) -> str:
        ...
        if any(pattern in fact_lower for pattern in ["i ", "my ", "i'm", "i am"]):
            # First person - depends on context
            return "entity"
    ...
    def _check_contradiction(self, new_fact: str, retrieved_memories: List[Dict]) -> bool:
        ...
        1. Only compare SAME entity (entity comparison, user vs user)
    ...
    def encode_memory(self, user_input, response, emotional_cocktail, emotion_tags, perspective=None, agent_state=None, connection_data=None):
        ...
        # FIX: Extract facts from the entity's response ONLY
        ...
        print(f"[MEMORY] Extracted {len(extracted_facts)} facts from the entity's response (user facts already stored in pre-response)")
        ...
        for fact_data in extracted_facts:
            ...
            # CRITICAL: Distinguish the entity's observations from false attributions
            if needs_confirmation:
                ...
                if not should_store:
                    # False attribution (the entity claiming user SAID something) - BLOCK
                    print(f"[FALSE ATTRIBUTION] X the entity claimed: '{fact_text[:60]}...' - NOT STORING.")
                    print(f"[FALSE ATTRIBUTION]   Reason: False attribution (user didn't say this)")
                    print(f"[FALSE ATTRIBUTION]   Source: entity | Perspective: {fact_perspective} | Topic: {fact_topic}")
                    continue  # Skip this fact

                if storage_type == "entity_observation":
                    # Valid observation (the entity's inference about user state) - ALLOW with tagging
                    print(f"[ENTITY OBSERVATION] ✓ Storing the entity's observation: '{fact_text[:60]}...'")
                    print(f"[ENTITY OBSERVATION]   Type: {fact_topic} | Observer: entity → {fact_perspective}")

                    # Tag as entity observation for retrieval filtering
                    fact_data = create_entity_observation(fact_data, observer="entity", observed="re")
                    # IMPORTANT: Don't skip - continue to storage below

            # Validate the entity's statements against retrieved memories (prevent hallucination)
            if fact_perspective == "entity" and retrieved_memories:
                is_valid_fact = self._validate_fact_against_sources(fact_text, fact_perspective, retrieved_memories)

                if not is_valid_fact:
                    print(f"[HALLUCINATION BLOCKED] X the entity fabricated '{fact_text[:60]}...' - NOT STORING.")
                    continue
                ...
                if is_contradictory:
                    ...
                    # the entity's reality can change (was confused → now clear-headed = growth)
                    # Don't block legitimate state changes, just log the potential conflict
                    print(f"[CONTRADICTION FLAGGED] ! the entity stated '{fact_text[:60]}...' may conflict with memory")
                    print(f"[CONTRADICTION FLAGGED]   Storing anyway - temporal changes are valid. Flag for review.")
        ...
        for fact_record in [mem for mem in self.memories
            ...
            # CRITICAL: Document-imported facts should NEVER be the entity's identity
            # They are things the entity READ ABOUT, not things the entity IS
    ...
    def retrieve_unified_importance(self, bias_cocktail, user_input, max_memories: int = 250, conversational_mode: bool = False) -> List[Dict[str, Any]]:
        ...
        # 1.1: Identity facts (the entity's core identity)
        ...
        for mem in all_other:
            ...
            if mem_type == 'full_turn':
                ...
                # LEGACY Import-related memories: search the entity's analysis, emotional observations, and shared context
                text_blob = (
                    mem.get('fact', '') + ' ' +
                    ...
                    mem.get('shared_by', '') + ' shared ' +  # Enable "what did the user share" queries
            ...
            # This enables "spatial memory" - the entity can recall WHERE he read information
            ...
            # "I am everything" — boundaries dissolve, not identity
        ...
        if len(relevant_dynamic) < GAP_THRESHOLD and len(search_words) > 2:
            ...
            if gap_type == 'true_gap':
                ...
                # Insert at front so the entity sees it first
                ...
                # This helps the entity understand it's new territory, not a continuity break
                info_marker = {
                    'type': 'info_marker',
                    'confidence': 'inferred',
                ...
                # Insert at front so the entity sees it
    ...
    def get_document_provenance(self, query: str) -> Optional[Dict]:
        ...
        Enables "spatial memory" - the entity can answer "where did you read about X?"

        Args:
            query: Search query (e.g., "whale song", "the user's favorite color")
    ...
    def _determine_chunk_count(self, query: str) -> int:
        ...
        TOKEN BUDGET: the entity's context already uses ~95k chars before documents.
    ...
    def retrieve_rag_chunks(self, query: str, n_results: int = None) -> List[Dict[str, Any]]:
        ...
        This allows the entity to "remember" uploaded documents without storing
    ...
    def store_document_summary(self, doc_id: str, filename: str, summary: str, entities: List[str]):
        ...
        This allows the entity to remember the "big picture" of a document
        ...
        summary_fact = {
            ...
            "doc_id": doc_id,
            "filename": filename,
            "fact": f"the entity read '{filename}': {summary}",
    ...
    def recall(self, agent_state, user_input, bias_cocktail=None, num_memories=30, use_multi_factor=True, include_rag=True, conversational_mode=False, osc_state=None):
        ...
        grouped = {
            ...
            "entity": [m for m in memories if m.get("perspective") == "entity"],
        ...
        # ADAPTIVE RETRIEVAL with BAND-BASED LIMITS (System H)
        rag_chunks = []
        if include_rag and self.vector_store:
            # Apply band-based retrieval limits when oscillator state is available
            ...
            rag_chunks = self.retrieve_rag_chunks(user_input, n_results=_rag_limit)
    ...
    def extract_and_store_user_facts(self, agent_state, user_input: str) -> List[Dict[str, str]]:
        """
        Extract facts from user input BEFORE the entity responds.
        ...
        CRITICAL: This prevents the entity from hallucinating when user provides facts.
        """
        import time
        ...
        for match in rel_matches:
            ...
            fact_text = f"{obj_name} is the user's {relation}"
            ...
            relationship_fact = {
                ...
                "perspective": "user",
                "topic": "relationships",
                "entities": ["user", obj_name],
                "attributes": [{"entity": obj_name, "attribute": "relation_to_re", "value": relation}],
                "relationships": [{"entity1": "user", "relation": "has_" + relation, "entity2": obj_name}],
        ...
        for fact_data in extracted_facts:
            ...
            # Track preferences if this is about the entity
            if fact_perspective == "entity":
            ...
            # Generic: "user has a spouse", "user has a dog" (no specific name)
            # Specific: "[partner] is the user's spouse", "[pet] is the user's pet" (has specific name)
            is_generic_relationship = False
            fact_text_lower = fact_text.lower()

            # Detect generic relationship patterns (no specific name)
            # Generic pattern: starts with "user has" or ends with "user's [relation]" without a name
            generic_patterns = [
                r"^re has (a|an|\d+) (husband|wife|spouse|dog|cat|pet)s?\.?$",  # "user has a dog"
                r"^re has (husband|wife|spouse|dog|cat|pet)s?\.?$",  # "user has husband"
            ]
    ...
    def store_visual_memory(
        ...
        Args:
            image_description: Description of what the entity saw
            kay_response: the entity's full response to the image
            emotional_response: List of emotions the entity reported feeling
        ...
        memory_entry = {
            ...
            'perspective': 'shared',  # Visual shared between user and entity
        ...
        if entities_detected and self.entity_graph:
            ...
            # Track relationships
            self.entity_graph.add_relationship('entity', 'witnessed', visual_entity_id)
    ...
    def log_memory_entry(self, conversation_turn: dict, agent_state, memory_stack: list = None) -> Dict[str, Any]:
        ...
        if speaker == "user":
            ...
            # the entity's response - extract from perspective of the entity speaking
            extracted_facts = self._extract_facts_with_entities("", raw_text)
...
Generate interpretation now:"""
    ...
    def _calculate_base_score(self, mem: Dict[str, Any], bias_cocktail: dict, user_input: str) -> float:
        ...
        query_entities = [word for word in search_words if word[0].isupper() or word in ["re", "entity"]]
