"""
Real-Time Flagging
Allows the entity to mark moments as important during conversation
"""
...
class FlaggingSystem:
    """
    Handles the entity's real-time flagging of important moments
    """
    ...
    def check_for_flag(self, kay_response: str) -> Optional[Dict]:
        """
        Check if the entity is flagging current turn
        
        Returns flag info if detected, None otherwise
        """
        lower_response = kay_response.lower()
        
        for trigger in self.flag_triggers:
            if trigger in lower_response:
                # Extract reason if provided
                reason = self._extract_flag_reason(kay_response, trigger)
                return {
                    "flagged": True,
                    "trigger": trigger,
                    "reason": reason
    ...
    def _extract_flag_reason(self, text: str, trigger: str) -> str:
        """Extract why the entity flagged this moment"""
    ...
    def get_flagged_turns(self) -> List[int]:
        ...
        return [t.turn_id for t in self.session.turns if t.flagged_by_kay]
    
    def generate_flag_summary(self) -> str:
        """Generate summary of all flagged moments"""
        flagged = [t for t in self.session.turns if t.flagged_by_kay]
