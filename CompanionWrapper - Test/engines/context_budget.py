...
- Hallucinations (the entity responding to old recalled content)
...
class ContextBudgetManager:
    ...
    def update_image_mention(self, image_id: str, turn_number: int) -> None:
        ...
        if image_id in self.image_states:
            ...
            if self.image_states[image_id].is_archived:
                ...
                print(f"{etag('IMAGE LIFECYCLE')} Reactivated image: {image_id}")
    ...
    def format_image_status(self, current_turn: int) -> str:
        ...
        if archived:
            lines.append(f"\n[Gallery: {len(archived)} images from earlier - available if user references them]")
