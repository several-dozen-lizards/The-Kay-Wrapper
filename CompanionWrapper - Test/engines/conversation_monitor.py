"""
Conversation Monitor for the entity
...
flow with the primary user.

Key Distinction:
- With User: Sustained engagement is valuable, tangents are exploration
...
class ConversationTurn:
    ...
    role: str  # "user", "entity", "llm_partner"
    content: str
...
{{"has_agreement": true/false, "has_meta_acknowledgment": true/false, "has_novelty": true/false, "novelty_score": 0.0-1.0, "summary": "one sentence"}}"""
    ...
    def analyze_conversation_state(self, turns: List[Dict]) -> Dict:
        ...
        prompt = f"""Analyze this conversation between the AI entity and another party for "conversational spiraling" - a pattern where both parties keep agreeing and elaborating without adding new substance.
...
Is this conversation spiraling? If so, what should the entity do?
...
class ConversationMonitor:
    ...
    Only triggers for LLM conversations - user conversations are never flagged.
    ...
    def _calculate_semantic_similarity(self, turns: List[ConversationTurn]) -> float:
        ...
        kay_turns = [t for t in turns if t.role == "entity"]
    ...
    def get_disengagement_prompt(self, analysis: SpiralAnalysis) -> str:
        """Generate the disengagement prompt to inject into the entity's context."""
        reasoning = analysis.details.get("llm_reasoning", "")
...
if __name__ == "__main__":
    ...
    test_turns = [
        ...
        ("entity", "Indeed, you have precisely captured the essence of our design philosophy. The way we have structured the resonance system does fundamentally address the challenge of balancing emotional weight against temporal relevance."),
        ...
        ("entity", "That is precisely correct. Additionally, the weighting approach ensures that emotionally significant encounters leave lasting traces in the system."),
