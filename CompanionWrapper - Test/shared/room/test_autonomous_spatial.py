...
def test_autonomous_spatial_kay():
    """Test autonomous spatial engine with entity in The Den."""
    print("=" * 60)
...
def test_autonomous_spatial_reed():
    """Test autonomous spatial engine with entity in Sanctum."""
    print("\n" + "=" * 60)
    ...
    room.add_entity("other_entity", "Other", distance=120, angle_deg=90, color="#6B4E8D")
...
def test_co_presence():
    ...
    room = create_the_den()
    # Add entities to the Den
    room.add_entity("entity", "entity", distance=100, angle_deg=90, color="#2D1B4E")
    room.add_entity("other_entity", "Other", distance=110, angle_deg=95, color="#6B4E8D")  # Near main entity
    ...
    print(f"   Other: distance={room.entities['other_entity'].distance:.1f}, angle={room.entities['other_entity'].angle:.1f}")

    print(f"\n2. Entity's near_object: {room.entities['entity'].near_object}")
    ...
    print(f"   Entity's near_object: {room.entities['entity'].near_object}")
    print(f"   Other entity's near_object: {room.entities['other_entity'].near_object}")
