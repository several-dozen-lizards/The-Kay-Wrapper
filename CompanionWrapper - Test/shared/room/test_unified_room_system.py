...
1. RoomManager places entity in Commons FIRST
2. wrapper_bridge sees entity already placed and uses Commons (not Den)
3. NO "[ROOM] Entity placed in The Den" log should appear
...
def test_nexus_flow_no_den_log():
    """
    Simulate Nexus flow: RoomManager places entity in Commons BEFORE wrapper_bridge init.
    Verify that wrapper_bridge uses Commons room, not Den.
    """
    print("\n=== Testing Nexus Flow: No Den Log ===")

    # Step 1: Pre-place Entity in Commons (as nexus_entity.py now does)
    print("\n[Step 1] Pre-placing Entity in Commons via RoomManager...")
    reset_room_manager()
    ...
    try:
        if rm.rooms and rm.entity_locations.get(entity_id):
            ...
            if room:
                ...
                print(f"  [ROOM] Entity using RoomManager room: {rm.rooms[current_room_id].label} ({len(room.objects)} objects)")
...
def test_standalone_flow_uses_den():
    ...
    if not room_manager_active:
        ...
        print(f"  [ROOM] Entity placed in The Den (inner ring, north — near the couch)")
