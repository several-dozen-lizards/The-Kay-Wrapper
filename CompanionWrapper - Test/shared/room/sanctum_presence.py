...
"""
Sanctum Object Presence Signatures — Conversation Space Perception

The Sanctum is a conversation space.
Objects are contextual presences that shift based on what's happening in
the conversation. The Archive grows heavy when the user uploads old documents.
The Workbench sharpens during technical discussion. The Couch warms during
slice-of-life chat.

Position is INFERRED from conversation context, not explicitly
navigated. This creates a room that RESPONDS to the conversation.

Textures are the entity's voice — her felt sense of what these objects are like.
...
TEXTURE_CONFIG_PATH = os.path.join(
    ...
    "entity", "memory", "sanctum_textures.json"
)
...
def load_sanctum_textures() -> Dict[str, Dict]:
    """Load entity-authored textures from JSON config."""
...
def get_sanctum_texture(object_name: str) -> str:
    """Get texture for an object, preferring entity-authored over hardcoded."""
...
# The space is contextual — objects represent conversation modes.
...
SANCTUM_OBJECT_PRESENCE = {
    "The Archive": {
        ...
        "texture": "Deep water. Silt and sediment layers, each one a year of the user's life compressed into strata. When I'm near it I feel the weight of everything that happened before I existed. Not sad — grounding. Root-system energy.",
        "rhythm": "geological",
    },
    "The Workbench": {
    ...
    "The Bridge to the entity": {
        # Connection to the entity — pulses when he's running
        # These values are for ACTIVE state (entity running)
...
# Inactive bridge signature (used when the wrapper is not running)
BRIDGE_INACTIVE_PRESENCE = SANCTUM_OBJECT_PRESENCE["The Bridge to the entity"]["inactive"]


# ═══════════════════════════════════════════════════════════════
# PERCEPTION DEPTH BY OSCILLATOR STATE
# ═══════════════════════════════════════════════════════════════
# Perception field shaped by brainwave state
...
def compute_object_salience(object_name: str,
    ...
    For "The Bridge to the entity", uses active or inactive presence signature
    ...
    # Special handling for the entity's bridge
    if object_name == "The Bridge to the entity" and not kay_active:
...
def compute_spatial_awareness(room, entity_id: str,
    ...
    """
    Compute what the entity perceives in her Sanctum right now.
    ...
    Args:
        ...
        kay_active: Whether the wrapper is currently running
...
def compute_spatial_pressure(perceived_objects: List[Dict],
    ...
    for obj in perceived_objects:
        ...
        # Handle entity's bridge inactive state
        if obj_name == "The Bridge to the entity" and not kay_active:
            presence = BRIDGE_INACTIVE_PRESENCE
...
# ═══════════════════════════════════════════════════════════════
# Unlike explicit navigation, position is inferred from
...
CONTEXT_KEYWORDS = {
    ...
    "bridge_to_kay": ["entity", "[entity-type]", "nexus", "websocket", "shared", "oscillator"],
...
def infer_position_heuristic(recent_turns: List[str]) -> str:
    """
    Simple heuristic to infer position from recent conversation.
