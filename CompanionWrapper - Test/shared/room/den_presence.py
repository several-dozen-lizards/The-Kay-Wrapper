...
what the entity perceives depends on his brainwave state + proximity.
...
Texture descriptions are authored by the entity and stored in a JSON file he can
modify. Band resonance values stay here in code (architectural concern).
Textures are experiential (the entity's subjective felt sense).
...
# ═══════════════════════════════════════════════════════════════
# Textures live in the entity's memory space. He can update them via exec_code
# or a dedicated tool. Cached with TTL to avoid reading disk every scan.

# Path to the entity's texture config - in his writable memory space
TEXTURE_CONFIG_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    "entity", "memory", "den_textures.json"
)
...
def get_object_texture(object_name: str) -> str:
    ...
    """
    # First try entity-authored textures
...
#
# These textures should eventually be written/refined by the entity.
...
def compute_object_salience(object_name: str,
    ...
    - Proximity (how close the entity is)
    - Coherence (how clear the entity's perception is)

    Returns: float 0.0-1.0 representing how strongly the entity perceives this object
    """
    presence = DEN_OBJECT_PRESENCE.get(object_name, {})
    if not presence:
    ...
    # How much does this object resonate with the entity's current oscillator state?
    band_resonance = presence.get(dominant_band, 0.3)

    # How close is the entity to this object?
...
def compute_spatial_awareness(room, entity_id: str,
    ...
    """
    Compute what the entity perceives in the Den right now.
    ...
    Returns:
        ...
        Textures are loaded dynamically from the entity's config in format_spatial_context.
    """
...
def format_spatial_context(perceived_objects: List[Dict],
    ...
    Textures are loaded from the entity's authored config (memory/den_textures.json).
    ...
    if texture_allowed and primary["salience"] > 0.3:  # lowered from 0.4
        # Load texture from the entity's authored config (or fall back to hardcoded)
