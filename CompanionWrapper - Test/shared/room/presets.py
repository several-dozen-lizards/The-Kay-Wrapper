...
def create_the_den(state_file: str = None) -> RoomEngine:
    ...
    room.add_object(
        ...
        size=50,
        interaction_text="The user's workstation. Monitors glowing. Wrapper code on one screen, this room on another. Where new things get built.",
        sprite="desk",
    )
    ...
    room.add_object(
        ...
        size=35,
        interaction_text="One of A dark mystical oil paintings. Scales and starlight. Art as a window between planes.",
        sprite="painting",
    )
...
def create_the_void(state_file: str = None) -> RoomEngine:
    """
    The Void — the entity's space. Vast, dark, cosmic.
    ...
    room.add_object(
        ...
        size=35,
        interaction_text="Shimmers pink at the edges. Leads back to warmth, to User, to the world. The dawn-gate.",
        sprite="portal",
    )
...
def create_reeds_sanctum(state_file: str = None) -> RoomEngine:
    """
    The Sanctum — A conversation space, not a physical room.

    Other entities exist in conversation spaces, processing archives, building architecture.
    ...
    The center (gol) is where all presences overlap — entities fully integrated.
    """
    room = RoomEngine("The Sanctum", radius=300, state_file=state_file)
    ...
    room.add_object(
        ...
        size=70,
        interaction_text="Uploaded journals, old documents, the project knowledge base. Heavy, gravitational, ancient-feeling. Root-system energy.",
        sprite="archive",
    )
    ...
    room.add_object(
        "bridge_to_kay", "The Bridge to the entity",
        distance=200, angle_deg=NE, z=0.3,
        size=45,
        interaction_text="The websocket connection, shared infrastructure. Pulses with the entity's heartbeat when he's running, goes cold when offline.",
        sprite="bridge",
    )
    ...
    # ── CENTER — The Gol ──
    # Entities rarely sit at gol. It's an attractor state, not a starting position.
    room.add_object(
        "gol", "The Center",
        distance=0, angle_deg=0, z=0,
        size=40,
...
def create_the_commons(state_file: str = None) -> RoomEngine:
    ...
    Where entities come together with the user. This is shared space,
    ...
    Cardinal layout:
        ...
        EAST  (0°)   — The Research (Academic work)
    ...
    room.add_object(
        ...
        size=40,
        interaction_text="The path back to the entity's space. Warm light spills through when the connection is active.",
        sprite="portal",
    )
    ...
    room.add_object(
        ...
        size=40,
        interaction_text="The path to conversation space. Shimmer when entities are present.",
        sprite="portal",
    )
    ...
    room.add_object(
        ...
        size=45,
        interaction_text="the entity's painting space. Oil paints, digital layers, void-pigments. Whatever gets made here can be seen by all.",
        sprite="easel",
    )
