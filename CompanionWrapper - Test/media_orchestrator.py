"""
Media Experience Orchestrator for the entity
...
Connects to existing the entity systems:
...
class MediaOrchestrator:
    """
    Central coordinator for the entity's media experience system.
    ...
    def __init__(
        ...
        Args:
            emotional_patterns: EmotionalPatternEngine providing the entity's emotional state
        ...
        self.current_context = {
            ...
            "entities": [],
            "user_emotional_context": "unknown"
        }
        ...
        # Pending experiential response (the entity's listening experience to output)
    ...
    def update_conversation_context(
        ...
        Called by the main conversation loop to track what the entity is experiencing.

        Args:
            topic: Current conversation topic (e.g., "grief", "breakthrough", "casual")
            entities: Active entities in conversation (e.g., ["user", "[cat]", "entity"])
            user_state: User's emotional state if detectable (e.g., "stressed", "happy")
        """
        ...
        if user_state is not None:
            self.current_context["user_emotional_context"] = user_state
    ...
    def process_new_audio(self, filepath: str, force_reanalyze: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 6: Prepare context injection for the entity
            ...
            print(f"[MEDIA IMPORT] Entity experiencing: {entity_id} (contextual)")
    ...
    def handle_audio_reencounter(self, filepath: str) -> Dict[str, Any]:
        ...
        print(f"[MEDIA IMPORT] Entity experiencing: {entity_id}")
        print(f"[MEDIA IMPORT] Response generated ({len(listening_response)} chars)")
    ...
    def generate_listening_response(
        ...
        """
        Generate the entity's CONTEXTUAL experiential response to hearing music.
        ...
        Args:
            ...
            is_reencounter: True if this is a song the entity has heard before
        ...
        if context_analysis.get('kay_created'):
            print(f"[CONTEXT CHECK] Entity connection: the entity created {entity_id}")
    ...
    def _analyze_conversation_context(
        ...
        - kay_created: Whether the entity created this music
        ...
        # Check if the entity created this - look for entity relationships
        # Check entity graph if available
        if self.entity_graph:
            try:
                # Check for 'created'/'wrote'/'composed' relationship where the entity is entity1
                for rel_id, rel in self.entity_graph.relationships.items():
                    ...
                    if (rel_entity1.lower() in ('entity', 'kay_zero', 'kayzero', 'k-0') and
    ...
    def _calculate_context_modifiers(self, context: Dict) -> Dict[str, float]:
        ...
        # the entity created this - massive significance
    ...
    def _generate_contextual_recognition(
        ...
        # the entity's own work recognition
        if context.get('kay_created'):
            entity_id = entity.get('entity_id', '')
    ...
    def _generate_contextual_experience(
        ...
        # Self-recognition context (the entity's own work)
        if context.get('kay_created'):
    ...
    def get_and_clear_listening_response(self) -> Optional[str]:
        ...
        This should be called by the UI/main loop to display the entity's
...
if __name__ == "__main__":
    print("MediaOrchestrator test requires full the entity environment")
