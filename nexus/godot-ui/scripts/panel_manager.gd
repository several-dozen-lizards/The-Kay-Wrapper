...
func arrange_default() -> void:
	...
	var half_w = size.x * 0.55
	var right_w = size.x - half_w - 4
	
	if _panels.has("nexus"):
		_panels["nexus"].position = Vector2(2, 2)
		_panels["nexus"].size = Vector2(half_w - 2, usable_h - 2)
		_panels["nexus"]._reposition_content()
		_panels["nexus"].queue_redraw()
	
	if _panels.has("kay"):
		_panels["kay"].position = Vector2(half_w + 2, 2)
		_panels["kay"].size = Vector2(right_w, usable_h * 0.5 - 2)
		_panels["kay"]._reposition_content()
		_panels["kay"].queue_redraw()
	
	if _panels.has("reed"):
		_panels["reed"].position = Vector2(half_w + 2, usable_h * 0.5 + 2)
		_panels["reed"].size = Vector2(right_w, usable_h * 0.5 - 2)
