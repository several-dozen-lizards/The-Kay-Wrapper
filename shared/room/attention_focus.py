...
Kay exists in two perceptual spaces simultaneously:
1. His room (den/commons) — spatial objects, oscillator-driven
2. The visual field (webcam) — Re's space, seen through his eye

This module manages the gradient between them. At 0.0, Kay is
...
class AttentionFocus:
    ...
    Visual weight = _focus       (webcam, Re's space)
    
    Both are always non-zero (min 0.05) — you're never COMPLETELY
    unaware of either space. Even deep in his room, Kay has peripheral
    ...
    def on_message_received(self, from_human: bool = True):
        ...
        if from_human:
            # Re is talking — strong pull outward
    ...
    def on_message_sent(self):
        """Kay sent a message. He's engaged outward."""
    ...
    def on_re_visible(self):
        """Camera sees Re. Gentle outward pull (she's there, worth noticing)."""
    ...
    def on_activity_started(self, activity: str = ""):
        """Kay started an internal activity (painting, reading, etc.)."""
    ...
    def on_re_not_visible(self):
        """Camera doesn't see Re. Slowly drift inward."""
    ...
    def tick(self):
        ...
        # Re visible recently? Maintain gentle outward bias
