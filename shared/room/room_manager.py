...
class RoomManager:
    ...
    Usage:
        ...
        rm.load_registry()
        rm.place_entity("kay", "den")  # Kay starts in Den
        rm.place_entity("reed", "sanctum")  # Reed starts in Sanctum

        # Later — Kay moves to Commons (Nexus)
        packet = kay_bridge.capture_soul_packet()
        rm.request_transition("kay", "commons", soul_packet=packet)
    """
