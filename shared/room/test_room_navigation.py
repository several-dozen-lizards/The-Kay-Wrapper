...
def test_room_registry():
    ...
    # Check default rooms
    assert rm.default_rooms.get("kay") == "den", "Kay's default room should be den"
    assert rm.default_rooms.get("reed") == "sanctum", "Reed's default room should be sanctum"
...
def test_entity_placement():
    ...
    # Place Kay in Den
    success = rm.place_entity("kay", "den")
    assert success, "Failed to place Kay in Den"
    assert rm.get_entity_room_id("kay") == "den", "Kay should be in Den"
    print(f"  Kay placed in: {rm.get_entity_room_id('kay')}")

    # Place Reed in Sanctum
    success = rm.place_entity("reed", "sanctum")
    assert success, "Failed to place Reed in Sanctum"
    assert rm.get_entity_room_id("reed") == "sanctum", "Reed should be in Sanctum"
    print(f"  Reed placed in: {rm.get_entity_room_id('reed')}")
    ...
    assert "kay" in den_occupants, "Kay should be in Den occupants"
    assert "reed" in sanctum_occupants, "Reed should be in Sanctum occupants"
...
def test_room_transition():
    ...
    result = rm.request_transition("kay", "commons", soul_packet=packet)
    ...
    assert rm.get_entity_room_id("kay") == "commons", "Kay should be in Commons after transition"
    ...
    stored_packet = rm.get_soul_packet("kay")
...
def test_room_context():
    ...
    rm.place_entity("kay", "commons")
    rm.place_entity("reed", "commons")

    # Get Kay's context
    context = rm.get_room_context("kay")
    print(f"  Kay's room context:")
    ...
    assert context.get("room_id") == "commons", "Kay should see Commons context"
    assert "reed" in context.get("other_entities", []), "Kay should see Reed in Commons"
...
def test_available_destinations():
    ...
    destinations = rm.get_available_destinations("kay")
    dest_ids = [d["room_id"] for d in destinations]

    print(f"  From Den, Kay can go to: {dest_ids}")
