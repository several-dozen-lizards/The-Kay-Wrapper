...
2. RoomManager places Kay in Commons
...
def test_room_switch_simulation():
    ...
    # Step 2: RoomManager is initialized and places Kay in Commons
    print("\n[Step 2] RoomManager places Kay in Commons...")
    reset_room_manager()
    rm = get_room_manager(WRAPPERS_ROOT)
    rm.load_registry()
    rm.place_entity("kay", "commons", color="#2D1B4E")
    print(f"  RoomManager says Kay is in: {rm.get_entity_room_id('kay')}")
    ...
    print("  If set_room(commons_room, 'kay', 'commons') is called:")
    ...
    # Add Kay entity to commons room
    commons_room.add_entity("kay", "Kay", distance=100, angle_deg=90, color="#2D1B4E")
    ...
    print(f"  Kay perceives {len(perceived)} objects in Commons:")
    for obj in perceived[:3]:
        print(f"    - {obj['name']} (salience: {obj['salience']:.3f})")

    # Verify Kay sees Commons objects, not Den objects
    ...
    assert "The Roundtable" in perceived_names or "The Wrapper Codebase" in perceived_names, \
        "Kay should perceive Commons objects"
    assert "The Couch" not in perceived_names, "Kay should NOT perceive Den objects"

    print("\n[PASS] Room wiring simulation successful!")
    print("  - Kay correctly perceives Commons objects after set_room()")
...
def test_standalone_mode():
    ...
    Kay should be in Den with Den objects driving spatial awareness.
    """
    print("\n=== Testing Standalone Mode (Kay in Den) ===")
    ...
    den_room = create_the_den()
    den_room.add_entity("kay", "Kay", distance=100, angle_deg=90, color="#2D1B4E")
    ...
    print(f"  Kay perceives {len(perceived)} objects in Den:")
    for obj in perceived[:3]:
        print(f"    - {obj['name']} (salience: {obj['salience']:.3f})")

    # Verify Kay sees Den objects
    perceived_names = [obj['name'] for obj in perceived]
    assert "The Couch" in perceived_names or "The Rug" in perceived_names, \
        "Kay should perceive Den objects"
