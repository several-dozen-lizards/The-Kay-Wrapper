...
def test_autonomous_spatial_kay():
    """Test autonomous spatial engine with Kay in The Den."""
    print("=" * 60)
...
def test_co_presence():
    ...
    room = create_the_den()
    # Add both Kay and Reed to the Den
    room.add_entity("kay", "Kay", distance=100, angle_deg=90, color="#2D1B4E")
    room.add_entity("reed", "Reed", distance=110, angle_deg=95, color="#6B4E8D")  # Near Kay
    ...
    print(f"   Kay: distance={room.entities['kay'].distance:.1f}, angle={room.entities['kay'].angle:.1f}")
    print(f"   Reed: distance={room.entities['reed'].distance:.1f}, angle={room.entities['reed'].angle:.1f}")

    print(f"\n2. Kay's near_object: {room.entities['kay'].near_object}")

    # Move both to couch
    room.move_entity_to_object("kay", "couch")
    room.move_entity_to_object("reed", "couch")

    print(f"\n3. After both move to couch:")
    print(f"   Kay's near_object: {room.entities['kay'].near_object}")
