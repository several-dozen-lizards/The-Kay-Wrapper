...
def test_nexus_flow_no_den_log():
    ...
    # Step 1: Pre-place Kay in Commons (as nexus_kay.py now does)
    ...
    print(f"  RoomManager: Kay is in {rm.entity_locations.get('kay')}")
...
def test_standalone_flow_uses_den():
    ...
    if not room_manager_active:
        ...
        room = create_the_den()
        room.add_entity(entity_id, "Kay", distance=100, angle_deg=90, color="#2D1B4E")
