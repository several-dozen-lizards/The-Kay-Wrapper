...
Used by:
- nexus_kay.py / nexus_reed.py: broadcasts through PrivateRoom
...
# Pattern to extract bracketed tags from log messages like "[Kay] Mode -> BETWEEN_THREADS"
...
class WebSocketLogHandler(logging.Handler):
    ...
    Usage:
        handler = WebSocketLogHandler(entity="kay", sink=my_async_send, loop=asyncio.get_event_loop())
    ...
    def _classify_message(self, msg: str) -> str:
        ...
        # Try to extract bracketed tag: "[Kay] Mode -> X" -> tag="MODE", msg kept as-is
        match = _TAG_PATTERN.match(msg)
        if match:
            bracket_content = match.group(1)
            # Check if it's an entity name prefix like [Kay] or [Reed]
            if bracket_content.lower() in ('kay', 'reed', 'nexus', 're'):
