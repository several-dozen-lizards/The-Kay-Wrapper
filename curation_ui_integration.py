"""
Curation UI Integration for Kay Zero

Integrates the memory curation system with the main Kay UI.
...
class CurationUIIntegration:
    ...
    def _update_main_window_progress(self, progress_data: Dict):
        """Update the main window progress UI with Kay's full reasoning."""
        try:
            ...
            for decision in latest_decisions:
                ...
                # Show Kay's summary if action was SUMMARIZE
    ...
    def _display_curation_in_main_panel(self, memories: List[Dict], parent):
        ...
        displaying Kay's reasoning with large, readable fonts.
        """
        self.current_view = "curation"
    ...
    def _update_main_panel_progress(self, progress_data: Dict):
        """Update the main panel progress UI with Kay's full reasoning."""
        try:
            current = progress_data.get("current", 0)
            total = progress_data.get("total", 1)
            stats = progress_data.get("stats", {})
    ...
    def _update_progress_ui(self, progress_data: Dict):
        """Update the progress UI with latest curation data including Kay's full reasoning."""
        try:
            current = progress_data.get("current", 0)
            total = progress_data.get("total", 1)
            stats = progress_data.get("stats", {})
            ...
            # Update status to show we're waiting for Kay
            if current < total:
    ...
    def _show_curation_results(self, parent, summary: Dict):
        """Show the curation session results with Kay's patterns and full reasoning."""
    ...
    def _analyze_curation_patterns(self, examples: Dict) -> str:
        """Analyze Kay's curation patterns from the examples."""
        patterns = []
