...
class CurationDashboard(ctk.CTkFrame):
    ...
    - Requires Judgment count (Kay must classify)
    """
        ...
        try:
            ...
            if stats.get("total_sessions", 0) > 0:
                stats_text = (
                    ...
                    f"Kay overrides: {stats['kay_overrides']} ({stats['override_rate']}%)"
                )
...
class CurationSessionPanel(ctk.CTkFrame):
    ...
    Shows memories grouped by content type for Kay to review and decide.
    """
    ...
    def _add_judgment_buttons(self, parent, memory, classification):
        """Add decision buttons when Kay needs to classify."""
        ctk.CTkLabel(
            parent,
...
class CurationResultsPanel(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        # Kay's judgment stats
        if self.session.kay_overrides > 0:
            ctk.CTkLabel(
                results_frame,
                text=f"Kay's overrides: {self.session.kay_overrides} "
                     f"({self.session.kay_overrides / max(self.session.memories_reviewed, 1) * 100:.0f}%)",
                font=ctk.CTkFont(family="Courier", size=9),
