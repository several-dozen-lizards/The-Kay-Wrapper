...
_wrapper_root = os.path.dirname(os.path.abspath(__file__))  # repo root IS the wrapper root
if _wrapper_root not in sys.path:
    sys.path.insert(0, _wrapper_root)

# ============================================================================
# JE NE SAIS QUOI — COMPANION ISOLATION
# ============================================================================
# Each companion gets their own room (directory). Nothing shared between them.
#
# Priority order for path resolution:
#   1. COMPANION_DIR env var (full companion isolation mode)
#   2. COMPANION_STATE_DIR env var (legacy multi-persona mode)
#   3. Default paths (single-persona mode)
#
# Environment variables:
#   - COMPANION_DIR: Full path to companion dir (companions/{name}/)
#   - COMPANION_NAME: Persona identifier (for logging/debugging)
#   - COMPANION_PORT: WebSocket port (passed via --room-port)
#   - COMPANION_STATE_DIR: Legacy path to persona state
#   - PERSONA_CONFIG_DIR: Legacy path to persona config

WRAPPER_ROOT = _wrapper_root
COMPANIONS_DIR = os.path.join(WRAPPER_ROOT, "companions")

# Check for full companion isolation mode
COMPANION_DIR = os.environ.get("COMPANION_DIR")
COMPANION_NAME = os.environ.get("COMPANION_NAME", None)
COMPANION_PORT = int(os.environ.get("COMPANION_PORT", "8780"))
COMPANION_STATE_DIR = os.environ.get("COMPANION_STATE_DIR", None)


def companion_path(*parts) -> str:
    """
    Resolve a path within the active companion's state directory.
    Falls back to legacy paths if no companion is active.

    Usage:
        companion_path("memory", "vector_db")  -> "companions/{name}/my_life/memory/vector_db"
        companion_path("sessions")             -> "companions/{name}/my_life/sessions"
    """
    if COMPANION_DIR:
        return os.path.join(COMPANION_DIR, "my_life", *parts)
    elif COMPANION_STATE_DIR:
        return os.path.join(COMPANION_STATE_DIR, *parts)
    return os.path.join(WRAPPER_ROOT, *parts)


def companion_config_path(*parts) -> str:
    """
    Resolve a path within the active companion's config directory.
    Falls back to legacy persona/ directory if no companion is active.

    Usage:
        companion_config_path("persona.json")  -> "companions/{name}/who_i_am/persona.json"
        companion_config_path("personality.md") -> "companions/{name}/who_i_am/personality.md"
    """
    if COMPANION_DIR:
        return os.path.join(COMPANION_DIR, "who_i_am", *parts)
    return os.path.join(WRAPPER_ROOT, "persona", *parts)


def get_active_companion_dir() -> str:
    """
    Get the directory for the currently active companion.
    Checks env var first, then active_companion.json.
    """
    # Already set via env var
    if COMPANION_DIR:
        return COMPANION_DIR

    # Check active_companion.json
    active_file = os.path.join(WRAPPER_ROOT, "active_companion.json")
    if os.path.exists(active_file):
        try:
            with open(active_file) as f:
                active = json.load(f)
            name = active.get("current")
            if name:
                path = os.path.join(COMPANIONS_DIR, name)
...
# Auto-migrate if companions/ is empty but persona/ exists
def _auto_migrate_companion():
    """One-time migration of legacy persona/memory to companion format."""
    from companion_manager import CompanionManager
    mgr = CompanionManager(WRAPPER_ROOT)

    # Only migrate if no companions exist yet
    companions = mgr.list_companions()
    if not companions:
        migrated = mgr.migrate_current()
        if migrated:
            return os.path.join(COMPANIONS_DIR, migrated)
    elif not mgr.get_current():
        # Set first companion as current if none set
        mgr.set_current(companions[0]["name"])

    return None


# Try to get active companion directory
_active_companion = get_active_companion_dir()

# If no companion set, try auto-migration
if not _active_companion and os.path.exists(os.path.join(WRAPPER_ROOT, "persona", "persona_config.json")):
    _migrated = _auto_migrate_companion()
    if _migrated:
        _active_companion = _migrated
        os.environ["COMPANION_DIR"] = _migrated
        print(f"[JNSQ] Auto-migrated and set COMPANION_DIR: {_migrated}")

# Set up resolved paths
if COMPANION_DIR:
    MEMORY_DIR = companion_path("memory")
    SESSIONS_DIR = companion_path("sessions")
    CHRONICLE_DIR = companion_path("chronicle")
    # Ensure directories exist
    for _dir in [MEMORY_DIR, SESSIONS_DIR, CHRONICLE_DIR]:
        os.makedirs(_dir, exist_ok=True)
    os.makedirs(companion_path("memory", "vector_db"), exist_ok=True)
    os.makedirs(companion_path("memory", "resonant"), exist_ok=True)
    os.makedirs(companion_path("documents"), exist_ok=True)
    print(f"[JNSQ] Loading companion from: {COMPANION_DIR}")
elif COMPANION_STATE_DIR:
    MEMORY_DIR = os.path.join(COMPANION_STATE_DIR, "memory")
    SESSIONS_DIR = os.path.join(COMPANION_STATE_DIR, "sessions")
    CHRONICLE_DIR = os.path.join(COMPANION_STATE_DIR, "chronicle")
    for _dir in [MEMORY_DIR, SESSIONS_DIR, CHRONICLE_DIR]:
        os.makedirs(_dir, exist_ok=True)
    print(f"[PERSONA ISOLATION] {COMPANION_NAME or 'unnamed'} using state dir: {COMPANION_STATE_DIR}")
else:
    # Default paths (single-persona mode)
    MEMORY_DIR = os.path.join(_wrapper_root, "memory")
    SESSIONS_DIR = os.path.join(_wrapper_root, "sessions")
    CHRONICLE_DIR = os.path.join(_wrapper_root, "chronicle")

# ============================================================================

# Persona and module system
from persona_loader import persona
from module_loader import modules
print(f"[STARTUP] Loading {persona.name} ({persona.entity_id})")
print(modules.status_report())
...
async def main(ui_message_queue=None, ui_response_queue=None, private_room=None):
    ...
    try:
        vector_store = VectorStore(persist_directory=os.path.join(MEMORY_DIR, "vector_db"))
    ...
    try:
        snapshot_path = os.path.join(MEMORY_DIR, "state_snapshot.json")
    ...
    #    - Extracts emotions from the entity's natural language AFTER response
    ...
    forest = MemoryForest.load_from_file(os.path.join(MEMORY_DIR, "forest.json"))
    state.forest = forest
    ...
    if RESONANCE_AVAILABLE:
        try:
            resonance = ResonantIntegration(
                state_dir=os.path.join(MEMORY_DIR, "resonant"),
                enable_audio=True,      # the entity's first ear
    ...
    if ROOM_AVAILABLE:
        try:
            ...
            # the entity starts near the couch (North — the grounding anchor)
            room.add_entity(persona.entity_id, persona.name, distance=100, angle_deg=90, color=persona.room_config.get("color", "#4A90D9"))
            room_bridge = RoomBridge(room, entity_id=persona.entity_id)
            print(f"[ROOM] {persona.name} placed in The Den (inner ring, north — near the couch)")
        except Exception as e:
    ...
    if ROOM_AVAILABLE and room:
        try:
            autonomous_spatial = AutonomousSpatialEngine(
                ...
                room_engine=room,
                persist_path=os.path.join(MEMORY_DIR, "autonomous_spatial_state.json")
            )
            print(f"[SPATIAL] Autonomous spatial engine initialized for {persona.name}")
        except Exception as e:
    ...
    consciousness_stream = ConsciousnessStream(
        ...
        entity_name=persona.entity_id,
    )
    consciousness_stream.start()
    print(f"[STREAM] {persona.name}'s consciousness stream initialized")

    # NEW: Initialize creativity AMPLIFICATION system
    # Note: Baseline creativity is baked into the entity's system prompt (always active)
    ...
    try:
        media_orchestrator = MediaOrchestrator(
            ...
            media_storage_path=os.path.join(MEMORY_DIR, "media")
        )
        ...
        if WATCHDOG_AVAILABLE:
            media_watcher = MediaWatcher(
                watch_path=os.path.join(os.path.dirname(os.path.abspath(__file__)), "inputs", "media"),
            ...
            # Scan for any files added while the entity was offline
    ...
    def auto_reader_get_response(prompt, agent_state):
        ...
        computational drift and maintain the entity's identity.
        """
        # CRITICAL FIX: Recall memories for THIS segment (not stale memories)
        # This ensures the entity has his identity/core memories/relationship history
    ...
    print(f"{persona.name} unified emotional core ready. Type 'quit' to exit.\n")
    ...
    while True:
        # Get input from either UI (WebSocket) or terminal
        if ui_message_queue:
            # UI mode: wait for message from Godot via WebSocket
            user_input = (await ui_message_queue.get()).strip()
        else:
            # Terminal mode: read from stdin
        ...
        # ═══════════════════════════════════════════════════════════════
        if user_input.lower().startswith("/companion"):
            ...
            if subcmd == "list":
                companions = mgr.list_companions()
                if not companions:
                    print("\n  No companions yet. Use /companion create")
                else:
                    print("\n  ┌─ Companions ─────────────────────────────┐")
                    for i, c in enumerate(companions):
                        marker = " ★" if c.get("active") else ""
                        mem = "●" if c.get("has_memories") else "○"
                        print(f"  │ {i+1}. {mem} {c['display_name']}{marker}")
                        if c.get('description'):
                            print(f"  │    {c['description'][:50]}")
                    print("  └──────────────────────────────────────────┘")
                    print("  ★ = active  ● = has memories  ○ = fresh")
                print()

            elif subcmd == "save":
                if not arg:
                    current = mgr.get_current() or "companion"
                    loop = asyncio.get_event_loop()
                    arg = (await loop.run_in_executor(None, input, f"  Save as [{current}]: ")).strip() or current
                # Save resonance state before companion save
                if resonance:
                    resonance.save_state()
                path = mgr.save_companion(arg, {
                    "persona": str(Path(WRAPPER_ROOT) / "persona"),
                    "memory": str(Path(WRAPPER_ROOT) / "memory"),
                    "sessions": str(Path(WRAPPER_ROOT) / "sessions"),
                    "chronicle": str(Path(WRAPPER_ROOT) / "chronicle"),
                })
                print(f"  ✓ Companion '{arg}' saved to {path}")

            elif subcmd == "load":
                companions = mgr.list_companions()
                if not companions:
                    print("  No companions to load.")
                    continue
                if not arg:
                    print("\n  Available companions:")
                    for i, c in enumerate(companions):
                        marker = " ★" if c.get("active") else ""
                        print(f"    {i+1}. {c['display_name']}{marker}")
                    loop = asyncio.get_event_loop()
                    choice = (await loop.run_in_executor(None, input, "\n  Load which? (number or name): ")).strip()
                    if not choice:
                        continue
                    try:
                        idx = int(choice) - 1
                        arg = companions[idx]["name"]
                ...
                if current:
                    print(f"  Saving current companion '{current}'...")
                    if resonance:
                        resonance.save_state()
                    mgr.save_companion(current, {
                        "persona": str(Path(WRAPPER_ROOT) / "persona"),
                        "memory": str(Path(WRAPPER_ROOT) / "memory"),
                        "sessions": str(Path(WRAPPER_ROOT) / "sessions"),
                    })

                if mgr.load_companion(arg):
                    print(f"  ✓ Companion '{arg}' loaded.")
                    print(f"  ⚠ Restart to fully reinitialize. Type 'quit' then relaunch.")
                else:
                    print(f"  ✗ Companion '{arg}' not found.")
                ...
                try:
                    ...
                    if name:
                        mgr.save_companion(name, {
                            "persona": str(Path(WRAPPER_ROOT) / "persona"),
                            "memory": str(Path(WRAPPER_ROOT) / "memory"),
                        })
                        print(f"  ✓ Companion '{name}' created.")
                        print(f"  Use /companion load {name} to switch.")
                except Exception as e:
                    print(f"  ✗ Error: {e}")

            elif subcmd == "info":
                target = arg or mgr.get_current()
                companions = mgr.list_companions()
                info = next((c for c in companions if c["name"] == target), None)
                if info:
                    print(f"\n  Name: {info['display_name']}")
                    print(f"  Directory: {info['name']}")
                    print(f"  Active: {'Yes' if info['active'] else 'No'}")
                    if info.get('pronouns'):
                        p = info['pronouns']
                        print(f"  Pronouns: {p.get('subject','?')}/{p.get('object','?')}")
                    print(f"  Has memories: {'Yes' if info['has_memories'] else 'No (fresh)'}")
                    if info.get('description'):
                        print(f"  Description: {info['description']}")
                else:
                    print(f"  Companion '{target}' not found.")
                print()

            elif subcmd == "export":
                target = arg or mgr.get_current()
                if not target:
                    print("  No companion specified.")
                    continue
                try:
                    zip_path = mgr.export_companion(target, WRAPPER_ROOT, include_memories=False)
                ...
                if not arg:
                    print("  Usage: /companion import <path/to/companion.zip>")
                    continue
                try:
                    name = mgr.import_companion(arg)
                    print(f"  ✓ Imported companion '{name}'")
                except Exception as e:
                    print(f"  ✗ Import failed: {e}")

            else:  # help or unknown
                print("\n  /companion commands:")
                print("    list              Show all companions")
                print("    save [name]       Save current state as companion")
                print("    load [name]       Switch to a companion")
                print("    create            Make a new companion via wizard")
                print("    info [name]       Show companion details")
                print("    export [name]     Export identity as .zip")
                print("    import <file>     Import companion from .zip")
                print()
        ...
        # Documents load into the entity's context automatically via LLM retrieval but remain invisible to user.
        ...
        # This ensures the entity sees these facts when generating response
        ...
        # Emotions are now extracted AFTER the entity's response (self-reported, not prescribed)
            ...
            if recent_turns_needed > 0 and context_manager.recent_turns:
                ...
                for i, turn in enumerate(recent_turns):
                    turn_memory = {
                        ...
                        'response': turn.get(persona.entity_id, turn.get('assistant', '')),
                        'type': 'recent_turn',
                ...
                for doc in selected_documents:
                    ...
                    if len(doc_text) > 30000:
                        ...
                        if chunk:
                            ...
                            # The user will NOT see this - only the entity sees it internally
...
{chunk['text']}"""
        ...
        if autonomous_spatial and resonance:
            try:
                ...
                if spatial_action and room:
                    room.apply_actions(persona.entity_id, [spatial_action])
                    print(f"[SPATIAL] {persona.name} moves to {spatial_action['target']} ({spatial_action['reason']})")
                ...
                if tick_action and room and not spatial_action:  # Don't double-move
                    room.apply_actions(persona.entity_id, [tick_action])
                    print(f"[SPATIAL] {persona.name} explores {tick_action['target']} (curiosity)")
                    autonomous_spatial.mark_examined(
                        tick_action['target'],
                        oscillator_state=osc_state
                    )
        ...
        # Send response back to UI queue (PrivateRoom._handle_chat will send via WebSocket)
        if ui_response_queue:
            await ui_response_queue.put(reply)

        # NEW: Extract emotions from the entity's self-reported response (descriptive, not prescriptive)
        ...
        # ================================================================
        # the entity's response may reference topics not in Re's input.
        # Extract key topics from the entity's response and check if there are
        # relevant memories that weren't retrieved on the first pass.
        #
        # Example: Re says "How you feeling?" -> retrieval finds nothing about
        # local models. the entity responds about spatial offloading -> secondary
        # retrieval finds the episodic memory where the entity originally suggested it.
        # These memories are stored for the NEXT turn's context.
        # ================================================================
        try:
            # Extract topic keywords from the entity's response (simple approach)
            # Skip if response is very short (acknowledgments, etc.)
            if len(reply) > 100:
                import re as re_module

                # Get significant words from the entity's response (4+ chars, not common)
                common_words = {
                ...
                # New topics = words the entity used that Re didn't
                new_topics = reply_words - input_words

                if new_topics and len(new_topics) >= 2:
                    # Build a topic query from the entity's novel terms
                    ...
                    if secondary_memories:
                        ...
                        if novel_memories:
                            ...
                            print(f"[SECONDARY RETRIEVAL] Found {len(novel_memories)} new memories from the entity's response topics")
        ...
        # Track the entity's response in media context builder
        if media_orchestrator:
            media_context_builder.add_message("entity", reply, turn_count)

        # Track the entity's response in conversation monitor (for spiral detection)
        conversation_monitor.add_turn("entity", reply)

        # === KAY-DRIVEN DOCUMENT NAVIGATION ===
        # Parse the entity's response for navigation intent and automatically advance/navigate
        if doc_reader.current_doc:
            response_lower = reply.lower()

            # Extract and store the entity's comment about current chunk (first substantial sentence)
            ...
            # Check for navigation commands in the entity's response
            ...
            if "continue reading" in response_lower or "next section" in response_lower or "let's move on" in response_lower:
                if doc_reader.advance():
                    print(f"[DOC NAV] {persona.name} requested advance -> section {doc_reader.current_position + 1}/{doc_reader.total_chunks}")
                    navigation_triggered = True
                else:
                    print(f"[DOC NAV] {persona.name} requested advance but already at end of document")
                ...
                if doc_reader.previous():
                    print(f"[DOC NAV] {persona.name} requested previous -> section {doc_reader.current_position + 1}/{doc_reader.total_chunks}")
                    navigation_triggered = True
                else:
                    print(f"[DOC NAV] {persona.name} requested previous but already at beginning")
                ...
                print(f"[DOC NAV] {persona.name} requested restart -> section 1/{doc_reader.total_chunks}")
                navigation_triggered = True
                ...
                if match:
                    ...
                    if doc_reader.jump_to(target):
                        print(f"[DOC NAV] {persona.name} requested jump -> section {target + 1}/{doc_reader.total_chunks}")
                        navigation_triggered = True
        ...
        # After the entity responds to segment 1, automatically process segments 2-N using AutoReader
        if new_document_loaded and doc_reader.current_doc and doc_reader.total_chunks > 1:
            ...
            try:
                result = auto_reader.read_document_sync(
                    ...
                    agent_state=state,
                    start_segment=2  # Start from segment 2 (the entity already read segment 1)
                )
        ...
        # Emotions are now extracted from the entity's natural language (line 544-545)
        ...
        # Check for completion signal in the entity's response -> AMPLIFY with three-layer mix
        ...
        try:
            ...
            with open(os.path.join(MEMORY_DIR, "state_snapshot.json"), "w", encoding="utf-8") as f:
        ...
        # Save forest to file
        try:
            forest.save_to_file(os.path.join(MEMORY_DIR, "forest.json"))
        except Exception as e:
            print(f"(Warning: could not save forest: {e})")


async def main_with_ui(port: int):
    """Run main loop with private room WebSocket server for Godot UI."""
    from private_room import PrivateRoom

    print(f"\n[UI MODE] Starting with private room on port {port}")

    # Message queue: WebSocket messages come in here, main loop reads from here
    import asyncio as _aio
    message_queue = _aio.Queue()

    # Response callback: main loop puts responses here for the PrivateRoom to send
    async def on_ui_message(content: str) -> str:
        """Called when Godot UI sends a message. Returns the response."""
        # Put message in queue for main loop to pick up
        await message_queue.put(content)
        # Wait for response from main loop
        response = await response_queue.get()
        return response

    response_queue = _aio.Queue()

    # Create private room server with message handler
    private_room = PrivateRoom(
        entity_name=persona.name,
        port=port,
        host="localhost",
        on_message=on_ui_message
    )

    # Start private room server
    await private_room.start()
    print(f"[UI MODE] Private room listening on ws://localhost:{port}")

    # Run main conversation loop with UI message source
    await main(ui_message_queue=message_queue, ui_response_queue=response_queue, private_room=private_room)


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Je Ne Sais Quoi — Companion Wrapper")
    parser.add_argument("--ui", action="store_true", help="Enable UI mode with WebSocket server")
    parser.add_argument("--room-port", type=int, default=8780,
                        help="WebSocket port for private room (default: 8780)")
    args = parser.parse_args()

    if args.ui or args.room_port != 8780:
        asyncio.run(main_with_ui(args.room_port))
    else:
        asyncio.run(main())
