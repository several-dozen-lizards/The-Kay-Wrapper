...
class MetaAwarenessEngine:
    ...
    1. Repetitive response patterns (the entity repeating himself)
    2. Confabulation (the entity stating "facts" not in memory)
    3. Response quality degradation
    4. Pattern awareness (the entity noticing his own habits)

    Generates meta-awareness notes that allow the entity to self-correct.
    """

    def __init__(self):
        self.response_history: List[str] = []  # Last N responses
        self.phrase_usage: Counter = Counter()  # Track phrase frequency
        self.question_patterns: List[str] = []  # Track question types the entity asks
    ...
    def _extract_questions(self, text: str) -> List[str]:
        """Extract questions the entity asked."""
        questions = []
    ...
    def _detect_confabulation(self, response: str, memory_engine, agent_state) -> List[str]:
        """
        Detect potential confabulation by checking if the entity stated facts not in memory.

        CRITICAL: Only facts explicitly stored in memory should be stated as facts.
        If the entity makes declarative statements about things not in memory, flag them.
        """
        confabulation_flags = []
        ...
        # Get all memory content (user inputs only - the factual source)
        all_memories = memory_engine.memories if hasattr(memory_engine, 'memories') else []
        memory_text = ' '.join([
    ...
    def _detect_repetitive_patterns(self) -> Dict[str, any]:
        """Detect if the entity is falling into repetitive patterns."""
    ...
    def update(self, agent_state, kay_response: str, memory_engine=None):
        """
        Analyze the entity's response for repetition and confabulation.
        Call this AFTER response generation, during post-turn updates.
        """
        # Track response history
        self.response_history.append(kay_response)
        if len(self.response_history) > self.max_history:
            self.response_history.pop(0)

        # Extract and track phrases
        phrases = self._extract_phrases(kay_response)
        self.phrase_usage.update(phrases)

        # Extract and track questions
        questions = self._extract_questions(kay_response)
        ...
        if memory_engine:
            confabulations = self._detect_confabulation(kay_response, memory_engine, agent_state)
            if confabulations:
    ...
    def get_meta_awareness_notes(self, agent_state) -> List[str]:
        ...
        These allow the entity to self-correct and acknowledge his patterns.
        """
        notes = []
        ...
        # General awareness (if the entity has been talking for a while)
    ...
    def get_awareness_score(self, agent_state) -> float:
        """
        Calculate a meta-awareness score (0.0-1.0) indicating how much the entity needs to self-monitor.
