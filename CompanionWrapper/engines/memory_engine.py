...
class MemoryEngine:
    ...
    def __init__(self, semantic_memory: Optional[Any] = None, file_path: str = None, motif_engine: Optional[Any] = None, momentum_engine: Optional[Any] = None, emotion_engine: Optional[Any] = None, vector_store: Optional[Any] = None):
        ...
        if file_path is None:
            ...
            if state_dir:
                file_path = os.path.join(state_dir, "memory", "memories.json")
            else:
...
FROM KAY'S RESPONSE:
- "your/you" in the entity's response = about Re (USER perspective, NOT the entity)
...
- When the entity mentions entities that are known to belong to Re, DO NOT create entity ownership
- Conversational references to Re's life = Re's facts, not the entity's facts

CRITICAL: When the entity says things like "your cats", "your dog", "you have", extract these as REINFORCING Re's ownership, NOT creating entity ownership.
...
the entity has a camera and can see Re. When the entity describes what he SEES, these are transient observations, NOT permanent facts:
...
EXAMPLE 2 - the entity makes conversational reference to Re's pets:
...
Note: the entity says "Your cats" - this is about Re's cats, NOT the entity's. Do NOT create "the entity owns X".
...
EXAMPLE 4 - the entity confused but describing Re's entities (DO NOT EXTRACT):
...
Note: Even though the entity says "my cats", the context shows these are Re's cats (user just stated it).
...
Extract facts now:"""
            ...
            for fact in facts:
                if isinstance(fact, dict) and "fact" in fact:
                    ...
                    if source_speaker == "entity" and perspective == "user":
                        # the entity is making a claim about the user (Re)
            ...
            if entity1 and relation_type and entity2:
                ...
                if relation_type == "owns":
                    ...
                    if speaker == "entity":
                        ...
                        # Check if this CORRECTS a previous the entity confusion
                        conflict_check = self.entity_graph.check_ownership_conflict(
                            entity=entity2,
                            claimed_owner=entity1,
                            identity_memory=self.identity
    ...
    def _extract_entity(self, fact_text: str) -> str:
        ...
        if any(pattern in fact_lower for pattern in ["i ", "my ", "i'm", "i am"]):
            # First person - depends on context, but often the entity speaking
            return "entity"
    ...
    def _check_contradiction(self, new_fact: str, retrieved_memories: List[Dict]) -> bool:
        ...
        1. Only compare SAME entity (the entity comparison, Re vs Re)
    ...
    def encode_memory(self, user_input, response, emotional_cocktail, emotion_tags, perspective=None, agent_state=None, connection_data=None):
        ...
        osc_state = getattr(agent_state, 'oscillator_state', None) if agent_state else None
        ...
        try:
            ...
            # Fallback if this version's MemoryLayerManager doesn't accept osc_state
    ...
    def retrieve_unified_importance(self, bias_cocktail, user_input, max_memories: int = 250, conversational_mode: bool = False) -> List[Dict[str, Any]]:
        ...
        if hasattr(self, 'memory_layers'):
            ...
            if conversational_mode:
                ...
                # Normal mode: use vector_store to narrow FIRST if available
                # BUGFIX: Was scoring ALL long-term memories (5400+) which caused 30s latency
                # Now: narrow to top 100 candidates via vector search, then score
                narrowed_memories = []

                if self.vector_store and user_input:
                    try:
                        # Phase 1: Fast semantic vector search (~100ms)
                        # Note: CompanionWrapper uses vector_store for both docs and memories
                        semantic_results = self.vector_store.query(
                            query_text=user_input,
                            n_results=100  # Narrow to top 100 candidates
                        )
                        ...
                        if semantic_results:
                            ...
                            if VERBOSE_DEBUG:
                                print(f"[UNIFIED MEMORY] vector_store narrowed: {len(narrowed_memories)} candidates")
                    except Exception as e:
                        if VERBOSE_DEBUG:
                            print(f"[UNIFIED MEMORY] vector_store narrowing failed: {e}")

                if narrowed_memories:
                    all_other.extend(narrowed_memories)
                else:
                    # Fallback: no vector_store or empty results - cap at 200 for safety
                    # This prevents 30s latency from scoring 5400+ memories
                    longterm_pool = self.memory_layers.long_term_memory
                    if len(longterm_pool) > 200:
                        print(f"[UNIFIED MEMORY] WARNING: Capping {len(longterm_pool)} memories to 200 (no vector narrowing)")
                    all_other.extend(longterm_pool[-200:])
        ...
        for mem in all_other:
            ...
            # "I am the entity AND I am everything" — boundaries dissolve, not identity
...
Most relevant memory numbers:"""
    ...
    def recall(self, agent_state, user_input, bias_cocktail=None, num_memories=30, use_multi_factor=True, include_rag=True, conversational_mode=False, osc_state=None):
        ...
        diversity_slots = RETRIEVAL_CONFIG.get("diversity_slots", 1)
    ...
    def store_visual_memory(
        ...
        memory_entry = {
            ...
            'perspective': 'shared',  # Visual shared between Re and the entity
