...
class ContextManager:
    ...
    def build_context(self, agent_state, user_input) -> Dict[str, Any]:
        ...
        # ===== PHASE 4: Build conversation history with compression =====
        # Last 5 turns stay raw for immediate context, older turns get compressed
        raw_recent = 5
        compressed_history = ""
        ...
        if self.recent_turns:
            try:
                ...
                history_budget = limits.get('history_budget', 3000)
                ...
                print(f"[CONTEXT] Conversation history: {len(self.recent_turns)} turns compressed "
                      f"(last {raw_recent} raw, budget {history_budget})")
            except Exception as e:
                print(f"[CONTEXT] Compression failed, using fallback: {e}")
                compressed_history = ""

        # Keep last N turns for working memory measurement and backwards compatibility
        ...
        if len(self.recent_turns) > limits['working_turns']:
            print(f"[CONTEXT] Working memory fallback: {len(working_turns)} turns "
        ...
        context = {
            "recent_context": working_turns,  # Raw recent turns for backwards compatibility
            "compressed_history": compressed_history,  # Token-budget-aware compressed conversation
