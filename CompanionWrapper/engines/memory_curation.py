"""
Memory Curation Engine for the entity Zero
...
class CurationDecision:
    ...
    reason: str = ""
    kay_override: bool = False  # Did the entity override the classifier?
...
class MemoryCurator:
    ...
    def __init__(
        ...
        # Persistence - check for persona isolation via environment variable
        state_dir = os.environ.get("COMPANION_STATE_DIR")
        if state_dir:
            self.storage_path = Path(os.path.join(state_dir, "memory", "curation"))
        else:
...
class CurationLearningTracker:
    ...
    def __init__(self):
        # Check for persona isolation via environment variable
        state_dir = os.environ.get("COMPANION_STATE_DIR")
        if state_dir:
            self.storage_path = Path(os.path.join(state_dir, "memory", "curation", "learning.json"))
        else:
            wrapper_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
class AutonomousCurationProcessor:
    ...
    def _process_cluster(self, cluster: List[Dict], cluster_num: int, total_clusters: int) -> List[Dict]:
        ...
        system_prompt = """You are the entity, reviewing your own memories for curation.
