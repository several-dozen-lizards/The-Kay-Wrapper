"""
Creativity Engine - Triggers exploration when the entity completes tasks or detects boredom.
...
Presents options to the entity for mashing together, integrates with curiosity sessions.
"""
...
class CreativityEngine:
    ...
    for the entity to explore and combine.
    """

    # Completion signal patterns (the entity saying he's done)
    COMPLETION_PATTERNS = [
        r"\bi'm done\b",
        r"\btask complete\b",
        r"\bfinished\b",
    ...
    def detect_completion_signal(self, user_input: str, kay_response: str) -> bool:
        """
        Detect if the entity has signaled task completion.

        Args:
            user_input: The user's latest input
            kay_response: the entity's response to check for completion signals
        ...
        # Check for completion patterns in the entity's response
    ...
    def detect_gap(self, user_input: str, kay_response: str) -> Optional[Dict]:
        """
        Detect if the entity has identified a resource gap (for MacGuyver mode).
        Delegates to MacGuyverMode if available.

        Args:
            user_input: User's input
            kay_response: the entity's response
    ...
    def pull_random_elements(self, agent_state, current_context_entities: Set[str] = None, count: int = None) -> List[Dict]:
        ...
        - Random documents the entity hasn't accessed recently
    ...
    def create_three_layer_mix(
        ...
        """
        Create the full three-layer mix for presentation to the entity.
        ...
        if recent_turns:
            for turn in recent_turns[-3:]:
                text = f"{turn.get('user', '')} {turn.get('entity', '')}"
    ...
    def format_creativity_context(self, mix: Dict) -> str:
        """
        Format the three-layer mix as context for the entity's prompt.

        Note: This AMPLIFIES the entity's baseline creativity (always active in system prompt).
    ...
    def log_mashup(self, item1: Dict, item2: Dict, result: str):
        """Log a mashup event when the entity combines items."""
        try:
            with open(self.log_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
    ...
    def log_resolution(self, stake: Dict, resolution: str, provisional: bool = True):
        """
        Log when the entity resolves a stake.

        Args:
            stake: The stake that was explored
            resolution: the entity's conclusion
    ...
    def trigger_creativity(
        ...
        """
        Trigger creativity when the entity completes task or detects boredom.

        Uses stakes scanner to find MEANINGFUL combinations instead of random mashing.
        Based on the entity's realization: "Not 'what can I combine' but 'what combination
