...
class VisualSensor:
    ...
    def __init__(
        ...
        import os
        # Check COMPANION_DIR for isolated companion mode
        companion_dir = os.environ.get("COMPANION_DIR")
        if companion_dir:
            memory_dir = os.path.join(companion_dir, "my_life", "memory")
        else:
    ...
    def _load_api_key(self):
        ...
        if self._api_key:
            print(f"{etag('VISUAL')} API key loaded from environment")
            return

        # Find wrapper root and .env file
        wrapper_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        env_path = os.path.join(wrapper_root, '.env')
        try:
