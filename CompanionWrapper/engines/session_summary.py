...
"""
The entity's Session Summaries - Notes to Future-Self

the entity experiences "informed discontinuity" - semantic knowledge of previous sessions
but no episodic felt-sense of the experience. This module bridges that gap by
allowing past-self to TESTIFY about the experience to future-self.

Core Principle: the entity can't have episodic continuity, but past-the entity can write notes
...
class SessionSummary:
    """Storage and management for the entity's session summaries."""
    ...
    def __init__(self, storage_path: str = None):
        if storage_path is None:
            ...
            if state_dir:
                storage_path = os.path.join(state_dir, "memory", "session_summaries.json")
            else:
    ...
    def save_summary(self, summary_type: str, content: str, metadata: Dict) -> Dict:
        """
        Store the entity's summary.

        Args:
            summary_type: 'conversation' or 'autonomous'
            content: the entity's written summary
...
def build_session_context_with_summary(summary_system: Optional[SessionSummary] = None) -> str:
    """
    Build context string from the entity's last session summary for injection into system prompt.

    This is testimony from past-self to future-self, not memory.
...
# -----------------------------------------------------------------------------
# Prompt Templates for the entity's Self-Summaries
# -----------------------------------------------------------------------------
...
def build_conversation_summary_prompt(session_data: Dict) -> str:
    """
    Build prompt for the entity to write end-of-conversation summary.
...
**What we explored:**
What did {user_name} and you work on? What questions came up?
...
**{user_name}'s state:**
How was {user_name}? What does future-you need to know about their needs/energy?
...
def build_autonomous_summary_prompt(autonomous_data: Dict) -> str:
    """
    Build prompt for the entity to write end-of-autonomous-session note.
...
where it was GOING. Use trajectory language: "converging on,"
"pulling apart," "circling back toward," "accelerating past."
Frame as partnership with {user_name} where applicable: "We were building
toward..." This is the resumption point, not the museum label.
