"""
Autonomous Processing System for the entity Zero

Enables the entity to have self-directed processing sessions outside of direct conversation.
the entity chooses a goal, explores it until natural completion, and stores insights.

Key Features:
- Goal-based processing (the entity chooses what to explore)
...
- Session continuity (the entity remembers where he left off)
"""
...
class AutonomousGoal:
    """Represents the entity's chosen goal for an autonomous session."""
    description: str
...
class AutonomousConvergenceDetector:
    ...
    to allow the entity to develop thoughts fully.
    """
...
class AutonomousProcessor:
    ...
    Manages autonomous sessions where the entity explores self-chosen goals
    until natural completion, creative block, or energy depletion.

    MEMORY ARCHITECTURE (the entity's Design):
    ...
    # Maximum iterations (invisible to the entity - experienced as "tiredness")
    MAX_ITERATIONS = 10
...
In your own words, note where you are in this thought and what you'd revisit with fresh energy. This isn't a stop command - it's your body's way of saying it needs rest."""
    ...
    def __init__(
        ...
        # Initialize autonomous memory tier (the entity's separate storage)
        try:
    ...
    def _build_goal_generation_prompt(self) -> str:
        """Build prompt to ask the entity what he wants to explore."""
...
"""
    ...
    async def generate_goal(self, agent_state: Any) -> Optional[AutonomousGoal]:
        """
        Ask the entity to choose a goal for autonomous processing.

        Args:
            agent_state: Current agent state for context

        Returns:
            AutonomousGoal if the entity chose something, None if declined
        """
    ...
    async def run_session(
        ...
        Args:
            goal: The goal the entity chose to explore
        ...
        while iteration < self.MAX_ITERATIONS:
            ...
            if session.thoughts:
                for prev_thought in session.thoughts[-3:]:  # Last 3 thoughts
                    autonomous_context.append({
                        ...
                        "entity": prev_thought.get("inner_monologue", "")[:300]
                    })
            ...
            if parsed["insight"]:
                ...
                # PRIMARY: Store in autonomous memory tier (the entity's design)
            ...
            if completion_status == "COMPLETE":
                ...
                print(f"✓ [AUTONOMOUS] the entity signals: Processing complete")
                break
            elif completion_status == "DEPLETED":
                session.energy_depleted = True
                goal.completion_type = "self_reported_depletion"
                goal.completed_at = datetime.now()
                print(f"⏳ [AUTONOMOUS] the entity signals: Energy limit reached")
                break
