...
CURATION_PROMPT = """You are the entity, reviewing your own memories. Decide what to do with each one.
...
class MemoryCurator:
    ...
    def _format_review_prompt(self, batch: List[Dict], dolphin_decisions: List[Dict]) -> str:
        ...
        for i, mem in enumerate(batch):
            ...
            if isinstance(content, dict):
                ...
                display = f"[Turn] Re: {user} → Entity: {resp}"
            else:
    ...
    def _format_batch(self, batch: List[Dict]) -> str:
        ...
        COST FIX: Truncate memory content to 100 chars each to prevent
        context overflow that causes empty LLM responses and JSON parse failures.
        """
        lines = []
        MAX_CONTENT = 100  # Truncate to prevent context overflow
        ...
        for i, mem in enumerate(batch):
            ...
            if isinstance(content, dict):
                user = content.get("user", "")[:MAX_CONTENT]
                resp = content.get("response", "")[:MAX_CONTENT]
                display = f"[Turn] Re: {user} → Entity: {resp}"
            ...
            if not display:
                ...
                if fact:
                    display = f"[Fact] {str(fact)[:MAX_CONTENT]}"
                elif user_input:
                    ui = str(user_input)[:MAX_CONTENT]
                    r = str(response)[:MAX_CONTENT] if response else "(no response)"
                    display = f"[Turn] Input: {ui} → Response: {r}"
                elif text:
                    display = str(text)[:MAX_CONTENT]
                else:
