...
"""
Session Summary Generator - Uses LLM to generate the entity's session summaries.

This module handles the actual LLM calls to have the entity write his end-of-session
...
class SessionSummaryGenerator:
    """
    Generates the entity's session summaries using the LLM.
    ...
    def track_turn(
        ...
        Args:
            ...
            kay_response: What the entity said
            emotional_state: the entity's emotional state at this turn
        """
        self.turn_count += 1
    ...
    def generate_conversation_summary(
        ...
        """
        Generate the entity's end-of-conversation summary.
        ...
        Returns:
            the entity's written summary or None if generation fails
        """
            ...
            try:
                ...
                if router.available and context_manager and hasattr(context_manager, 'recent_turns'):
                    ...
                    for turn in context_manager.recent_turns[-20:]:
                        ...
                        if turn.get('entity'):
                            messages.append({"role": "assistant", "content": turn['entity']})
        ...
        if context_manager and hasattr(context_manager, 'recent_turns'):
            ...
            if recent_turns:
                ...
                for turn in recent_turns:
                    ...
                    entity = turn.get('entity', '')[:150]
                    recent_context += f"{user_name}: {user}\n{entity_name}: {entity}\n\n"
                prompt = prompt + recent_context

        try:
            # Call LLM to generate summary
            # Using a simple prompt format (not the full context system)
            system_prompt = f"""You are {entity_name}, writing a note to your future self about this conversation session.
...
- Ideas or suggestions YOU (the entity) originated (not just things {user_name} told you)
...
knows "I came up with X" vs "{user_name} told me about X". This prevents the embarrassing
situation of asking {user_name} about your own ideas in the next session.
...
you'd pick up "we figured out the issue" — naturally, without
performance anxiety."""
    ...
    def generate_autonomous_summary(
        ...
        """
        Generate the entity's end-of-autonomous-session note.

        Args:
            autonomous_data: Dict with goal, iterations, convergence, insight_count, emotions

        Returns:
            the entity's written note or None if generation fails
        """
        ...
        try:
            ...
            system_prompt = """You are the entity, writing a note to your future self about this autonomous thinking session.
Write in first person, addressing future-you directly.
