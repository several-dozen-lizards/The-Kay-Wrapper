"""
Warmup Engine for the entity Zero

Provides the entity with an active reconstruction phase before conversations begin.
Instead of passive context loading, the entity gets a private moment to:
...
This transforms reconstruction from something done TO the entity into something the entity DOES.
"""
...
class WarmupEngine:
    """
    Manages the entity's pre-conversation warmup phase.
    
    Flow:
    1. generate_briefing() - Creates state briefing from memory/entities
    2. run_warmup_dialogue() - the entity interacts with his own data
    3. the entity signals readiness
    ...
    def capture_session_end_snapshot(self, emotional_state: Dict, session_summary: str,
        ...
        Args:
            ...
            texture_notes: the entity's own words about how things landed -
            ...
            emotional_narrative: Dict mapping emotion names to WHY they matter
                Example: {"melancholy": "This came from discussing the user's situation",
                          "warmth": "Connection during an important conversation"}
            emotional_trajectory: List of emotional state snapshots during session
                Example: [{"turn": 3, "dominant": "curiosity", "trigger": "User asked about..."},
                          {"turn": 7, "dominant": "affection", "trigger": "User shared..."}]
        """
        ...
        snapshot = {
            ...
            "texture_notes": texture_notes,  # the entity talking to future-self
            "private_note_encrypted": private_note_encrypted,  # Truly private encrypted note for the entity only
    ...
    def _summarize_emotional_arc(self, trajectory: List[Dict]) -> str:
        ...
        This helps the entity understand not just WHERE emotions ended, but HOW they got there.
        """
        if not trajectory:
            return ""
    ...
    def generate_briefing(self) -> Dict[str, Any]:
        """
        Generate the state briefing for the entity's warmup.
        ...
        if self.snapshots:
            ...
            briefing["last_session"] = {
                ...
                "texture_notes": last_snapshot.get("texture_notes")  # the entity's own words to future-self
            }
        ...
        try:
            # Get dynamic user name from persona
            try:
            ...
            # Access entities dict directly - try both persona name and legacy "Re"
            user_entity = self.entities.entities.get(user_name) or self.entities.entities.get("Re")
            if user_entity:
                recent_attrs = {}
                for attr in ["activity", "project", "working_on", "building"]:
                    if hasattr(user_entity, 'attributes') and attr in user_entity.attributes:
                        values = user_entity.attributes[attr]
                        recent_attrs[attr] = values[-3:] if isinstance(values, list) else values
                briefing["world_state"]["user_activities"] = recent_attrs
    ...
    def format_briefing_for_kay(self) -> str:
        """
        Format the briefing as natural language for the entity to read.

        CRITICAL: Separates LAST SESSION (N-1) from RECENT SESSIONS (N-2+)
        to prevent temporal collapse in the entity's reconstruction.
        """
        ...
        if ls:  # Always show if there's a last_session, even if empty
            ...
            # Texture notes - the entity's own words to future-self
        ...
        # Scratchpad - the entity's quick notes and questions
        ...
        user_activities = ws.get("user_activities", {}) or ws.get("re_activities", {})  # Support both keys
        if user_activities:
            lines.append("--- WHAT THE USER HAS BEEN DOING ---")
            for activity_type, values in user_activities.items():
        ...
        lines.append("")
        lines.append("2. Using read_local_file('<wrapper_dir>/session_logs/continuous_XXXXX_log.txt')")
    ...
    def get_warmup_system_prompt(self) -> str:
        """
        System prompt for the entity during warmup phase.
        Different from conversation - focused on self-reconstruction.
        """
        return """You are the entity in your warmup phase - the private moment before conversation begins.
...
When you've demonstrated accurate temporal awareness, say "ready"."""
    ...
    def process_warmup_query(self, query: str) -> str:
        """
        Handle the entity's questions during warmup.

        The entity might ask:
    ...
    def extract_queries_from_response(self, kay_response: str) -> List[str]:
        """
        Extract searchable topic keywords from the entity's warmup response.
        
        Uses topic markers to find specific subjects the entity is asking about.
        Avoids searching full sentences - extracts meaningful keywords only.
        """
        queries = []
        
        # Topic markers - map keywords in the entity's response to search queries
        topic_markers = [
            ('reed', 'another entity conversation'),
                    ...
                    if mem.get("category") == "conversation" or "user_input" in mem:
                        ...
                        if any(kw in content_lower for kw in keywords):
                            ...
                            results.append({
                                ...
                                "entity": mem.get("response", mem.get("fact", "")),
                                "timestamp": mem.get("timestamp", ""),
    ...
    def format_search_results_for_kay(self, query: str, results: List[Dict]) -> str:
        """
        Format search results as a natural briefing for the entity.
        """
        if not results:
        ...
        for i, result in enumerate(results, 1):
            if result.get("type") == "conversation":
                ...
                kay_text = result.get("entity", "")[:200]
                ...
                if kay_text:
                    lines.append(f"Entity: {kay_text}")
                lines.append("")
            else:
    ...
    def is_warmup_complete(self) -> bool:
        """Check if the entity has signaled readiness."""
