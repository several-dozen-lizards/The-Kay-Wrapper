...
Master timeline of the entity's sessions, curated by the entity.
Each session ends with the entity writing an essay about what mattered.
Next session starts with that essay as primary context.

Design principles:
- Chronological (timing is context)
- Layered (summary → detail on demand)
- the entity's voice (not algorithmic)
...
class SessionChronicle:
    """
    Manages the master timeline of the entity's sessions.
    
    Each chronicle entry contains:
    - the entity's essay (his words, his priorities)
    - Session metadata (time, duration, topics)
    - Links to scratchpad items created/resolved
    - Importance rating (the entity's own assessment)
    """
    ...
    def add_session_entry(
        ...
        Called at end of session after the entity writes his essay.
        ...
        Args:
            ...
            kay_essay: the entity's own summary of what mattered
    ...
    def get_timeline_summary(self, count: int = 10) -> str:
        ...
        - First line of the entity's essay
        - Key topics
...
def create_empty_chronicle(base_dir: Path) -> Path:
    ...
    Args:
        base_dir: Base directory for the entity's data
