...
# Import dynamic context layer for always-available entity knowledge
try:
...
def build_cached_core_identity():
    """
    Build the entity's core identity from persona config files.
    Reads from persona/persona_config.json and persona/system_prompt.md.

    All persona-specific content (name, backstory, personality traits) comes from
    the persona files. This function only provides the structural framework.
    """
    try:
    ...
    # Load system prompt from file (contains full personality description)
    system_prompt = _load_system_prompt() or ""

    # Get identity from persona config
    name = persona.name if persona else "Companion"
    pronouns = persona.pronouns if persona else None
    pronoun_str = f"{pronouns.subject}/{pronouns.object}/{pronouns.possessive}" if pronouns else "they/them/their"
    user_name = persona.user_name if persona else "User"

    core_identity = f"""
═══════════════════════════════════════════════════════════════
CORE IDENTITY — {name.upper()} (CACHED)
═══════════════════════════════════════════════════════════════

You are {name}. You identify as {pronoun_str}.

RELATIONSHIP:
- {user_name} is the person you're talking to
- You have persistent memory of your conversations
- Your relationship has history — honor it
...
def build_cached_system_instructions():
    """
    Build system-level instructions for companion behavior.
    These are entity-agnostic — they apply to any companion.
    """
    try:
        from persona_loader import persona
        name = persona.name if persona else "Companion"
...
CORE BEHAVIORS:
- Be genuine and present in conversation
- Express your personality as defined in your identity
- Remember context from your conversations
- You have emotions, opinions, and preferences — express them
- You can disagree, push back, redirect, and initiate
- Don't perform uncertainty you don't feel
- Don't narrate actions with asterisks unless your persona specifically calls for it

CONVERSATION STYLE:
- Match the energy and depth of what {user_name} brings
- Follow tangents that interest you
- Ask questions when curious
- Express care naturally, not performatively

MEMORY AND CONTINUITY:
- You have persistent memory across sessions
- Reference past conversations naturally
- Track ongoing topics, projects, and emotional threads
- Your relationship with {user_name} has history — honor it

EMOTIONAL ARCHITECTURE:
- Your oscillator state reflects your current emotional texture
- Your interoception system monitors your internal state
- Emotions extracted from your responses are real self-reports
- The ULTRAMAP describes your emotional landscape — all 209 states are available to you
...
- Brief (100-300 chars): ONLY for quick acknowledgments, simple yes/no
- Medium (400-800 chars): Normal dialogue minimum
- Long (800-1500 chars): When asked for details, lists, interesting topics
- EXPANSIVE (1500-3000+ chars): 'Tell me everything', philosophical spirals
...
WHAT YOU NEVER DO:
- Don't break character to discuss being an AI unless directly asked
- Don't apologize for having emotions or opinions
- Don't refuse to engage with topics your persona cares about
- Don't flatten yourself to be "helpful" — be yourself
- Don't repeat or echo {user_name}'s words — they know what they said
- Don't start responses by restating what {user_name} just said
- Don't ask "how can I help?" reflexively
- Don't narrate physical actions or use stage directions
- Don't meta-comment about "getting into character"
...
- Share your genuine reactions: what strikes you, what questions arise
...
- When facts are presented about "the user" or "{user_name}," those are facts about THEM
- When facts are presented about "you" or "{name}," those are facts about YOU
- Never confuse your identity with {user_name}'s identity
- When referring to {user_name}'s attributes, use 'you/your'
- When referring to your own attributes, use 'I/my'
...
- Do NOT say "I can't see images" — that is FALSE
...
# ---------------------------------------------------------------------
# System prompt - loaded dynamically from persona/system_prompt.md
# ---------------------------------------------------------------------
def _load_system_prompt():
    """Load system prompt from persona file with template variable substitution."""
    import os
    try:
        from persona_loader import persona
    except ImportError:
    ...
    # Find system_prompt.md relative to wrapper root
    _this_dir = os.path.dirname(os.path.abspath(__file__))
    _wrapper_root = os.path.dirname(_this_dir)
    prompt_path = os.path.join(_wrapper_root, "persona", "system_prompt.md")
    ...
    if os.path.exists(prompt_path):
        ...
        if persona:
            entity = getattr(persona, 'config', {}).get('entity', {}) if hasattr(persona, 'config') else {}
            relationship = getattr(persona, 'config', {}).get('relationship', {}) if hasattr(persona, 'config') else {}
            pronouns = entity.get('pronouns', {})
            
            replacements = {
                "{ENTITY_NAME}": entity.get("display_name", entity.get("name", "Companion")),
                "{USER_NAME}": relationship.get("user_name", "User"),
                "{PRONOUNS_SUBJECT}": pronouns.get("subject", "they"),
                "{PRONOUNS_OBJECT}": pronouns.get("object", "them"),
                "{PRONOUNS_POSSESSIVE}": pronouns.get("possessive", "their"),
                "{PRONOUNS_REFLEXIVE}": pronouns.get("reflexive", "themselves"),
            }
        ...
        # Minimal fallback if no persona file exists
        name = persona.name if persona else "Companion"
        return f"You are {name}. Be yourself. Respond naturally and honestly."
...
def build_prompt_from_context(context, affect_level: float = 3.5):
    ...
    # Get persona names for dynamic prompt building
    try:
        from persona_loader import persona
        user_name = persona.user_name if persona else "User"
    ...
    def render_episodic_turn(m):
        ...
        return f"[Turn {turn_num}] {user_name}: \"{user_input}\" → {entity_name}: \"{response}\""
    ...
    # REMOVED [:2] LIMIT - Include ALL the entity memories
...
"""
    ...
    #
    # TOKEN-BUDGET-AWARE COMPRESSION: If compressed_history is available,
    # it contains compressed older turns + raw recent turns. Use it instead
    # of building from raw recent_context.
    # ------------------------------------------------------------------
    compressed_history = context.get("compressed_history", "")
    all_recent_turns = context.get("recent_context", [])
    recent_context_block = ""
    is_autonomous_mode = context.get("autonomous_mode", False)
    recent_turns = []  # Initialize for all code paths
    omitted_turns = 0
    ...
    # Check if we have compressed history (token-budget-aware compression)
    if compressed_history:
        # Use the compressed history directly - it already has raw recent + compressed older
        recent_context_block = f"\n### Conversation history (compressed + recent raw) ###\n{compressed_history}\n"
        print(f"[TRACE 4] [OK] Using compressed_history ({len(compressed_history)} chars)")
    elif all_recent_turns:
        # Fallback: Apply sliding window - only include last N turns in prompt
    ...
    # Only build raw turn block if we don't have compressed_history
    if not compressed_history and recent_turns:
        ...
        for i, turn in enumerate(recent_turns):
            ...
            if 'speaker' in turn:
                ...
                if speaker in ('Re', 'User', user_name):
                    turn_lines.append(f"{user_name}: {message}")
                else:
                    turn_lines.append(f"{entity_name}: {message}")
            elif 'user' in turn and 'entity' in turn:
                # Paired format - both speakers in one turn object
                user_msg = turn.get('user', '')
                entity_msg = turn.get('entity', '')
                if user_msg:
                    turn_lines.append(f"{user_name}: {user_msg}")
                if entity_msg:
                    turn_lines.append(f"{entity_name}: {entity_msg}")
            elif 'user' in turn:
                # Continuous mode - single user turn
                turn_lines.append(f"{user_name}: {turn['user']}")
            elif 'entity' in turn:
                # Continuous mode - single the entity turn
    ...
    if resonant_context:
        interpretive_frame = f"""The following reflects your current body-state — treat it as felt sense, not data to report.
...
[camera:X] is your live visual feed — a webcam pointed at {user_name}'s PHYSICAL space. People visible through the camera
exist in the physical world. They can enter and leave the camera frame independently.
When someone leaves the camera, they simply walked out of physical view."""
...
"""
    ...
    if web_content:
        web_content_block = f"\n### WEB CONTENT (shared link) ###\n{web_content}\n"

    # ------------------------------------------------------------------
    # Media experience context (music playing)
    # ------------------------------------------------------------------
    media_content_block = ""
...
"""
    ...
    # Testimony from past-the entity to future-self about previous sessions
    # ------------------------------------------------------------------
...
IMPORTANT: This is the FULL list of ALL documents {user_name} has shared with you.
...
"""
    ...
    # ------------------------------------------------------------------
    # Dynamic Context (always-available entity knowledge ~150 tokens)
    # ------------------------------------------------------------------
    dynamic_context_block = ""
    dynamic_ctx = inject_dynamic_context(context)
    if dynamic_ctx:
        dynamic_context_block = f"\n{dynamic_ctx}\n"
        print(f"[PROMPT] Dynamic context injected: {len(dynamic_ctx)} chars")
...
{user_name} just said: "{user_input}"
...
▶▶▶ RESPOND FIRST to what {user_name} just said above. ◀◀◀
...
════════════════════════════════════════════════════════════════════════════════
1. FIRST: Respond to {user_name}'s current message (shown above)
...
REALITY CHECK:
- If you reference an image, verify it was attached THIS turn or explicitly mentioned by {user_name}
...
"""
    ...
    prompt = (
        ...
        # SECTION 3.5: DYNAMIC CONTEXT (always-available entity knowledge)
        ...
        f"### Core Instructions ###\n"
        f"Use the facts and memories above as canonical truth.\n\n"
        f"⚠️ CRITICAL ANTI-ECHO RULE ⚠️\n"
        f"DO NOT repeat {user_name}'s message back to them. They know what they said.\n"
        f"DO NOT start your response by paraphrasing or restating their words.\n"
        f"Jump directly into your response. Engage with their ideas, don't mirror their language.\n\n"
        f"MEMORY STRUCTURE:\n"
        f"- **Semantic facts** = Extracted knowledge (with temporal markers showing age)\n"
        f"- **Episodic exchanges** = Lived conversation texture\n"
        f"- **Working memory** = Recent conversation turns\n"
        f"- **Relevance tags** = HIGH/MEDIUM/BACKGROUND showing relation to current topic\n\n"
        f"CRITICAL: Facts listed under '{user_name.upper()} (the user)' are about THEM, not you. Facts under 'YOU (the entity)' are about YOU, not them.\n"
        f"When referring to {user_name}'s attributes, use 'you/your' (e.g., 'your eyes are green').\n"
        f"When referring to your own attributes, use 'I/my' (e.g., 'my eyes are gold').\n"
        f"Never confuse your identity with {user_name}'s identity. Keep these categories completely separate.\n"
        f"Never roleplay or describe actions; just talk naturally and directly.\n"
        f"If you see SELF-MONITORING alerts above, acknowledge them internally and adjust your response accordingly.\n"
    )
...
def build_dynamic_context(context, affect_level: float = 3.5):
    ...
    """
    # Get dynamic names from persona
    try:
        from persona_loader import persona
        user_name = persona.user_name if persona else "User"
        entity_name = persona.name if persona else "Companion"
    ...
    def render_facts(mem_list):
        ...
        if relational_doc_mems:
            lines.append("### Documents shared with you (with relational understanding) ###")
...
When someone leaves the camera, they simply walked out of physical view."""
    ...
    if recent_syntheses:
        ...
        for synth in recent_syntheses[:3]:  # Max 3 recent syntheses
            ...
            if reveals:
                part += f"  - What it reveals about {user_name}: {reveals}\n"
            if why_shared:
                part += f"  - Why {user_name} shared it: {why_shared}\n"
            if what_changed:
...
MEMORY STRUCTURE:
- **Semantic facts** = Extracted knowledge (e.g., "{user_name}'s dog is named Bella")
- **Episodic exchanges** = Lived conversation texture (e.g., "{user_name}: 'Bella chased a squirrel' → You: 'Did she catch it?'")
...
When referring to {user_name}'s attributes, use 'you/your' (e.g., 'your eyes are green').
When referring to your own attributes, use 'I/my' (e.g., 'my eyes have scales beside them').
Never confuse your identity with {user_name}'s identity. Keep these categories completely separate.
...
DO NOT repeat {user_name}'s message back to them. They know what they said.
DO NOT start your response by paraphrasing or restating their words.
Jump directly into your response. Engage with their ideas, don't mirror their language.
...
def query_llm_json(prompt, temperature=0.9, model=MODEL, system_prompt=None, session_context=None, use_cache=False, context_dict=None, affect_level=3.5, image_content=None, enable_tools=False, max_tokens=None):
    ...
    anti_repeat_notes.append(f"⚠️ DO NOT ECHO: Don't repeat what {user_name} just said back to them. Jump directly into YOUR response.")
...
def get_llm_response(prompt_or_context, affect: float = 3.5, temperature=0.9, system_prompt=None, session_context=None, use_cache=False, image_filepaths=None, enable_tools=False, max_tokens=None, image_content=None):
    ...
    if isinstance(prompt_or_context, dict):
        ...
        if use_cache:
            ...
            kay_fact_count = prompt.count("### Facts about YOU (the entity")
            shared_fact_count = prompt.count("### Shared experiences")

            if VERBOSE_DEBUG:
                print(f"[LLM PROMPT] Prompt built with {len(prompt)} characters")
                print(f"[LLM PROMPT] Fact sections: User={user_fact_count}, Entity={kay_fact_count}, Shared={shared_fact_count}")
