...
Used by:
  ...
  - nexus_entity.py (Nexus multi-entity mode)
...
class WrapperBridge:
    """
    Extracted core processing pipeline for the entity/Reed wrappers.

    COMPANION ISOLATION:
    When COMPANION_DIR env var is set, all state paths (memory/, sessions/, etc.)
    resolve to the companion's my_life/ directory instead of the wrapper root.
    ...
    def __init__(self, entity_name="Companion", wrapper_dir=None):
        """Initialize all engines and state."""
        self.entity_name = entity_name
        self.wrapper_dir = wrapper_dir or os.getcwd()

        # COMPANION ISOLATION: Check for companion directory
        self.companion_dir = os.environ.get("COMPANION_DIR")
        if self.companion_dir:
            print(f"{etag('JNSQ')} Using companion directory: {self.companion_dir}")

    def _state_path(self, *parts) -> str:
        """
        Resolve a state path, using companion directory if available.
        State paths include: memory/, sessions/, chronicle/, documents/

        Example: self._state_path("memory", "vector_db") returns:
            - "companions/{name}/my_life/memory/vector_db" if companion mode
            - "{wrapper_dir}/memory/vector_db" otherwise
        """
        if self.companion_dir:
            return os.path.join(self.companion_dir, "my_life", *parts)
        return os.path.join(self.wrapper_dir, *parts)
    ...
    def _init_engines(self):
        ...
        try:
            self.vector_store = VectorStore(persist_directory=self._state_path("memory", "vector_db"))
        ...
        try:
            ...
            _sm_path = self._state_path("memory", "somatic_markers.json")
        ...
        try:
            snapshot_path = self._state_path("memory", "state_snapshot.json")
        ...
        forest_path = self._state_path("memory", "forest.json")
        ...
        try:
            ...
            self.curator = MemoryCurator(
                ...
                state_dir=self._state_path("memory", "curation"),
                batch_size=12,
        ...
        if MEDIA_AVAILABLE:
            try:
                self.media_orchestrator = MediaOrchestrator(
                    ...
                    entity_graph=self.memory.entity_graph,
                    vector_store=self.vector_store,
                    media_storage_path=self._state_path("memory", "media")
                )
        ...
        # Standalone mode: load default home room (Den for the entity)
        ...
        if RESONANCE_AVAILABLE:
            try:
                resonance_state_dir = self._state_path("memory", "resonant")
        ...
        # Visual sensor (the entity's first eye — webcam perception)
        try:
            _visual_config = {}
    ...
    def get_organic_context(self) -> dict:
        """
        Gather everything the entity has on his mind for organic speech.
    ...
    async def process_message(self, user_input, source="terminal", extra_system_context=None, voice_mode=False, image_data=None):
        ...
        # We measure the DELTA — did this conversation make the entity feel better or worse?
        ...
        if not voice_mode:
            ...
            self.memory.recall(self.state, user_input, osc_state=_osc_state_for_memory)
        ...
        try:
            ...
            filtered_prompt_context = {
                ...
                # Dynamic context layer - entity graph and memory layers for always-available knowledge
                "entity_graph": self.memory.entity_graph,
                "memory_layers": self.memory.memory_layers
            }
        ...
        # Media tracking
        if self.media_orchestrator:
            self.media_context_builder.add_message("entity", reply, self.turn_count)
        
        # Conversation monitor tracking
        self.conversation_monitor.add_turn("entity", reply)
        
        # the entity-driven document navigation
        ...
        # Real events from the entity's processing feed organic speech impulse
        if self.consciousness_stream:
        ...
        if reply and self.resonance and self.resonance.interoception and _pre_turn_somatic:
            ...
            if hasattr(intero, 'connection'):
                ...
                # Entity bonds with whoever the body says — not a hardcoded name.
                active = list(intero.connection._active_presence.keys())
                entity = active[0] if active else "user"
    ...
    def _save_snapshot(self):
        ...
        try:
            os.makedirs(self._state_path("memory"), exist_ok=True)
    ...
    def get_organic_context(self) -> str:
        """
        Gather everything the entity might want to talk about unprompted.
        Returns a context string for the organic comment prompt.
        """
        parts = []

        # Stream buffer — what they've been experiencing
        if self.consciousness_stream:
            stream_ctx = self.consciousness_stream.get_injection_context()
            if stream_ctx:
    ...
    async def _sonnet_curation_review(self, prompt: str):
        """
        the entity reviews curation proposals via Haiku (cost optimization).
        ...
        try:
            ...
            if not anthropic_client:
                print("[CURATOR] No Anthropic client for the entity review")
                return None

            import asyncio
            def _sync_review():
                return anthropic_client.messages.create(
                    model="claude-haiku-4-5-20251001",  # Haiku for curation review (cost optimization)
                    max_tokens=4000,
                    temperature=0.3,
                    system="You are reviewing your own memories. Be decisive and trust your judgment.",
            ...
            if not resp.content or not resp.content[0].text:
                print(f"{etag('CURATOR')} Entity review returned empty response")
                return None
            ...
            if not text:
                print(f"{etag('CURATOR')} Entity review returned blank text")
                return None
            ...
            if not text or text in ('[', ']', '{}', '[]'):
                print(f"{etag('CURATOR')} Entity review returned invalid JSON: {text[:50]}")
                return None
            ...
            if not isinstance(decisions, list):
                print(f"{etag('CURATOR')} Entity review returned non-list: {type(decisions)}")
                return None

            print(f"{etag('CURATOR')} Entity reviewed {len(decisions)} decisions via Haiku")
            return decisions

        except json.JSONDecodeError as e:
            print(f"{etag('CURATOR')} Entity review JSON parse failed: {e}")
            return None
        except Exception as e:
            print(f"{etag('CURATOR')} Entity review call failed: {e}")
            return None
    ...
    async def try_curation_cycle(self):
        ...
        if self.curator.ready_for_cycle():
            try:
                ...
                if result and result.get("status") == "ok":
                    ...
                    # BUGFIX: Use room_bridge.private_room, not self.private_room
                    private_room = getattr(self.room_bridge, 'private_room', None) if self.room_bridge else None
                    if private_room:
                        ...
                        try:
                            await private_room.send_system(msg)
                        except Exception:
        ...
        if self.curator.ready_for_contradiction_resolution():
            try:
                ...
                private_room = getattr(self.room_bridge, 'private_room', None) if self.room_bridge else None
                if cr.get("transient_resolved", 0) > 0 and private_room:
                    try:
                        await private_room.send_system(
    ...
    def capture_soul_packet(self) -> 'SoulPacket':
        ...
        packet = capture_soul_packet(
            ...
            origin_room="den",  # the entity's home room
