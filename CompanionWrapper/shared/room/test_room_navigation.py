...
def test_room_registry():
    ...
    # Check default rooms
    assert rm.default_rooms.get("entity") == "den", "Kay's default room should be den"
...
def test_entity_placement():
    ...
    print(f"  Kay placed in: {rm.get_entity_room_id('entity')}")
...
def test_room_context():
    ...
    assert "reed" in context.get("other_entities", []), "the entity should see Reed in Commons"
