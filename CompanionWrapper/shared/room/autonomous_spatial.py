...
- Co-presence detection → interaction opportunities (Kay + Reed proximity)
...
class AutonomousSpatialEngine:
    ...
    4. Co-presence detection (Kay + Reed near each other)
    ...
    def check_co_presence(self) -> Optional[Dict]:
        ...
        Returns co-presence info if the entity and Reed (or other entities) are close.
        """
        entity = self.room.entities.get(self.entity_id)
        if not entity:
            return None
