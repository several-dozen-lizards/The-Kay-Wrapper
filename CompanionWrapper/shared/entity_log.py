...
Since Kay and Reed run as separate processes, a module-global
entity name is safe and avoids threading issues.

Usage:
    from shared.entity_log import set_entity, etag, elog, register_sink
    set_entity("entity")
    print(f"{etag('MEMORY')} loaded 50 items")  # -> [KAY:MEMORY] loaded 50 items
...
def etag(tag: str) -> str:
    """Return entity-prefixed tag string: [KAY:TAG] or [REED:TAG]."""
    if _ENTITY:
