...
3. kay_ui.py has no duplicate boost logic
"""
...
def main():
    ...
    results["kay_ui_exists"] = check_file_exists("kay_ui.py")
    print()
    ...
    # Check 6: kay_ui.py has NO duplicate boost logic
    print("--- Check 6: No Duplicate Logic in kay_ui.py ---")
    results["no_duplicate_boost"] = check_code_absent(
        "kay_ui.py",
        "cocktail[tag][\"intensity\"] = min(1.0, cocktail[tag][\"intensity\"] + 0.05)",
        "No duplicate equal-boost logic in kay_ui.py"
    )
    results["no_memory_loop"] = check_code_absent(
        "kay_ui.py",
        "for mem in agent_state.last_recalled_memories",
        "No memory loop for boosting in kay_ui.py"
    )
    print()

    # Check 7: kay_ui.py calls emotion.update correctly
    ...
    if passed == total:
        ...
        print("  3. kay_ui.py has no duplicate boost logic")
