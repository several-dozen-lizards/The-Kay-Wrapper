...
Provides high-level integration between autonomous processing and the main Kay Zero system.
Handles:
...
class AutonomousIntegration:
    ...
    This class provides the main interface between Kay's conversation loop
    ...
    async def offer_autonomous_session(self, agent_state: Any) -> Tuple[bool, str]:
        """
        Offer Kay the option to have an autonomous session.
    ...
    async def run_autonomous_session(
        ...
        try:
            ...
            if goal_override:
                ...
                goal = await self.processor.generate_goal(agent_state)
                if not goal:
                    # Kay declined autonomous processing
                    self.is_in_autonomous_mode = False
...
def main():
    ...
    print("=" * 60)
    print("Kay Zero Autonomous Processing System Test")
    print("=" * 60)
