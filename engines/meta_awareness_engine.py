...
class MetaAwarenessEngine:
    ...
    4. Pattern awareness (Kay noticing his own habits)

    Generates meta-awareness notes that allow Kay to self-correct.
    """

    def __init__(self):
        self.response_history: List[str] = []  # Last N responses
        self.phrase_usage: Counter = Counter()  # Track phrase frequency
        self.question_patterns: List[str] = []  # Track question types Kay asks
    ...
    def _detect_confabulation(self, response: str, memory_engine, agent_state) -> List[str]:
        """
        Detect potential confabulation by checking if Kay stated facts not in memory.

        CRITICAL: Only facts explicitly stored in memory should be stated as facts.
        If Kay makes declarative statements about things not in memory, flag them.
        """
        confabulation_flags = []
    ...
    def _detect_repetitive_patterns(self) -> Dict[str, any]:
        """Detect if Kay is falling into repetitive patterns."""
    ...
    def update(self, agent_state, kay_response: str, memory_engine=None):
        """
        Analyze Kay's response for repetition and confabulation.
    ...
    def get_meta_awareness_notes(self, agent_state) -> List[str]:
        ...
        These allow Kay to self-correct and acknowledge his patterns.
        """
        notes = []
        ...
        # General awareness (if Kay has been talking for a while)
    ...
    def get_awareness_score(self, agent_state) -> float:
        """
        Calculate a meta-awareness score (0.0-1.0) indicating how much Kay needs to self-monitor.
