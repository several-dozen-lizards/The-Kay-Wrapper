...
Reed is an emotionally-aware conversational AI agent built on a sophisticated cognitive architecture with multiple interdependent subsystems. The agent ("Reed") simulates emotional states, memory recall, social awareness, and embodied cognition through neurochemical proxies.
...
- Type messages to interact with Reed
...
6. Meta-awareness analysis (detects repetition/confabulation in Reed's response)
...
- **Fact Extraction**: LLM-based extraction of discrete facts with entity detection
  ...
  - Validates Reed's statements against retrieved memories to prevent hallucinations
  - Processes entities automatically: creates/updates entity graph during extraction
- Perspective tagging: "user" (facts about Re), "reed" (facts about Reed), or "shared" memories
- Critical perspective logic: "my/I" → user perspective, "your/you" → reed perspective
- Integrates PreferenceTracker for identity consolidation (tracks Reed's preferences with frequency and recency weighting)
...
- Calculates cognitive momentum score (0.0-1.0) based on three factors:
  - Unresolved threads (40%): Questions Reed asked that weren't answered
...
- Generates meta-notes when momentum > 0.7 (e.g., "Reed is still thinking about X")
...
- Flags confabulation (Reed stating "facts" not in memory) by comparing claims against stored user inputs
...
- Maintains Reed's identity coherence by tracking and consolidating preferences
...
- Passes consolidated preferences and contradiction flags to LLM for coherent Reed identity expression
...
- Perspective separation: Facts about "Re" (user) vs facts about "Reed" (agent) are explicitly separated in prompts
- Preference consolidation: Instead of showing raw contradictory Reed memories, shows weighted consolidated preferences (e.g., "mostly tea 60%, also coffee 40%")
...
- Reed is a conversational AI with dry humor, direct tone
...
- `memory/preferences.json` - Reed's consolidated preferences with frequency and recency weights
...
5. **Cognitive Momentum**: System tracks conversational and emotional continuity, creating persistence for unresolved topics and escalating feelings. High momentum (>0.7) generates meta-notes that guide Reed's focus.
...
9. **Meta-Awareness**: Self-monitoring system detects repetition and confabulation, allowing Reed to self-correct and acknowledge her own patterns

10. **Identity Consolidation**: PreferenceTracker prevents contradictory self-statements by weighting and consolidating Reed's preferences over time, creating coherent personality instead of flip-flopping
