...
# UPDATED 2025-11-19: Stronger weights to combat semantic dominance
# Previous weights (2.0, 1.8, 0.6) weren't strong enough when semantic layer
# contains >50% of stored memories. New weights provide 8x episodic/semantic ratio.
...
def should_store_claim(
    ...
    Args:
        ...
        source_speaker: Who made this statement ("user" or "kay")
        perspective: Whose perspective ("user" or "kay")
    ...
    Returns:
        ...
        storage_type can be:
            ...
            - "entity_observation" (store as Reed's observation)
            - "blocked" (don't store)
    ...
    # If source is Kay and perspective is Kay, store as normal
    if source_speaker == "kay" and perspective == "kay":
        return (True, "normal")

    # If source is Kay and perspective is user, we need to decide
...
def create_entity_observation(
    ...
    Entity observations are Reed's inferences/interpretations about the user,
    stored separately from authoritative user facts.

    Args:
        fact_data: Original fact data dict
        observer: Who made the observation (default: "kay")
