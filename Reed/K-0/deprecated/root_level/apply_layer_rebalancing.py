...
def apply_patches(filepath):
    ...
    old_unconfirmed = r"""            # CRITICAL BUG FIX: Block Reed's unconfirmed claims about the user
            if needs_confirmation:
                print\(f"\[UNCONFIRMED CLAIM\] X Kay claimed \(needs confirmation\): '\{fact_text\[:60\]\}\.\.\.'" - NOT STORING AS USER FACT\."\)
                print\(f"\[UNCONFIRMED CLAIM\]   Source: Reed's response \| Perspective: \{fact_perspective\} \| Topic: \{fact_topic\}"\)
                # DO NOT store this as a user fact - it could be wrong
                # Skip to next fact
                continue"""

    new_unconfirmed = """            # CRITICAL: Distinguish Reed's observations from false attributions
            if needs_confirmation:
