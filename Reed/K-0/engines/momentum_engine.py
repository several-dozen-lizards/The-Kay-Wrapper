...
class MomentumEngine:
    ...
    def _detect_questions_from_kay(self, reed_response: str) -> List[str]:
        """Extract questions Kay asked that need answers."""
        questions = []

        # Direct question marks
        sentences = re.split(r'[.!]', reed_response)
        ...
        for pattern in indirect_patterns:
            if re.search(pattern, reed_response.lower()):
                match = re.search(pattern + r"[^.!?]*", reed_response.lower())
                if match:
    ...
    def update(self, agent_state, user_input: str, reed_response: Optional[str] = None):
        """
        ...
        if reed_response:
            # Add new questions from Reed's response
            new_questions = self._detect_questions_from_kay(reed_response)
