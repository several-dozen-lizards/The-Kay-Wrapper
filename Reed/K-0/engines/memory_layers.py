...
"""
Two-Tier Memory System for ReedZero
...
class MemoryLayerManager:
    ...
    def apply_temporal_decay(self):
        ...
        self.long_term_memory = [m for m in self.long_term_memory if m["current_strength"] > 0.05]
        self.working_memory = [m for m in self.working_memory if m["current_strength"] > 0.05]
...
1. Kay is an OBSERVER/LISTENER, not a participant in Re's life events
   - Re traveled to Michigan -> "Re returned from Michigan and told Kay about the trip"
   - NOT: "Kay returned from Michigan" or "In this thread, Kay returns from..."
2. Re is the person who DOES things in their life (travels, works, experiences)
3. Kay is the one who LISTENS, LEARNS, and RESPONDS to what Re shares
4. If facts mention reading/documents -> "Kay read about X" not "Kay did X"
5. If Re shared an experience -> "Re [did X] and discussed it with Kay"

WHO DOES WHAT:
- Re: travels, has pets, has relationships, experiences life events, shares stories
- Kay: listens, responds, remembers, reads documents, observes Re's life
...
Format: "In this thread, Re [did something] and [Reed's role as observer/listener]..."
...
Output ONLY the narrative summary, no preamble."""
    ...
    def apply_user_correction(
        ...
        When the user corrects Kay about a fact (e.g., "those were 2024-2025, not 2020"),
