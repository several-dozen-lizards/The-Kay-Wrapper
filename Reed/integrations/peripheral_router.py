...
- All conversation with Kay's personality
- Tool use decisions
- Creative output
- Complex reasoning

Design principle: The peripheral model is Kay's cerebellum/thalamus.
...
class PeripheralRouter:
    ...
    def _call_peripheral(self, system: str, user: str, max_tokens: int = 150) -> Optional[str]:
        """Make a call to the peripheral model."""
        ...
        try:
            ...
            if self._consecutive_failures == 3:
                self._circuit_open_until = _time.time() + 600
                print(f"[PERIPHERAL] ⚠ Circuit breaker OPEN — disabling for 10 minutes.")
            elif self._consecutive_failures > 3 and _time.time() >= self._circuit_open_until:
                self._circuit_open_until = _time.time() + 600
                print(f"[PERIPHERAL] ⚠ Still failing. Extending circuit breaker 10 more minutes.")
            return None
