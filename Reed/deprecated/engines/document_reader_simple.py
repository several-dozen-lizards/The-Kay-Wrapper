...
Be authentic - what did you actually think and feel while reading this?"""
        ...
        # Get Reed's response
        try:
            reed_response = self._call_llm(reading_prompt)
        except Exception as e:
            error_msg = f"Error generating analysis: {e}"
            print(f"[DOCUMENT] {error_msg}")
            return error_msg

        # Extract emotional response from Reed's analysis
        emotional_state = self._extract_emotional_response(reed_response, full_text)
        ...
        return reed_response

    def _call_llm(self, prompt: str, max_tokens: int = 4096) -> str:
        """
        Call LLM to generate Reed's response.
    ...
    def _extract_emotional_response(self, reed_response: str, document_text: str = "") -> Dict:
        """
        Extract ULTRAMAP emotions from Reed's response.

        Args:
            reed_response: Reed's analysis of the document
        ...
        if self.emotion_engine:
            try:
                # Analyze Reed's emotional response
                emotions = self.emotion_engine.detect_emotions(reed_response)
