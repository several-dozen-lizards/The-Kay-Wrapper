"""
Curation UI Integration for Reed

Integrates the memory curation system with the main Reed UI.
...
class CurationUIIntegration:
    """
    Integrates memory curation UI with the main KayUI.
    ...
    def __init__(self, kay_ui):
        """
        Initialize curation UI integration.

        Args:
            kay_ui: The main KayUI instance
        """
        self.kay_ui = kay_ui
        self.palette = kay_ui.palette

        # Get panel font sizes from kay_ui (with defaults if not available)
        self.font_sizes = getattr(kay_ui, 'panel_font_sizes', {
            'tiny': 10,
            'small': 12,
            'normal': 13,
            'medium': 14,
        ...
        # Get memory engine from kay_ui
        # Note: In KayApp, memory engine is stored as 'memory', not 'memory_engine'
        self.memory_engine = getattr(kay_ui, 'memory', None) or getattr(kay_ui, 'memory_engine', None)
    ...
    def _output_to_main_window(self, message: str, tag: str = "system"):
        ...
        Args:
            ...
            tag: Message tag ('system', 'kay', 'user')
        """
        if not self.display_in_main_window:
            return

        # Check if kay_ui has the add_message method
        if hasattr(self.kay_ui, 'add_message'):
            self.kay_ui.add_message(tag, message)
        elif hasattr(self.kay_ui, 'chat_log'):
            # Fallback: write directly to chat_log
            try:
                self.kay_ui.chat_log.configure(state="normal")
                prefix = "Kay" if tag == "kay" else "System"
                self.kay_ui.chat_log.insert("end", f"[{prefix}]: {message}\n\n", tag)
                self.kay_ui.chat_log.see("end")
                self.kay_ui.chat_log.configure(state="disabled")
    ...
    def _show_purge_reserve_list(self, parent):
        ...
        list_view.on_view_full = lambda m: MemoryDetailView(self.kay_ui, self.palette, m)
    ...
    def _start_curation_session(self, parent):
        """Start an AUTONOMOUS curation session where Kay actually reviews memories.
        ...
        # This uses kay_ui's show_curation_in_main_area() to replace chat_log
        if hasattr(self.kay_ui, 'show_curation_in_main_area'):
            print("[CURATION SESSION] Displaying in MAIN WINDOW (large format)")
            self.kay_ui.show_curation_in_main_area(
    ...
    def _show_sidebar_controls_only(self, parent):
        ...
        ctk.CTkLabel(
            controls_frame,
            text="Kay is reviewing memories\nin the main window →",
    ...
    def _create_curation_main_view(self, parent, memories: List[Dict]):
        ...
        def run_curation():
            try:
                ...
                self.kay_ui.after(0, lambda: self._show_results_in_main_window(parent, summary))
                ...
                import traceback
                traceback.print_exc()
                self.kay_ui.after(0, lambda: self._show_error_in_main_window(parent, str(e)))
    ...
    def _create_main_window_progress(self, parent, total_memories: int):
        ...
        self.main_window_log.insert("end", "Waiting for Kay to start reviewing...\n\n")

    def _update_main_window_progress(self, progress_data: Dict):
        """Update the main window progress UI with Reed's full reasoning."""
        try:
            ...
            self.main_window_cluster.configure(
                text=f"Cluster {current}/{total} - Kay is reviewing..."
            )
            ...
            for decision in latest_decisions:
                ...
                # Show Reed's summary if action was SUMMARIZE
                ...
                if kay_override:
                    log_entry += f"  [Kay overrode classifier suggestion]\n"
            ...
            if current < total:
                self.main_window_status.configure(
                    text=f"Kay reviewed cluster {current}/{total}..."
                )
            else:
                self.main_window_status.configure(
                    text=f"Kay has reviewed all {total} clusters. Applying decisions..."
                )
    ...
    def _show_results_in_main_window(self, parent, summary: Dict):
        ...
        if any(examples.values()):
            ...
            ctk.CTkLabel(
                examples_frame,
                text="💭 Example Decisions (with Kay's reasoning):",
                font=ctk.CTkFont(family="Courier", size=FONT_LARGE, weight="bold"),
                text_color=self.palette.get("accent_hi", "#6BB6B6")
            ).pack(anchor="w", padx=20, pady=(15, 10))
    ...
    def _finish_curation_and_restore_chat(self):
        ...
        if hasattr(self.kay_ui, 'restore_chat_in_main_area'):
            self.kay_ui.restore_chat_in_main_area()
    ...
    def _display_curation_in_main_panel(self, memories: List[Dict], parent):
        ...
        displaying Reed's reasoning with large, readable fonts.
        """
        self.current_view = "curation"
        ...
        def run_curation():
            try:
                ...
                # Update UI on main thread
                self.kay_ui.after(0, lambda: self._show_curation_results_in_main_panel(parent, summary))
                ...
                import traceback
                traceback.print_exc()
                self.kay_ui.after(0, lambda: self._show_curation_error_in_main_panel(parent, str(e)))
    ...
    def _create_main_panel_progress(self, parent, total_memories: int):
        ...
        # Initial message
        self.main_decision_log.insert("end", "Waiting for Kay to start reviewing...\n\n")
    ...
    def _update_main_panel_progress(self, progress_data: Dict):
        """Update the main panel progress UI with Reed's full reasoning."""
        try:
            current = progress_data.get("current", 0)
            total = progress_data.get("total", 1)
            stats = progress_data.get("stats", {})
            ...
            for decision in latest_decisions:
                ...
                # Note if Kay overrode the classifier
                if kay_override:
                    log_entry += f"  [Kay overrode classifier suggestion]\n"
            ...
            if current < total:
                self.main_status_label.configure(
                    text=f"Kay reviewed cluster {current}/{total}... waiting for next cluster"
                )
            else:
                self.main_status_label.configure(
                    text=f"Kay has reviewed all {total} clusters. Applying decisions..."
    ...
    def _open_curation_main_window(self, memories: List[Dict], sidebar_parent):
        ...
        self.curation_window = ctk.CTkToplevel(self.kay_ui)
        self.curation_window.title("🧠 Kay's Memory Curation Session")
        self.curation_window.geometry("900x700")
        self.curation_window.transient(self.kay_ui)  # Keep on top of main window
        ...
        def run_curation():
            try:
                ...
                # Update UI on main thread
                self.kay_ui.after(0, lambda: self._show_curation_results_in_main_window(summary))
                ...
                import traceback
                traceback.print_exc()
                self.kay_ui.after(0, lambda: self._show_curation_error_in_main_window(str(e)))
    ...
    def _create_progress_panel(self, parent, total_memories: int):
        ...
        # Initial message
        self.decision_log.insert("end", "Waiting for Kay to start reviewing...\n\n")
    ...
    def _update_progress_ui(self, progress_data: Dict):
        """Update the progress UI with latest curation data including Reed's full reasoning."""
        try:
            current = progress_data.get("current", 0)
            total = progress_data.get("total", 1)
            stats = progress_data.get("stats", {})
            ...
            if self.display_in_main_window and latest_decisions:
                for decision in latest_decisions[:2]:  # Show first 2 decisions per cluster
                    ...
                    if reasoning:
                        ...
                        self._output_to_main_window(summary_msg, "kay")
            ...
            # Update status to show we're waiting for Reed
            if current < total:
    ...
    def _show_curation_results(self, parent, summary: Dict):
        """Show the curation session results with Reed's patterns and full reasoning."""
    ...
    def _analyze_curation_patterns(self, examples: Dict) -> str:
        """Analyze Reed's curation patterns from the examples."""
        patterns = []

        # Analyze what Kay kept
        kept = examples.get("kept", [])
        if kept:
            kept_reasons = [e.get("reasoning", "").lower() for e in kept]
            patterns.append("Kay tends to KEEP:")
        ...
        # Analyze what Kay summarized
        summarized = examples.get("summarized", [])
        if summarized:
            patterns.append("\nKay tends to SUMMARIZE:")
        ...
        if not patterns:
            return "Analyzing Kay's patterns..."
        ...
        for mem in raw_memories:
            ...
            if mem_type == 'full_turn':
                ...
                if user_input or response:
                    content = f"User: {user_input}\nKay: {response}" if response else user_input
    ...
    def _show_no_memories_message(self, parent):
        ...
        ctk.CTkLabel(
            parent,
            text="Have some conversations with Kay first\nto build up memories for curation.",
            font=ctk.CTkFont(family="Courier", size=10),
            text_color=self.palette.get("muted", "#9B7D54"),
            justify="center"
        ).pack(pady=10)
    ...
    def _handle_decision(self, decision: Dict):
        ...
        if kay_override:
            # Determine what Kay classified it as based on action
    ...
    def _apply_to_memory_system(
        ...
        if action == "delete":
            ...
            if target:
                ...
                # Get deletion reason
                reason = memory.get("_deletion_reason", "Kay chose to delete during curation")
                kay_note = memory.get("_kay_note")
    ...
    def _complete_session(self, parent, decisions: list):
        ...
        if hasattr(self.kay_ui, '_update_memory_layer_display'):
            try:
                self.kay_ui._update_memory_layer_display()
            except:
                pass


def setup_curation_ui(kay_ui) -> CurationUIIntegration:
    """
    ...
    """
    return CurationUIIntegration(kay_ui)
