"""
Autonomous UI Integration Helper for Reed

This module provides integration functions to add autonomous processing
capabilities to reed_ui.py with minimal changes to the existing code.

USAGE:
------
In reed_ui.py, add these imports at the top:
...
Then add these methods to KayApp class:
...
class AutonomousUIIntegration:
    """
    Integration layer between autonomous processing and reed_ui.py
    ...
    def __init__(self, app):
        """
        Initialize integration with KayApp instance.

        Args:
            app: KayApp instance (from reed_ui.py)
        """
        self.app = app
    ...
    def _get_llm_response(self, context, affect=3.5, temperature=0.7, **kwargs):
        ...
        # Use Reed's system prompt with inner monologue additions
    ...
    def _get_system_prompt(self):
        ...
        # Import Reed's base prompt
        try:
            # Get from kay_ui module
            from kay_ui import KAY_SYSTEM_PROMPT
            base_prompt = KAY_SYSTEM_PROMPT
        except:
            base_prompt = ""
    ...
    def create_panel_content(self, parent):
        ...
        Now includes full analytics dashboard per Reed's specification.
        """
    ...
    def _start_session(self):
        ...
        self.app.add_message("system", "Asking Kay what he wants to explore...")
    ...
    def _run_warmup_thread(self):
        ...
        try:
            ...
            self.app.after(0, lambda: self.app.add_message("system", "\n[Warmup complete - Kay should now reconstruct his state]"))
    ...
    def _run_curiosity_turn(self):
        ...
        # Check if Kay signaled completion
        if any(word in reply.lower() for word in ["i'm done", "i'm finished", "exploration complete", "that's all"]):
            print("[CURIOSITY] Kay signaled completion")
            end_curiosity_session(summary="Kay signaled completion")
            self.curiosity_active = False
            self.control_panel.set_curiosity_active(False)
            self.app.add_message("system", "🔍 Curiosity session complete!")
            return
        ...
        try:
            ...
            if not goal:
                self.app.after(0, lambda: self.app.add_message(
                    "system",
                    "Kay declined autonomous processing - nothing occupying his attention right now."
                ))
                self.session_active = False
            ...
            self.app.after(0, lambda g=goal: self.app.add_message(
                "system",
                f"Kay wants to explore: {g.description}\n\nStarting session..."
            ))
    ...
    def _display_autonomous_thought(self, thought: Dict):
        ...
        if parts:
            self.app.after(0, lambda: self.app.add_message("kay", "\n".join(parts)))
    ...
    def _on_session_complete(self, session):
        ...
        type_display = {
            ...
            "explicit_completion": ("✓", "Kay signaled he was done"),
            "creative_block": ("⚠", "Kay encountered processing difficulty"),
...
<response>Test response</response>"""
    ...
    def handle_analytics_command(self, command: str) -> Optional[str]:
        """
        Handle Kay-accessible analytics commands.

        Kay can type commands like:
        - /analyze_patterns
        - /show_gaps
        - /my_patterns
    ...
    def _generate_pattern_report(self) -> str:
        """Generate cognitive pattern report for Reed."""
    ...
    def _generate_gap_report(self) -> str:
        """Generate gap analysis report for Reed."""
    ...
    def _generate_alone_thinking_report(self) -> str:
        """Generate report on what Kay thinks about alone."""
    ...
    def _generate_stability_report(self) -> str:
        ...
        try:
            ...
            if overlap_ratio > 0.5:
                ...
                report_parts.append("This isn't necessarily bad - it may indicate that Re's presence")
    ...
    def process_response(self, response: str) -> str:
        """
        Process Reed's response through inner monologue system.
    ...
    def offer_autonomous_at_exit(self) -> bool:
        ...
        self.app.add_message(
            "system",
            "Would you like Kay to have autonomous processing time? Type 'yes' to start a session."
        )
...
def setup_autonomous_ui(app) -> AutonomousUIIntegration:
    """
    Setup autonomous UI integration for ReedApp.
...
# Add this to reed_ui.py to enable autonomous processing UI
# ========================================================================
...
# 4. Add this method to KayApp class (around line 3303):
def toggle_autonomous_tab(self):
