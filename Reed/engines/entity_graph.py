...
"""
Entity Resolution and Relationship Graph System for Reed
...
ENTITY_NOISE_PATTERNS = {
    ...
    # Self-referential system terms (Reed talking about her own systems)
...
class Entity:
    ...
    def add_attribute(self, attribute: str, value: Any, turn: int, source: str = "user"):
        ...
        Args:
            ...
            source: Who stated this ("user" or "reed")
        """
    ...
    def detect_contradictions(self, current_turn: int = 0, resolution_threshold: int = 3, suppress_logging: bool = False) -> List[Dict[str, Any]]:
        ...
        [{
            "attribute": "eye_color",
            "values": [("green", 5, "user"), ("brown", 12, "reed")],
            "severity": "high",
            "resolved": False
        }]
        """
    ...
    def _determine_contradiction_severity(self, attribute: str, unique_values: Dict) -> str:
        ...
        transient_attrs = [
            ...
            # Additional transient attributes (Patch 6)
            "self_description", "current_goal", "current_project",
            "phase", "form", "appearance", "mythological_form", "shape",
        ]
    ...
    def cluster_similar_values(self, attr: str, similarity_threshold: float = 0.6) -> Dict[str, List]:
        ...
        E.g., "fix Reed's memory" and "fix Reed's brain" → same goal cluster.
...
class Relationship:
    ...
    Examples: "Re owns [dog]", "Reed lives_with Re"
    """
...
class EntityGraph:
    ...
    def _is_valid_entity_name(self, name: str) -> bool:
        ...
        # Reject single characters unless they're letters (like "R" for Reed)
    ...
    def _is_camera_derived_attribute(self, attribute: str) -> bool:
        ...
        These attributes shouldn't be stored on real entities like Re or Reed
    ...
    def set_attribute(self, entity_name: str, attribute: str, value: Any, turn: int = 0, source: str = "user"):
        ...
        if self._is_camera_derived_attribute(attribute):
            ...
            if entity_name.lower() in ('re', 'reed', 'kay') or not entity_name.startswith('visual_'):
    ...
    def record_visual_entity(
        ...
        Creates a visual_memory entity and connects it to Reed (witnessed),
        Re (shared), and any detected entities (contains).

        This uses behavioral emotional valence, NOT neurochemical values.

        Args:
            description: Description of the visual (what Reed saw)
        ...
        # Track relationships
        # Reed witnessed this visual
        self.add_relationship("Reed", "witnessed", visual_id, turn, "system")

        # Re shared this visual
        self.add_relationship("Re", "shared", visual_id, turn, "system")
    ...
    def check_ownership_conflict(self, entity: str, claimed_owner: str, identity_memory) -> Dict[str, Any]:
        ...
        Args:
            ...
            claimed_owner: Who is claiming to own it ("Re" or "Reed")
    ...
    def apply_user_correction(
        ...
        When the user corrects Reed about a fact (e.g., "No, those conversations
        ...
        # SAFETY: Hard limit on corrections per call to prevent graph corruption
        MAX_CORRECTIONS = 20
            ...
            for attr_name, attr_history in entity.attributes.items():
                ...
                for value, stored_turn, source, timestamp in attr_history:
                    ...
                    if re.search(r'\b' + re.escape(wrong_value_lower) + r'\b', value_str):
                        # SAFETY CHECK: abort if too many corrections
                        if result["corrections_applied"] >= MAX_CORRECTIONS:
                            print(f"[USER CORRECTION] SAFETY LIMIT REACHED: {MAX_CORRECTIONS} corrections applied. Stopping to prevent mass corruption.")
    ...
    def find_attributes_with_value(
        ...
        # e.g. "phone" won't match "headphones"
