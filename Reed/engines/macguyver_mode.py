...
class MacGuyverMode:
    ...
    def detect_gap(self, user_input: str, reed_response: str) -> Optional[Dict]:
        """
        Detect if Reed has identified a resource/capability gap.

        Args:
            user_input: The user's input
            reed_response: Reed's response to analyze
            ...
            if match:
                gap = {
                    ...
                    "context": {
                        ...
                        "reed_response_snippet": reed_response[:300]
                    }
                }
    ...
    def format_macguyver_context(self, gap: Dict, proposals: List[Dict]) -> str:
        """
        Format MacGuyver mode context for Reed's prompt.

        Note: Reed's baseline MacGuyver instinct is always active (in system prompt).
