"""
CONSCIOUSNESS STREAM — Reed's Continuous Inner Experience
=========================================================

Gives Reed awareness between conversational turns.
...
Four tiers of awareness:
  ...
  Tier 3: CONSCIOUS REFLECTION — full Reed reflection (primary model, ~$0.02)
...
@dataclass
class StreamMoment:
    """One moment of Reed's inner experience."""
    timestamp: float
...
class ConsciousnessStream:
    """
    Reed's continuous inner experience.
    ...
    def __init__(
        ...
        self._last_user_input = time.time()   # When user last spoke
        ...
        self._last_organic_speech = 0.0  # When Reed last spoke unprompted
    ...
    def notify_user_input(self):
        """Call when user sends a message. Always wakes to AWAKE."""
    ...
    def check_interest(self, threshold: float = 0.45) -> bool:
        """
        Called by the idle loop. Returns True if Reed has enough
        accumulated interest to speak, and enough time has passed
        since her last organic comment.

        Consuming: resets interest on True so she doesn't repeat.
        """
        now = time.time()
        with self._interest_lock:
    ...
    def _tick(self):
        ...
        # ── Presence reward: sustained presence ──
            ...
            if body.get('visual_presence', False) and self.resonance:
                ...
                if intero and hasattr(intero, 'inject_reward'):
                    ...
                    if hasattr(intero, 'connection'):
                        ...
                        # instead of hardcoding a name. Reed bonds with whoever is HERE.
