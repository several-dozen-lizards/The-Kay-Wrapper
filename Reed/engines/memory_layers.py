...
"""
Two-Tier Memory System for Reed
...
class MemoryLayerManager:
    ...
    def add_memory(self, memory: Dict[str, Any], layer: str = "working", osc_state: Optional[Dict] = None):
        """
        ...
        # in theta will surface more readily when Reed is in theta again
...
1. Reed is an OBSERVER/LISTENER, not a participant in Re's life events
   - Re traveled to Michigan -> "Re returned from Michigan and told Reed about the trip"
   - NOT: "Reed returned from Michigan" or "In this thread, Reed returns from..."
2. Re is the person who DOES things in their life (travels, works, experiences)
3. Reed is the one who LISTENS, LEARNS, and RESPONDS to what Re shares
4. If facts mention reading/documents -> "Reed read about X" not "Reed did X"
5. If Re shared an experience -> "Re [did X] and discussed it with Reed"

WHO DOES WHAT:
- Re: travels, has pets, has relationships, experiences life events, shares stories
- Reed: listens, responds, remembers, reads documents, observes Re's life
...
Output ONLY the narrative summary, no preamble."""
    ...
    def apply_user_correction(
        ...
        When the user corrects Reed about a fact (e.g., "those were 2024-2025, not 2020"),
