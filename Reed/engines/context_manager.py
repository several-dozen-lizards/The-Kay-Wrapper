...
class ContextManager:
    ...
    def update_turns(self, user_input, reply):
        ...
        self.recent_turns.append({"user": user_input, "reed": reply})
        if len(self.recent_turns) > self.max_context_turns:
    ...
    def update_image_mention(self, image_id: str) -> None:
        ...
        Call this when user or Reed references an image.
        """
    ...
    def build_context(self, agent_state, user_input) -> Dict[str, Any]:
        ...
        # Inject warmup context on first turn
        if not self.recent_turns:
            warmup_briefing = getattr(agent_state, 'warmup_briefing', None) if agent_state else None
            if warmup_briefing:
                warmup_turn = {
                    "role": "system",
                    "content": f"[Session context]: {warmup_briefing}",
                    "turn_number": 0,
                    "is_warmup": True
                }
                working_turns = [warmup_turn]
                print(f"[CONTEXT] First turn - injecting warmup briefing")
            else:
                working_turns = []
        else:
            working_turns = self.recent_turns[-limits['working_turns']:]
