"""
Memory Curation Engine for Reed
...
Kay learns to recognize content types and apply appropriate retention strategies.
"""
...
class CurationDecision:
    """A curation decision made by Kay."""
    memory_id: str
    ...
    reason: str = ""
    kay_override: bool = False  # Did Kay override the classifier?
...
class ContentTypeClassifier:
    ...
    def __init__(self):
        ...
        self.learning_adjustments: Dict[str, float] = {}  # Adjust weights based on Kay's feedback
    ...
    def classify(self, memory: Dict) -> ContentTypeClassification:
        ...
        # If confidence is low, require Reed's judgment
        if max_score < 0.3:
            return ContentTypeClassification(
                content_type=ContentType.REQUIRES_JUDGMENT,
                confidence=max_score,
                indicators_matched=max_indicators,
                reason="Low confidence across all types - Kay should classify",
                memory_id=memory.get("id", "")
            )
    ...
    def adjust_from_feedback(self, memory_id: str, correct_type: ContentType):
        """
        Learn from Reed's corrections to improve future classification.

        Args:
            memory_id: The memory that was misclassified
            correct_type: The type Kay determined was correct
        """
...
class MemoryCurator:
                ...
                with open(sessions_file, 'r', encoding='utf-8') as f:
                    ...
                    for s in data.get("sessions", []):
                        session = CurationSession(
                            ...
                            kay_overrides=s.get("kay_overrides", 0),
    ...
    def generate_curation_prompt(self, batch_analysis: Dict[ContentType, List[Dict]]) -> str:
        """
        Generate a prompt for Reed to review the curation analysis.
        """
    ...
    def apply_decision(
        ...
        Args:
            ...
            reason: Reed's reason for the decision
            kay_override: Whether Kay overrode the classifier
        """
        ...
        if self.current_session:
            ...
            if kay_override:
                self.current_session.kay_overrides += 1

            self.current_session.decisions.append(decision)

        # If Kay overrode, update classifier learning
        if kay_override and classification.content_type != ContentType.REQUIRES_JUDGMENT:
            # Reed's override indicates the classifier was wrong
            # We'd need to know what Kay classified it as to learn properly
            pass
...
"""
    ...
    def get_stats(self) -> Dict:
        ...
        kay_overrides = sum(s.kay_overrides for s in self.sessions)
        ...
        return {
            ...
            "kay_overrides": kay_overrides,
            "override_rate": round(kay_overrides / max(total_reviewed, 1) * 100, 1)
        }
...
class CurationLearningTracker:
    """
    Tracks how Reed's curation judgment develops over time.

    Records:
    - Accuracy of content type recognition
    - Patterns in Reed's overrides
    ...
    def record_session_confidence(self, session: CurationSession):
        ...
        self.learning_data["confidence_over_time"].append({
            ...
            "override_rate": session.kay_overrides / session.memories_reviewed,
    ...
    def get_learning_summary(self) -> Dict:
        """Get summary of Reed's curation learning progress."""
...
class AutonomousCurationProcessor:
    ...
    2. Builds prompts for Reed to review each cluster
    ...
    def run_curation_session(
        ...
        Kay reviews all memories and makes decisions.
    ...
    def _process_cluster(self, cluster: List[Dict], cluster_num: int, total_clusters: int) -> List[Dict]:
        """
        Process a single cluster of memories through Reed's LLM.
        ...
        # Call Reed's LLM
        print(f"[CURATION] Asking Kay to review cluster {cluster_num}...")

        system_prompt = """You are Kay, reviewing your own memories for curation.
...
Respond with your decisions in the exact format requested."""
        ...
        try:
            ...
            print(f"[CURATION] Kay responded ({len(response)} chars)")

        except Exception as e:
            print(f"[CURATION] LLM error: {e}")
            return []

        # Parse Reed's response into decisions
        decisions = self._parse_reed_response(response, cluster)
    ...
    def _build_cluster_prompt(self, cluster: List[Dict], cluster_num: int, total_clusters: int) -> str:
        """
        Build the prompt for Reed to review a memory cluster.
        """
...
"""
    ...
    def _parse_reed_response(self, response: str, cluster: List[Dict]) -> List[Dict]:
        """
        Parse Reed's response into structured decisions.
        """
        decisions = []
        ...
        if not decisions:
            print("[CURATION] Warning: Could not parse Kay's response, using defaults")
            for item in cluster:
                decisions.append({
                    "memory": item["memory"],
                    "classification": item["classification"],
                ...
                if action == "delete":
                    ...
                    if self.purge_reserve:
                        self.purge_reserve.soft_delete(
                            ...
                            content=memory.get("content", ""),
                            reason=reasoning,
                            kay_note=f"Curation decision: {reasoning}",
