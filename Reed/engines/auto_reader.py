...
Reed reads through entire documents internally, responding naturally to each segment
without exposing document text or navigation to the user.

User flow:
1. User clicks Import
2. Reed automatically reads through all segments
3. Reed's natural responses appear in chat as normal conversation
...
class AutoReader:
    ...
    Feeds each segment to Reed's LLM and collects natural responses.
    """
    ...
    async def read_document_async(self, doc_reader, doc_name: str, agent_state=None, start_segment=1):
        ...
        try:
            for i in range(start_pos, total_segments):
                ...
                # Build Reed's internal reading prompt
                reading_prompt = self._build_reading_prompt(chunk_info)

                try:
                    # Get Reed's response (this uses full LLM pipeline)
                    response = await self._get_reed_response_async(reading_prompt, agent_state)
                    ...
                    # Display ONLY Reed's response in chat (NOT the document text)
    ...
    def read_document_sync(self, doc_reader, doc_name: str, agent_state=None, start_segment=1):
        ...
        try:
            for i in range(start_pos, total_segments):
                ...
                try:
                    # Get Reed's response (synchronous)
                    response = self._get_reed_response_sync(reading_prompt, agent_state)
    ...
    def _build_reading_prompt(self, chunk_info: Dict[str, Any]) -> str:
        """
        Build the internal prompt that Reed sees (never shown to user).

        This creates natural reading instructions that prompt Reed to respond
...
Your response (be natural and specific):"""
    ...
    async def _get_reed_response_async(self, prompt: str, agent_state=None) -> str:
        """
        Get Reed's response to a segment (async version).
    ...
    def _get_reed_response_sync(self, prompt: str, agent_state=None) -> str:
        """
        Get Reed's response to a segment (synchronous version).
        """
        try:
    ...
    def _store_reading_turn(self, chunk_info: Dict[str, Any], response: str, agent_state=None):
        """
        Store Reed's reading response in memory.
...
def create_auto_reader(llm_response_func, message_display_func, memory_engine=None):
    ...
    Args:
        llm_response_func: Function that gets Reed's response to a prompt
