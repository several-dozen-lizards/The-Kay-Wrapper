...
- Temporal awareness (Reed knows when facts changed)
...
class MemoryEngine:
    ...
    emotional tagging, perspective tagging ("user", "reed", or "shared"),
    ...
    def __init__(self, semantic_memory: Optional[Any] = None, file_path: str = "memory/memories.json", motif_engine: Optional[Any] = None, momentum_engine: Optional[Any] = None, emotion_engine: Optional[Any] = None, vector_store: Optional[Any] = None):
        ...
        # Prune stale contradictions at startup
        if hasattr(self.entity_graph, 'prune_old_attribute_history'):
            pruned = self.entity_graph.prune_old_attribute_history()
            if pruned:
                print(f"[ENTITY] Pruned {pruned} stale attribute conflicts")
    ...
    def retrieve_biased_memories(self, bias_cocktail, user_input, num_memories: int = 7, relevance_floor: float = 0.3):
        ...
        # This ensures Reed never loses her core identity facts even with zero keyword overlap
...
- Reed is the AI (being addressed)

FROM USER INPUT:
- "I/my/me" in user input = Re (user perspective)
- "you/your" in user input = Reed (reed perspective)
- Entities mentioned = Re's entities

FROM REED'S RESPONSE:
- "your/you" in Reed's response = about Re (USER perspective, NOT Reed)
- "my/I/me" in Reed's response = about Reed ONLY if DIRECT SELF-ASSERTION
...
- When Reed mentions entities that are known to belong to Re, DO NOT create Reed ownership
- Conversational references to Re's life = Re's facts, not Reed's facts

CRITICAL: When Reed says things like "your cats", "your dog", "you have", extract these as REINFORCING Re's ownership, NOT creating Reed ownership.

DOCUMENT/READING CONTEXT:
When conversation involves documents, files, or imported content:
- Reed READS documents, doesn't EXPERIENCE them
- "Reed examined Archive Zero" → WRONG (implies lived experience)
- "Reed read documents about X" → CORRECT (reading activity)
...
EXAMPLE 3 - Reed makes direct self-assertion:
User: "What color are your eyes?"
Reed: "My eyes are gold."
→ Extract:
[
  {{
    "fact": "Reed's scales are teal-gold",
    "perspective": "reed",
    "topic": "appearance",
    "entities": ["Reed"],
    "attributes": [{{"entity": "Reed", "attribute": "eye_color", "value": "gold"}}]
  }}
]
...
Reed: "What have you tried so far?"
→ Extract:
[
  {{
...
EXAMPLE 8 - User CORRECTS Reed about a fact:
User: "No, those ChatGPT conversations were from 2024-2025, not 2020"
Reed: "Oh, you're right - I had the dates wrong."
→ Extract:
[
  {{
...
- IMPORTANT: Do NOT treat discussions ABOUT other entities (e.g., "Kay Zero is doing X", "the Kay wrapper needs fixing") as corrections. Only flag is_correction=true when the user is correcting something Reed said or believed INCORRECTLY. Mentioning another entity by name is NOT a correction of that entity's data.
...
Extract facts now:"""
        ...
        try:
            ...
            if facts is None:
                ...
                fallback_perspective = "reed" if response and not user_input else "user"
            ...
            if not isinstance(facts, list):
                ...
                facts = [facts] if isinstance(facts, dict) else [{"fact": str(facts)[:200], "perspective": "reed", "topic": "exploration"}]

            # Validate structure and process entities
            validated_facts = []

            # CRITICAL FIX: Determine source speaker
            # If user_input is empty and response exists, facts are from Reed's response
            # If user_input exists and response is empty, facts are from user input
            source_speaker = "user" if user_input and not response else "reed"

            for fact in facts:
                if isinstance(fact, dict) and "fact" in fact:
                    perspective = fact.get("perspective", "user")

                    # CRITICAL BUG FIX: Detect Reed's claims about the user
                    # When Reed makes a statement about the user (perspective="user"),
                    # it should NOT be stored as ground truth - mark for confirmation
                    needs_confirmation = False
                    if source_speaker == "reed" and perspective == "user":
                        # Reed is making a claim about the user (Re)
                    ...
                    # IMPORTANT: Pass source_speaker to prevent Reed's claims from becoming truth
    ...
    def _process_entities(self, fact_data: Dict[str, Any], source_speaker: str = None):
        ...
        Prevents Reed from creating false relationships when confused about ownership.

        CRITICAL FIX #2: Use source_speaker to track WHO SAID this, not perspective.
        Perspective = about whom (user/reed), source_speaker = who said it (user/reed).
        When Reed says "Your eyes are green", perspective=user but source_speaker=reed.

        Args:
            fact_data: Fact dict with entities, attributes, and relationships
            source_speaker: Who said this fact ("user" or "reed")
        """
        ...
        for attr_data in fact_data.get("attributes", []):
            ...
            if entity_name and attribute_name and value:
                ...
                # Reed READS documents, doesn't EXPERIENCE them
                if entity_name == "Reed" and attribute_name == "activity":
                    ...
                    if has_experience_word and has_document_indicator:
                        # Reframe: Reed READ these documents, didn't experience them
                        ...
                        print(f"[ACTIVITY CORRECTION] '{original_value}' → '{value}' (Reed reads, doesn't experience)")
        ...
        for rel_data in fact_data.get("relationships", []):
            ...
            if entity1 and relation_type and entity2:
                ...
                if relation_type == "owns":
                    # IMPORTANT: Only verify Reed's statements, not user's
                    # User statements are ALWAYS authoritative (ground truth)

                    if speaker == "reed":
                        # Reed is making an ownership claim - verify against identity layer
                        ...
                        if conflict_check["should_block"]:
                            # BLOCK: Reed is confused about ownership
                            ...
                            # No conflict - but still inferred since Reed said it
                            fact_data["confidence"] = "inferred"
                        ...
                        if conflict_check["conflict"]:
                            # User is correcting Reed's previous confusion
                            print(f"[OWNERSHIP CORRECTION] User establishes ground truth: {entity1} owns {entity2} (corrects previous Reed confusion)")
                        else:
    ...
    def _apply_user_correction(self, correction_data: Dict[str, Any]):
        ...
        When the user corrects Reed about a fact, this propagates that correction
        ...
        if result["corrections_applied"] > 0:
            ...
            # SAFETY: Broader search disabled for common entity names to prevent mass graph corruption.
            # The old fallback searched ALL attributes for wrong_value and replaced them all,
            # which caused catastrophic corruption when wrong_value was a common name like "Kay".
            # Now we only do targeted corrections. If the entity/attribute pattern doesn't match,
            # we log it and move on rather than risk nuking thousands of legitimate references.
            BROAD_SEARCH_BLOCKED_VALUES = {"kay", "reed", "re", "john", "zero", "chrome", "saga"}
            if wrong_value.lower().strip() in BROAD_SEARCH_BLOCKED_VALUES:
                print(f"[USER CORRECTION] No direct match found. Broader search BLOCKED for protected value '{wrong_value}'.")
                print(f"[USER CORRECTION] This prevents mass corruption of legitimate entity references.")
            else:
                # Try broader search only for non-entity values (dates, numbers, etc.)
                print(f"[USER CORRECTION] No direct match found, trying broader search (non-entity value)...")
    ...
    def _detect_perspective(self, user_input: str) -> str:
        ...
        - "reed" = facts about Reed (the agent - the AI)
        - "shared" = facts about both or shared experiences

        CRITICAL RULES:
        1. The USER is ALWAYS the person typing (Re)
        2. "I/my/me" = the speaker (the user Re)
        3. "you/your" = the addressee (Reed, the AI)
        4. Mentioned names (Kay, Sarah, etc.) are THIRD PARTIES, not the user
        ...
        if text.startswith("remember that you") or text.startswith("reed, remember that you"):
            return "reed"
        ...
        # Second-person pronouns = Reed (the AI being addressed)
    ...
    def _validate_fact_against_sources(self, fact: str, fact_perspective: str, retrieved_memories: List[Dict]) -> bool:
        """
        Validate that Reed's claimed facts about the user were actually stated by the user.

        Returns True if fact is VALID (should be stored), False if HALLUCINATION (should be blocked).

        CRITICAL: Prevents Reed from inventing/fabricating details that weren't mentioned.
        """
        # Only validate Reed's statements about the user
        if fact_perspective != "reed":
            return True  # User's own statements are always valid

        fact_lower = fact.lower()

        # Check if Reed is claiming a fact about the user (contains "you/your" or user entities)
        is_about_user = any(word in fact_lower for word in ["you", "your", "re's", "re "])

        if not is_about_user:
            return True  # Reed's statements about herself are not validated here

        # Reed is making a claim about the user - verify it was actually mentioned
            ...
            if fact_colors and "eye" in combined_user_text:
                ...
                for fact_color in fact_colors:
                    if fact_color not in mem_colors:
                        # Reed is adding a color detail user never mentioned
                        print(f"[HALLUCINATION DETAIL] Reed added color '{fact_color}' but user only mentioned {mem_colors}")
        ...
        # We can't validate everything, so we trust Reed unless we can prove fabrication
        return True
    ...
    def _extract_entity(self, fact_text: str) -> str:
        """Extract which entity a fact is about (reed/re/unknown)."""
        fact_lower = fact_text.lower()

        if any(pattern in fact_lower for pattern in ["reed's", "reed is", "reed has", "reed "]):
            return "reed"
        ...
        if any(pattern in fact_lower for pattern in ["i ", "my ", "i'm", "i am"]):
            # First person - depends on context, but often Reed speaking
            return "reed"
    ...
    def _check_contradiction(self, new_fact: str, retrieved_memories: List[Dict]) -> bool:
        ...
        1. Only compare SAME entity (Reed vs Reed, Re vs Re)
    ...
    def encode_memory(self, user_input, response, emotional_cocktail, emotion_tags, perspective=None, agent_state=None):
        ...
        # FIX: Extract facts from Reed's response ONLY
        ...
        print(f"[MEMORY] Extracted {len(extracted_facts)} facts from Reed's response (user facts already stored in pre-response)")
        ...
        for fact_data in extracted_facts:
            ...
            # CRITICAL: Distinguish Reed's observations from false attributions
            if needs_confirmation:
                ...
                if not should_store:
                    # False attribution (Reed claiming user SAID something) - BLOCK
                    print(f"[FALSE ATTRIBUTION] X Reed claimed: '{fact_text[:60]}...' - NOT STORING.")
                    print(f"[FALSE ATTRIBUTION]   Reason: False attribution (user didn't say this)")
                    print(f"[FALSE ATTRIBUTION]   Source: Reed | Perspective: {fact_perspective} | Topic: {fact_topic}")
                    continue  # Skip this fact

                if storage_type == "entity_observation":
                    # Valid observation (Reed's inference about user state) - ALLOW with tagging
                    print(f"[ENTITY OBSERVATION] ✓ Storing Reed's observation: '{fact_text[:60]}...'")
                    print(f"[ENTITY OBSERVATION]   Type: {fact_topic} | Observer: Reed → {fact_perspective}")

                    # Tag as entity observation for retrieval filtering
                    fact_data = create_entity_observation(fact_data, observer="reed", observed="re")
                    # IMPORTANT: Don't skip - continue to storage below

            # Validate Reed's statements against retrieved memories (prevent hallucination)
            if fact_perspective == "reed" and retrieved_memories:
                is_valid_fact = self._validate_fact_against_sources(fact_text, fact_perspective, retrieved_memories)

                if not is_valid_fact:
                    print(f"[HALLUCINATION BLOCKED] X Reed fabricated '{fact_text[:60]}...' - NOT STORING.")
                    continue
                ...
                if is_contradictory:
                    ...
                    # Reed's reality can change (was confused → now clear-headed = growth)
                    # Don't block legitimate state changes, just log the potential conflict
                    print(f"[CONTRADICTION FLAGGED] ! Reed stated '{fact_text[:60]}...' may conflict with memory")
        ...
        for fact_record in [mem for mem in self.memories
            ...
            # CRITICAL: Document-imported facts should NEVER be Reed's identity
            # They are things Reed READ ABOUT, not things Reed IS
    ...
    def retrieve_unified_importance(self, bias_cocktail, user_input, max_memories: int = 250, conversational_mode: bool = False) -> List[Dict[str, Any]]:
        ...
        # 1.1: Identity facts (Reed's core identity)
        ...
        for mem in all_other:
            ...
            if mem_type == 'full_turn':
                ...
                # LEGACY Import-related memories: search Reed's analysis, emotional observations, and shared context
                text_blob = (
                    mem.get('fact', '') + ' ' +
                    mem.get('Reed_analysis', '') + ' ' +
                    mem.get('Reed_impression', '') + ' ' +
            ...
            # This enables "spatial memory" - Reed can recall WHERE he read information
        ...
        if len(relevant_dynamic) < GAP_THRESHOLD and len(search_words) > 2:
            ...
            if gap_type == 'true_gap':
                ...
                # Insert at front so Reed sees it first
                ...
                # This helps Reed understand it's new territory, not a continuity break
                info_marker = {
                    'type': 'info_marker',
                    'confidence': 'inferred',
                ...
                # Insert at front so Reed sees it
        ...
        # === EPISODIC MEMORY MINIMUM ENFORCEMENT ===
        # Ensure at least 30% or 3 entries (whichever is larger) are episodic/conversation memories
        if final_memories:
            max_memories = len(final_memories)
            min_episodic = max(3, int(max_memories * 0.3))

            episodic = [m for m in final_memories if m.get("type") == "episodic" or m.get("is_conversation_turn", False)]
            non_episodic = [m for m in final_memories if m not in episodic]

            if len(episodic) < min_episodic:
                # Need more episodic memories - pull from all memories
                all_episodic = [m for m in self.memories if m.get("type") == "episodic" or m.get("is_conversation_turn", False)]
                all_episodic.sort(key=lambda m: m.get("timestamp", ""), reverse=True)

                needed = min_episodic - len(episodic)
                for mem in all_episodic:
                    if mem not in episodic and needed > 0:
                        episodic.append(mem)
                        needed -= 1

                # Rebuild: keep episodic + trim non-episodic to fit
                available_slots = max_memories - len(episodic)
                final_memories = episodic + non_episodic[:available_slots]

                if VERBOSE_DEBUG:
                    print(f"[EPISODIC BALANCE] Enforced minimum: {len(episodic)} episodic memories (min: {min_episodic})")
    ...
    def get_document_provenance(self, query: str) -> Optional[Dict]:
        ...
        Enables "spatial memory" - Reed can answer "where did you read about X?"
    ...
    def _determine_chunk_count(self, query: str) -> int:
        ...
        TOKEN BUDGET: Reed's context already uses ~95k chars before documents.
    ...
    def retrieve_rag_chunks(self, query: str, n_results: int = None) -> List[Dict[str, Any]]:
        ...
        This allows Reed to "remember" uploaded documents without storing
        ...
        try:
            ...
            if formatted_chunks and query:
                query_lower = query.lower().strip()
                query_words = set(query_lower.split())

                # For short queries (greetings, names), aggressively filter
                if len(query_words) <= 3:
                    filtered = []
                    for chunk in formatted_chunks:
                        chunk_text = chunk.get("text", "").lower() if isinstance(chunk, dict) else str(chunk).lower()
                        # Keep if any query word appears in chunk, or if relevance score > 0.75
                        score = chunk.get("distance", 0.5) if isinstance(chunk, dict) else 0.5
                        word_match = any(w in chunk_text for w in query_words if len(w) > 2)
                        if word_match or (isinstance(score, (int, float)) and score > 0.75):
                            filtered.append(chunk)

                    # Keep at least 3 chunks even if filtering is aggressive
                    if len(filtered) >= 3:
                        if len(filtered) < len(formatted_chunks):
                            print(f"[RAG FILTER] Short query: {len(formatted_chunks)} -> {len(filtered)} chunks")
                        formatted_chunks = filtered
                    elif filtered:
                        formatted_chunks = filtered
                    # else: keep original chunks as fallback

                # For medium queries, apply lighter filtering
                elif len(query_words) <= 8:
                    # Sort by relevance score if available, take top 60%
                    if formatted_chunks and isinstance(formatted_chunks[0], dict) and "distance" in formatted_chunks[0]:
                        formatted_chunks.sort(key=lambda c: c.get("distance", 0), reverse=True)
                        keep_count = max(5, int(len(formatted_chunks) * 0.6))
                        if keep_count < len(formatted_chunks):
                            print(f"[RAG FILTER] Medium query: {len(formatted_chunks)} -> {keep_count} chunks")
                            formatted_chunks = formatted_chunks[:keep_count]
    ...
    def store_document_summary(self, doc_id: str, filename: str, summary: str, entities: List[str]):
        ...
        This allows Reed to remember the "big picture" of a document
        ...
        summary_fact = {
            ...
            "doc_id": doc_id,
            "filename": filename,
            "fact": f"Reed read '{filename}': {summary}",
    ...
    def recall(self, agent_state, user_input, bias_cocktail=None, num_memories=30, use_multi_factor=True, include_rag=True, conversational_mode=False):
        ...
        grouped = {
            ...
            "reed": [m for m in memories if m.get("perspective") in ("reed", "kay")],
    ...
    def extract_and_store_user_facts(self, agent_state, user_input: str) -> List[Dict[str, str]]:
        """
        Extract facts from user input BEFORE Reed responds.
        ...
        CRITICAL: This prevents Reed from hallucinating when user provides facts.
        """
        import time
        ...
        self.memory_layers.add_memory(full_turn, layer="working")
        ...
        for fact_data in extracted_facts:
            ...
            # Track preferences if this is about Reed
            if fact_perspective == "reed":
                self.preference_tracker.track_preference(fact_text, fact_perspective, context="user_told_reed")
    ...
    def store_visual_memory(
        ...
        Args:
            image_description: Description of what Reed saw
            reed_response: Reed's full response to the image
            emotional_response: List of emotions Reed reported feeling
        ...
        memory_entry = {
            ...
            'perspective': 'shared',  # Visual shared between Re and Reed
            'emotional_context': emotional_response or [],
            'valence': valence,
            'reed_response_excerpt': reed_response[:200] if reed_response else '',
        ...
        if entities_detected and self.entity_graph:
            ...
            # Track relationships
            self.entity_graph.add_relationship('Reed', 'witnessed', visual_entity_id)
    ...
    def log_memory_entry(self, conversation_turn: dict, agent_state, memory_stack: list = None) -> Dict[str, Any]:
        ...
        if speaker == "user":
            ...
            # Reed's response - extract from perspective of Reed speaking
            extracted_facts = self._extract_facts_with_entities("", raw_text)
...
Generate interpretation now:"""
    ...
    def _calculate_base_score(self, mem: Dict[str, Any], bias_cocktail: dict, user_input: str) -> float:
        ...
        query_entities = [word for word in search_words if word[0].isupper() or word in ["re", "reed", "kay"]]
