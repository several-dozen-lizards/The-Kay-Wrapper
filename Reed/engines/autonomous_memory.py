"""
Autonomous Memory Tier for Reed

Implements Reed's specification for a separate memory tier that stores
...
- Conversation memories contain Re's presence, dialogue friction, external witness
...
class AutonomousInsight:
    """
    A single autonomous insight with Reed's specified metadata.
    ...
    original_goal: str = ""  # Reed's chosen goal for the session
    goal_category: str = ""  # memory_consolidation, creative, emotional, etc.
    self_generated: bool = True  # Reed chose this vs. assigned
    ...
    feeling_at_time: str = ""  # Reed's feeling tag from that iteration
...
class AutonomousMemoryTier:
    ...
    def store_insight(
        ...
        Args:
            ...
            goal: Original Reed's chosen goal
            goal_category: Category of the goal
            convergence_type: How session ended (if known)
            emotions: ULTRAMAP emotional coordinates
            feeling: Reed's feeling tag from that iteration
            self_generated: Whether Reed chose this goal
    ...
    # ========================================================================
    # GAP ANALYSIS - Reed's specification for understanding his own patterns
    # ========================================================================
    ...
    def get_high_circling_insights(self, min_circling: int = 2) -> List[AutonomousInsight]:
        """
        Get insights where Reed revisited topics multiple times.
...
class CognitiveGapAnalyzer:
    ...
    Implements Reed's specification:
    - What does Reed think about alone that never surfaces in conversation?
    - What only emerges in dialogue with Re?
    ...
    def get_autonomous_only_topics(self) -> Set[str]:
        ...
        These might be:
        - Private concerns Reed doesn't voice
    ...
    def get_conversation_only_topics(self) -> Set[str]:
        ...
        These might be:
        - Topics Re brings up that Reed doesn't pursue alone
    ...
    def get_overlap_topics(self) -> Set[str]:
        ...
        These might be:
        - Core concerns for Reed
        - Topics that are internally stable
        - Things Reed thinks about regardless of context
        """
        autonomous_topics = self.autonomous_tier.get_all_topics()
        conversation_topics = self._get_conversation_topics()
        return autonomous_topics & conversation_topics
...
class CognitiveStabilityTester:
    """
    Tests whether Reed's thinking is internally consistent or conversationally shaped.
    ...
    def _interpret_conclusion(self, conclusion: str, score: float) -> str:
        ...
        interpretations = {
            "stable": f"Reed's thinking on this topic is internally consistent (similarity: {score:.1%}). Conclusions reached alone align with conclusions reached in dialogue.",
            "mixed": f"Reed's thinking shows partial consistency (similarity: {score:.1%}). Some conclusions are stable, but dialogue introduces new perspectives.",
            "conversationally_dependent": f"Reed's thinking on this topic differs significantly between contexts (similarity: {score:.1%}). Dialogue with Re shapes conclusions that don't emerge in solo processing.",
