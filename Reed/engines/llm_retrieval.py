...
def select_relevant_documents(
    ...
    if is_meta_document_query:
        prompt = f"""You are helping Reed list ALL available documents.

The user is asking about what documents Reed has access to. This is a meta-query about documents themselves.
...
so Reed can tell the user about all available documents.
"""
    else:
        prompt = f"""You are helping Reed find relevant documents to answer a query.
...
"""
    ...
    if emotional_state:
        prompt += f"\nReed's current emotional state: {emotional_state}\n"
...
"""
    ...
    if context['request_type'] == 'reading':
        ...
        prompt += f"- Request type: ANALYSIS (user wants Reed's thoughts/opinions)\n"
        prompt += f"- Include documents that Reed should analyze\n"
    ...
    prompt += f"""
Which documents should Reed load to respond to this query?

Selection Guidelines:
- These are CURRENT SESSION documents only - historical documents already filtered out
- For casual conversation ("Hey Reed, how are you?"), return NONE unless documents are clearly relevant
