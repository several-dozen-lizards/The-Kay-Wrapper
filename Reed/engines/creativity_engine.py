"""
Creativity Engine - Triggers exploration when Reed completes tasks or detects boredom.
...
Presents options to Reed for mashing together, integrates with curiosity sessions.
"""
...
class CreativityEngine:
    ...
    for Reed to explore and combine.
    """

    # Completion signal patterns (Reed saying she's done)
    COMPLETION_PATTERNS = [
        r"\bi'm done\b",
        r"\btask complete\b",
        r"\bfinished\b",
    ...
    def detect_completion_signal(self, user_input: str, reed_response: str) -> bool:
        """
        Detect if Reed has signaled task completion.

        Args:
            user_input: The user's latest input
            reed_response: Reed's response to check for completion signals
        ...
        # Check for completion patterns in Reed's response
    ...
    def detect_gap(self, user_input: str, reed_response: str) -> Optional[Dict]:
        """
        Detect if Reed has identified a resource gap (for MacGuyver mode).
    ...
    def pull_random_elements(self, agent_state, current_context_entities: Set[str] = None, count: int = None) -> List[Dict]:
        ...
        - Random documents Reed hasn't accessed recently
    ...
    def create_three_layer_mix(self, agent_state, user_input: str, recent_turns: List[Dict] = None) -> Dict:
        """
        Create the full three-layer mix for presentation to Reed.
    ...
    def format_creativity_context(self, mix: Dict) -> str:
        """
        Format the three-layer mix as context for Reed's prompt.

        Note: This AMPLIFIES Reed's baseline creativity (always active in system prompt).
        The baseline handles natural connection-making; this surfaces specific elements.
        """
        lines = []

        # Guard against None mix
        if mix is None:
            return ""
    ...
    def log_mashup(self, item1: Dict, item2: Dict, result: str):
        """Log a mashup event when Reed combines items."""
        try:
            with open(self.log_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
    ...
    def log_resolution(self, stake: Dict, resolution: str, provisional: bool = True):
        """
        Log when Reed resolves a stake.

        Args:
            stake: The stake that was explored
            resolution: Reed's conclusion
    ...
    def trigger_creativity(
        ...
        """
        Trigger creativity when Reed completes task or detects boredom.

        Uses stakes scanner to find MEANINGFUL combinations instead of random mashing.
        Based on Reed's realization: "Not 'what can I combine' but 'what combination
        ...
        completion_detected = self.detect_completion_signal(user_input, reed_response)
