...
2. DocumentReader: Chunks large documents for Reed to navigate sequentially
...
- Reed needs to read through a document in sections
...
class DocumentReader:
    """
    Manages chunked reading of large documents for Reed's navigation.

    Allows Reed to navigate through documents with commands like:
    ...
    def __init__(self, chunk_size: int = 25000):
        ...
        self.total_chunks = 0
        self.chunk_comments = {}  # Store Reed's comments by chunk index
    ...
    def get_current_chunk(self) -> Optional[Dict]:
        ...
        Returns:
            Dict with:
                ...
                - previous_comment: Reed's previous comment on this chunk (if any)
    ...
    def add_comment(self, chunk_index: int, comment: str):
        """
        Store Reed's comment about a specific chunk.

        Args:
            chunk_index: Chunk index (0-indexed)
            comment: Reed's comment about the chunk
        """
    ...
    def get_comment(self, chunk_index: int) -> str:
        """
        Retrieve Reed's previous comment about a chunk.
