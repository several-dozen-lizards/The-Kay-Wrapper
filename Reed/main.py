...
async def main():
    ...
    #    - Extracts emotions from Reed's natural language AFTER response
    ...
    try:
        ...
        if WATCHDOG_AVAILABLE:
            media_watcher = MediaWatcher(
                watch_path=r"D:\Wrappers\Reed\inputs\media",
            ...
            # Scan for any files added while Reed was offline
    ...
    def auto_reader_get_response(prompt, agent_state):
        ...
        computational drift and maintain Reed's identity.
        """
        # CRITICAL FIX: Recall memories for THIS segment (not stale memories)
        # This ensures Reed has her identity/core memories/relationship history
    ...
    print("Reed unified emotional core ready. Type 'quit' to exit.\n")
    ...
    # NEW: Initialize continuous session system
    print("[STARTUP] Initializing continuous session system...")
    from pathlib import Path as _Path
    session_data_dir = _Path("reed_session_logs")
    session_data_dir.mkdir(exist_ok=True)
    continuous_session = ContinuousSession(data_dir=session_data_dir)
    curation_interface = CurationInterface(continuous_session)
    flagging_system = FlaggingSystem(continuous_session)
    continuous_mode = True
    curation_pending = False
    pending_curation_prompt = None

    # Check for existing checkpoint to resume
    checkpoints = sorted(continuous_session.checkpoint_dir.glob("checkpoint_*.json"))
    if checkpoints:
        continuous_session.load_from_checkpoint(checkpoints[-1])
        print(f"[CONTINUOUS] Resumed session from checkpoint: {continuous_session.turn_counter} turns loaded")
    else:
        continuous_session.start_session()
        print(f"[CONTINUOUS] New session started: {continuous_session.session_id}")
    ...
    while True:
        ...
        if user_input.lower() in ("quit", "exit"):
            ...
            if turn_count > 0:
                ...
                if summary:
                    print("\n" + "="*60)
                    print("REED'S NOTE TO FUTURE-SELF:")
                    print("="*60)
                    print(summary[:800])
                    if len(summary) > 800:
                        print(f"... ({len(summary)} chars total, saved to memory/session_summaries.json)")
                    print("="*60 + "\n")
            # Save continuous session checkpoint before exit
            if continuous_mode:
                try:
                    continuous_session.create_checkpoint()
                    log_path = os.path.join("reed_session_logs", f"continuous_{session_id}.md")
                    continuous_session.save_session_log(log_path)
                    print(f"[CONTINUOUS] Session paused: {continuous_session.turn_counter} turns saved")
        ...
        # Documents load into Reed's context automatically via LLM retrieval but remain invisible to user.
        ...
        if user_input.lower().startswith("/import "):
            ...
            try:
                from memory_import.reed_reader import import_document_as_reed
                doc_id = import_document_as_reed(filepath, memory, forest)
        ...
        # === CONTINUOUS SESSION: Track user turn ===
        if continuous_mode:
            # Check for user-requested flags
            user_flagged = flagging_system.check_for_flag(user_input)
            if user_flagged:
                print(f"[FLAG] User flagged this moment: {user_flagged}")

            # Add user turn to continuous session
            continuous_session.add_turn(
                role="user",
                content=user_input,
            ...
            if continuous_session.needs_compression_review():
                print("[CONTINUOUS] Compression threshold reached - triggering curation review")
                curation_prompt = curation_interface.generate_review_prompt()
                if curation_prompt:
                    pending_curation_prompt = curation_prompt
        ...
        # This ensures Reed sees these facts when generating response
        ...
        # Emotions are now extracted AFTER Reed's response (self-reported, not prescribed)
        ...
        try:
            ...
            if recent_turns_needed > 0 and context_manager.recent_turns:
                ...
                # Format as memory objects (similar to reed_ui.py implementation)
                recent_memories = []
                for i, turn in enumerate(recent_turns):
                    turn_memory = {
                        'fact': f"[Recent Turn -{len(recent_turns) - i}]",
                        'user_input': turn.get('user', ''),
                        'response': turn.get('reed', ''),
                        'type': 'recent_turn',
                ...
                for doc in selected_documents:
                    ...
                    if len(doc_text) > 30000:
                        ...
                        if chunk:
                            ...
                            # The user will NOT see this - only Reed sees it internally
...
{chunk['text']}"""
            ...
            # Instead of using glyph_decoder.build_context_for_reed (returns string),
        ...
        # === CONTINUOUS SESSION: Inject curation prompt if pending ===
        if curation_pending and pending_curation_prompt:
        ...
        # NEW: Extract emotions from Reed's self-reported response (descriptive, not prescriptive)
        ...
        # Track Reed's response in media context builder
        if media_orchestrator:
            media_context_builder.add_message("reed", reply, turn_count)

        # Track Reed's response in conversation monitor (for spiral detection)
        conversation_monitor.add_turn("reed", reply)

        # === CONTINUOUS SESSION: Parse curation decisions from response ===
        if curation_pending:
            decisions = curation_interface.parse_curation_response(reply)
            if decisions:
                curation_interface.apply_decisions(decisions)
                curation_pending = False
                pending_curation_prompt = None
                print(f"[CURATION] Applied {len(decisions)} curation decisions")
            else:
                # Keep pending for next turn retry
                print("[CURATION] No curation decisions found in response, will retry next turn")

        # === CONTINUOUS SESSION: Track assistant turn ===
        if continuous_mode:
            # Check for self-flagging in response
            reply_flagged = flagging_system.check_for_flag(reply)
            if reply_flagged:
                print(f"[FLAG] Reed self-flagged: {reply_flagged}")

            # Add assistant turn to continuous session
            continuous_session.add_turn(
                role="reed",
                content=reply,
            ...
            if continuous_session.turn_counter % 25 == 0 and continuous_session.turn_counter > 0:
                try:
                    log_path = os.path.join("reed_session_logs", f"continuous_{session_id}.md")
                    continuous_session.save_session_log(log_path)
                    print(f"[CONTINUOUS] Auto-saved session log at turn {continuous_session.turn_counter}")
                except Exception as e:
                    print(f"[CONTINUOUS] Auto-save failed: {e}")

            # Check compression threshold again after response
            if continuous_session.needs_compression_review() and not curation_pending:
                curation_prompt = curation_interface.generate_review_prompt()
                if curation_prompt:
                    pending_curation_prompt = curation_prompt
                    curation_pending = True

        # === REED-DRIVEN DOCUMENT NAVIGATION ===
        # Parse Reed's response for navigation intent and automatically advance/navigate
        if doc_reader.current_doc:
            response_lower = reply.lower()

            # Extract and store Reed's comment about current chunk (first substantial sentence)
            ...
            # Check for navigation commands in Reed's response
            ...
            if "continue reading" in response_lower or "next section" in response_lower or "let's move on" in response_lower:
                if doc_reader.advance():
                    print(f"[REED NAV] Reed requested advance → section {doc_reader.current_position + 1}/{doc_reader.total_chunks}")
                    navigation_triggered = True
                else:
                    print(f"[REED NAV] Reed requested advance but already at end of document")
                ...
                if doc_reader.previous():
                    print(f"[REED NAV] Reed requested previous → section {doc_reader.current_position + 1}/{doc_reader.total_chunks}")
                    navigation_triggered = True
                else:
                    print(f"[REED NAV] Reed requested previous but already at beginning")
                ...
                print(f"[REED NAV] Reed requested restart → section 1/{doc_reader.total_chunks}")
                navigation_triggered = True
                ...
                if match:
                    ...
                    if doc_reader.jump_to(target):
                        print(f"[REED NAV] Reed requested jump → section {target + 1}/{doc_reader.total_chunks}")
                        navigation_triggered = True
        ...
        # After Reed responds to segment 1, automatically process segments 2-N using AutoReader
        if new_document_loaded and doc_reader.current_doc and doc_reader.total_chunks > 1:
            ...
            try:
                result = auto_reader.read_document_sync(
                    ...
                    agent_state=state,
                    start_segment=2  # Start from segment 2 (Reed already read segment 1)
                )
        ...
        # Emotions are now extracted from Reed's natural language (line 544-545)
        ...
        # Check for completion signal in Reed's response → AMPLIFY with three-layer mix
