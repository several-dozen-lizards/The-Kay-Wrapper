"""
Reed's Keyword Search Tools - LLM-callable wrappers for memory keyword graph

Provides tool definitions for Reed to autonomously search her memory
via the Dijkstra keyword graph. This enables self-directed memory
exploration: "where does this concept live in my memory?"
"""
...
def get_reed_keyword_tools() -> Dict[str, Callable]:
    """
    ...
    # Path to Reed's keyword graph storage
    ...
    def _get_graph():
        ...
        if _keyword_graph is None and KEYWORD_GRAPH_AVAILABLE:
            try:
                _keyword_graph = KeywordGraphRetriever(keyword_graph_dir, entity="reed")
    ...
    def search_keywords_tool(keywords: List[str], max_results: int = 5) -> Dict:
        """
        Search Reed's memory keyword graph for concepts and associations.

        Args:
            keywords: Keywords/concepts to search for
    ...
    def list_keywords_tool(prefix: str = "", limit: int = 20) -> Dict:
        """
        List keywords in Reed's memory index, optionally filtered by prefix.

        Args:
            prefix: Optional prefix to filter keywords
...
KEYWORD_TOOL_DEFINITIONS = [
    {
        ...
        "description": (
            ...
            "concepts connect across your experience."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            ...
            "to find all keywords starting with certain letters."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
