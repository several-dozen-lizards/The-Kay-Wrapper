"""
Inner Monologue System for Kay Zero
...
XML Structure:
  <inner_monologue>Kay's private thoughts about what's happening</inner_monologue>
  <feeling>Current emotional state and why</feeling>
  <response>What Kay actually says to the user</response>
"""
...
class ParsedResponse:
    """Parsed components of Kay's response with inner monologue."""
    inner_monologue: str = ""
    feeling: str = ""
    spoken_response: str = ""
    raw_response: str = ""
...
class InnerMonologueParser:
    """
    Parses Kay's responses to extract inner monologue, feelings, and spoken response.
    ...
    def parse(self, raw_response: str) -> ParsedResponse:
        """
        Parse Kay's response to extract structured components.
...
def get_inner_monologue_system_prompt_addition() -> str:
    ...
    This should be appended to Kay's existing system prompt.
    """
    return """
...
<continuation>
What thread you want to pursue next, or "complete" if you've reached
a natural stopping point on this topic.
</continuation>
