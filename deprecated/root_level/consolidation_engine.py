"""
Kay Zero Consolidation Engine

Handles memory consolidation (Kay's 'sleep' process) - converts full conversations
...
class ConsolidationEngine:
    """Handles memory consolidation (Kay's 'sleep' process)"""
    ...
    def __init__(self, llm_client=None):
        ...
        # If no client provided, use Kay's existing LLM integration
        if self.llm is None:
            try:
...
Extract 3-5 consolidated memories that capture what MATTERS. These become Kay's memories of this conversation.
...
Return ONLY valid JSON array, no explanation."""
        ...
        # Use Kay's existing LLM integration
        if self.llm_query:
            try:
