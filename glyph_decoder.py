"""
Glyph Decoder for Kay Zero
Translates compressed glyph output from Filter LLM into natural language context
Expands symbolic representations into structured data for Kay's response generation
"""
...
class GlyphDecoder:
    ...
    def build_context_for_kay(self, decoded: Dict, user_input: str) -> str:
        """
        Build natural language context block for Kay's prompt.

        This is what Kay actually sees - not the glyphs, but their expanded meaning.
        """
        context_blocks = []
        ...
        if decoded.get("rag_chunks"):
            ...
            print(f"[DECODER] Including {len(decoded['rag_chunks'])} RAG chunks in Kay's context")
